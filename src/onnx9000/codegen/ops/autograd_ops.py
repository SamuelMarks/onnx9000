...
def generate_adamw(node: Node, ctx: Generator) -> str:
    ...
    lr = node.attributes.get("lr", 0.001)  # pragma: no cover
    beta1 = node.attributes.get("beta1", 0.9)  # pragma: no cover
    beta2 = node.attributes.get("beta2", 0.999)  # pragma: no cover
    eps = node.attributes.get("eps", 1e-8)  # pragma: no cover
    weight_decay = node.attributes.get("weight_decay", 0.01)  # pragma: no cover
    step_t = node.attributes.get(  # pragma: no cover
        "step_t", 1.0
    )  # Should be a dynamic tensor but hardcoded here for mock

    pragma = get_omp_pragma(f"{param}.size()")  # pragma: no cover
