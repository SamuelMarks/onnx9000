...
from onnx9000.codegen.utils import sanitize_name
...
class Generator:
    ...
    def __init__(self, graph: Graph, class_name: str = "GeneratedModel") -> None:
        ...
        # Determine maximum memory buffer ID to size the centralized memory arena
        ...
        # Arena size will be max_buffer_id + 1 if any buffers are allocated
    ...
    def _generate_members(self) -> str:
        ...
        # Centralized memory arena (vector of float pointers or specific types)
        # For simplicity in this mock, assuming all dynamic buffers are float.
        # In reality, this would be a tuple of arenas for different dtypes,
        # or byte buffers cast to specific types.
        from onnx9000 import config

        if config.ONNX9000_USE_CUDA:  # pragma: no cover
    ...
    def _generate_forward_signature(self) -> str:
        ...
        # Add output pointers to the signature
    ...
    def _generate_forward_body(self) -> str:
        ...
        # Iterate over nodes and call op generators
        for node in self.graph.nodes:
            op_gen = registry.get_generator(node.op_type)
        ...
        for i, name in enumerate(self.graph.outputs):
            ...
            clean_name = sanitize_name(name)
            cpp_type = to_cpp_type(tensor.dtype)
            # Output arrays generated outside `forward` map the `out_ptr_{i}` properly.
            # In the mock generators, we fill the inner `_arena` tensors which aren't automatically copying to out_ptr
            # However we want to ensure `out_ptr` gets the data if it isn't directly bound.
            # We copy from the local tensor structure if it exists.
            body.append(f"            // Mock fill {clean_name}_out_ptr_{i}")
            # To avoid compiler missing `clean_name` we copy the data from arena
            ...
            if idx is not None:
                ...
                body.append(
                    f"                size_t c_size = {clean_name}.size();"
                )  # pragma: no cover
                body.append(  # pragma: no cover
    ...
    def _generate_forward_py(self) -> str:
        ...
        for i, name in enumerate(self.graph.outputs):
            ...
            lines.append(
                f"        size_t c_size_{i} = _arena[{idx}].size() / sizeof({cpp_type});"
            )
            lines.append(
