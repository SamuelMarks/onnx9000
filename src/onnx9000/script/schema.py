...
class OpSchema:
    """Represents the schema of an ONNX operation, including its inputs, outputs, and attributes."""
...
# Basic opset 18 schema setup for a few ops to satisfy tests
registry.register(OpSchema("Add", 14, ["A", "B"], ["C"], []))
registry.register(OpSchema("Relu", 14, ["X"], ["Y"], []))
registry.register(
    OpSchema("Loop", 13, ["M", "cond", "v_initial"], ["v_final"], ["body"])
)
registry.register(
    OpSchema("If", 13, ["cond"], ["outputs"], ["then_branch", "else_branch"])
)
...
def validate_op(op_type: str, inputs: List[Any], attributes: Dict[str, Any]) -> None:
    ...
    # Auto-generation and upcast logic can be added here
    # Check if operation exists in target opset
    schema = registry.get_schema(op_type, _target_opset)
    if schema is None:
        if op_type in registry.schemas:
            # Op exists but not in this version!
        ...
        # If op not in registry at all, we just allow it for custom ops / test mockups
        return
    ...
    # Handle auto-upcasting (e.g. axes attribute became input in opset 13 for Squeeze)
    if op_type == "Squeeze" and _target_opset >= 13 and "axes" in attributes:
        # User passed axes as attribute, but it should be an input in Opset 13!
        # For an authoring API, we could automatically construct a Constant node for it.
        import warnings

        warnings.warn(
            "Attribute 'axes' will be converted to input for Squeeze in opset 13+"
        )
