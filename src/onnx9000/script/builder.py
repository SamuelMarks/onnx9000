...
class GraphBuilder:
    ...
    def set_metadata(
        self, doc_string: str = "", domain: str = "", version: int = 1
    ) -> None:
    ...
    def add_input(
        self, name: str, dtype: DType, shape: tuple[Union[int, str], ...]
    ) -> Var:
    ...
    def replace_input(self, node: Node, old_var: Var, new_var: Var) -> None:
        ...
        if node in self.nodes:
            node.inputs = [
                new_var.name if x == old_var.name else x for x in node.inputs
            ]
    ...
    def rename_all(self, prefix: str) -> None:
        ...
        # Rename nodes and their connections
        new_node_by_name = {}
        ...
        for node in self.nodes:
            ...
            for k, v in node.attributes.items():
                if isinstance(v, GraphBuilder):
                    # We might need to propagate inputs down if they are implicitly captured
                    # But for now, just call infer_shapes on it
                    v.infer_shapes()
            ...
            if node.op_type in ("Add", "Sub", "Mul", "Div", "Relu", "Sigmoid"):
                if node.inputs and node.inputs[0] in tensor_shapes:
                    for out in node.outputs:
                        ...
                        tensor_dtypes[out] = tensor_dtypes.get(
                ...
                # Simplistic propagation from subgraph outputs
                if "then_branch" in node.attributes:
                    sub_g = node.attributes["then_branch"]
                    # If shapes were inferred inside, they'd be available.
                    # ONNX actually unifies shapes from then/else branches.
    ...
    def extract_subgraph(self, inputs: List[str], outputs: List[str]) -> "GraphBuilder":
        ...
        required_tensors: Set[str] = set(outputs)
        ...
        while queue:
            ...
            if current_tensor in tensor_to_producer:
                ...
                if producer.name and producer.name not in required_nodes:
                    ...
                    # If this producer has subgraphs, they also need DCE
                    for k, v in producer.attributes.items():
                        if isinstance(v, GraphBuilder):
                            # The outputs of the subgraph are its own `outputs`
                            # We can extract the subgraph that produces its outputs from its implicit inputs
                            sub_outputs = [out["name"] for out in v.outputs]
                            sub_inputs = [inp["name"] for inp in v.inputs]
                            # implicitly captured variables would be visited if we traced them, but
                            # extract_subgraph requires explicit inputs right now.
                            # Just recursively call extract_subgraph
    ...
    def If(self, cond: Var, num_outputs: int = 1) -> Any:
        ...
        from onnx9000.script.control_flow import IfContextManager
    ...
    def Loop(self, max_trip_count: Var, cond: Var, num_outputs: int = 1) -> Any:
        ...
        from onnx9000.script.control_flow import LoopContextManager
    ...
    def to_onnx(self) -> pb.ModelProto:  # type: ignore
        ...
        for inp in self.inputs:
            ...
            if inp["dtype"] == DType.FLOAT32:
                ...
                tensor_type.elem_type = pb.TensorProto.FLOAT  # fallback
        ...
        for out in self.outputs:
            ...
            # Simple fallback type for output
            tensor_type = pb.TypeProto.Tensor()
            tensor_type.elem_type = pb.TensorProto.FLOAT
            value_info.type.tensor_type.CopyFrom(tensor_type)
    ...
    @classmethod
    def from_onnx(
        cls, model_proto: Union[pb.ModelProto, pb.GraphProto]
    ) -> "GraphBuilder":  # type: ignore
        """Import an existing .onnx file into a GraphBuilder for editing."""
        if hasattr(model_proto, "graph"):
            graph_proto = model_proto.graph
        else:
            graph_proto = model_proto
        ...
        for inp in graph_proto.input:
            ...
            # Simple fallback for parsing type
            dtype = (
                DType.FLOAT32 if inp.type.tensor_type.elem_type == 1 else DType.INT64
            )
