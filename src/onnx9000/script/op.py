...
class OpNamespace:
    ...
    def __getattr__(self, op_type: str) -> Any:
        ...
        # Lazy generator for node builder
        def _node_builder(*args: Any, **kwargs: Any) -> Union[Var, Tuple[Var, ...]]:
            ...
            for arg in args:
                if isinstance(arg, list) and op_type == "Concat":
                    # Concat expects a list of tensors as its first input
                    for a in arg:
                        inputs.append(_make_var(a))
                else:
                    inputs.append(_make_var(arg))

            # Special logic for some ops?
            # E.g., multi-output ops
            num_outputs = 1
            if op_type in ["TopK", "Split", "LSTM"]:
                # simplified multi-output heuristic
                # In a real system, we look up ONNX schema.
                if op_type == "TopK":
                    num_outputs = 2

            from onnx9000.script.schema import validate_op

            # Auto-upcast some kwargs to inputs based on schema version
...
# Step 25: Implement op.Constant
def Constant(value: Any) -> Var:
    """Implement op.Constant explicitly."""
    # Serialize Python values to appropriate ONNX representation (simulated here)
    if isinstance(value, np.ndarray):
        ...
        # Determine if int or float list
...
from onnx9000.script.schema import set_target_opset

op.Constant = Constant  # type: ignore
op.If = If  # type: ignore
op.Loop = Loop  # type: ignore
op.Scan = Scan  # type: ignore
op.set_target_opset = set_target_opset  # type: ignore

# Add, Sub, Mul, Div, etc. will all just flow through __getattr__ except if we want explicit type checks
