...
def to_cpp_type(dtype: DType) -> str:
    ...
    if dtype not in mapping:
        raise ValueError(f"No C++ type mapped for DType: {dtype}")  # pragma: no cover
...
def to_emscripten_type(dtype: DType) -> str:
    ...
    mapping = {  # pragma: no cover
        ...
        # Bool might map to Uint8Array in JS land
        DType.BOOL: "Uint8Array",
        DType.FLOAT16: "Uint16Array",
        DType.BFLOAT16: "Uint16Array",
    }
    if dtype not in mapping:  # pragma: no cover
        raise ValueError(
            f"No Emscripten TypedArray mapped for DType: {dtype}"
        )  # pragma: no cover
    return mapping[dtype]  # pragma: no cover
