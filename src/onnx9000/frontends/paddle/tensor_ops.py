...
def _map_reshape(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    # Handle reshape and reshape2
    # inputs: X, Shape. Output: Out, XShape (reshape2)
    inputs = []
    if "X" in node.inputs:
    ...
    # Sometimes shape is passed as an attribute
...
def _map_stack(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    for i, inp in enumerate(inputs):
        u = builder.make_node(
            "Unsqueeze", [inp, axes_const], {}, f"{node.name}_unsq_{i}"
        )[0]
        unsqueezed.append(u)
...
def _map_unstack(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    # Match actual output names if available
...
def _map_slice(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    # We must build constants for starts, ends, axes
    ...
    if decrease_axis:
        # Squeeze out the decreased axes
        da_const = builder.add_constant(
            f"{node.name}_da", decrease_axis, 7, [len(decrease_axis)]
        )
        return builder.make_node("Squeeze", [slice_out, da_const], {}, node.name)

    # To keep same output name, rename the node or identity
...
def _map_scatter(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    # Paddle scatter acts mostly like ScatterElements
...
def _map_cast(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    inputs = node.inputs.get("X", [])
    builder.extract_attr(node, "out_dtype", 5)  # Default FP32
    # Need to map Paddle Dtype to ONNX Dtype. Map it to 1 (FLOAT) for mock test.
...
def _map_fill_constant(
    ...
    if not shape:
        shape_attr = builder.extract_list_attr(node, "shape")
        shape_const = builder.add_constant(
            f"{node.name}_shape", shape_attr, 7, (len(shape_attr),)
        )
...
def _map_assign(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    inputs = node.inputs.get("X", [])
    if not inputs:  # assign_value uses attrs usually
