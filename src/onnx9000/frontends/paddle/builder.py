...
class PaddleToONNXGraphBuilder:
    ...
    def __init__(self, name: str = "paddle_to_onnx"):
        """Initialize."""
    ...
    def add_constant(
        self, name: str, value: Any, dtype: int, shape: tuple[int, ...]
    ) -> str:
    ...
    def infer_shape(
        self, node: PaddleNode, input_shapes: list[tuple[int, ...]]
    ) -> tuple[int, ...]:
    ...
    def extract_attr(
        self, node: PaddleNode, attr_name: str, default: Any = None
    ) -> Any:
    ...
    def flatten_lod_tensor(self, input_name: str, output_name: str) -> list[str]:
        # Paddle LoD tensors often need flattening into 1D for certain ops
        """Docstring for flatten_lod_tensor."""
        return self.make_node(
            "Flatten", [input_name], {}, output_name, outputs=[output_name]
        )
    ...
    def make_node(
        ...
        if not outputs:
            ...
            # We don't unique_name the provided outputs so we can predict them
    ...
    def make_node_optional_inputs(
        ...
        return self.make_node(
            op_type, resolved_inputs, attributes, name_prefix, outputs
        )
