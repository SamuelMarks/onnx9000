...
def _map_selu(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Selu", node.inputs, {"alpha": alpha, "gamma": gamma}, node.name
    )
...
def _map_conv2d(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    # Need to handle NHWC -> NCHW translation
    ...
    # Weight in TF is HWIO, ONNX needs OIHW
    ...
    # Transpose back if we were NHWC
    if data_format == "NHWC":
        conv_out = builder.make_node(
...
def _map_depthwise_conv2d_native(
...
def _map_conv2d_backprop_input(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_conv3d_backprop_input_v2(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_batch_norm(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "BatchNormalization", node.inputs, {"epsilon": epsilon}, node.name
    )
