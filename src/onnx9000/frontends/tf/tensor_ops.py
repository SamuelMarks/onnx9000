...
def _map_squeeze(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    if axes:
        axes_tensor = builder.add_constant(
            f"{node.name}_axes", axes, 7, (len(axes),)
        )  # INT64
        return builder.make_node(
            "Squeeze", [node.inputs[0], axes_tensor], {}, node.name
        )
    else:
...
def _map_tensor_scatter_update(
...
def _map_cast(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    to_dtype = builder.extract_attr(node, "DstT", 1)  # default FLOAT
...
TENSOR_OPS_MAPPING: Dict[str, Callable[[TFToONNXGraphBuilder, TFNode], List[str]]] = {
    ...
    "BroadcastArgs": _map_matrix_diag,  # custom
    "Where": _map_where,
    "Select": _map_select,
