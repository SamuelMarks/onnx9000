...
def _map_crop_and_resize(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    # Very complex op, maps generally to RoiAlign or custom subgraph
    return builder.make_node("RoiAlign", node.inputs, {}, node.name)


def _map_extract_image_patches(
...
def _map_random_standard_normal(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_truncated_normal(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    # Custom subgraph usually handles the truncation correctly.
    # For IR we map to custom node or a Normal + Clip
