...
# Keras layers are often mapped as composite Subgraphs or specialized Custom ops,
# since a Keras node contains many internal variables and state.


def _map_keras_dense(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    """Executes the  map keras dense operation."""
    # Custom_Dense -> Simplifier decomposes to MatMul + Add
...
def _map_keras_separable_conv1d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras separable conv1d operation."""
    return builder.make_node(
        "Custom_KerasSeparableConv1D", node.inputs, node.attr, node.name
    )


def _map_keras_separable_conv2d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras separable conv2d operation."""
    return builder.make_node(
        "Custom_KerasSeparableConv2D", node.inputs, node.attr, node.name
    )


def _map_keras_depthwise_conv2d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras depthwise conv2d operation."""
    return builder.make_node(
        "Custom_KerasDepthwiseConv2D", node.inputs, node.attr, node.name
    )


def _map_keras_conv2d_transpose(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras conv2d transpose operation."""
    return builder.make_node(
        "Custom_KerasConv2DTranspose", node.inputs, node.attr, node.name
    )


def _map_keras_conv3d_transpose(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras conv3d transpose operation."""
    return builder.make_node(
        "Custom_KerasConv3DTranspose", node.inputs, node.attr, node.name
    )
...
def _map_keras_max_pooling_1d(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasMaxPooling1D", node.inputs, node.attr, node.name
    )
...
def _map_keras_max_pooling_2d(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasMaxPooling2D", node.inputs, node.attr, node.name
    )
...
def _map_keras_max_pooling_3d(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasMaxPooling3D", node.inputs, node.attr, node.name
    )
...
def _map_keras_avg_pooling_1d(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasAvgPooling1D", node.inputs, node.attr, node.name
    )
...
def _map_keras_avg_pooling_2d(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasAvgPooling2D", node.inputs, node.attr, node.name
    )
...
def _map_keras_avg_pooling_3d(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasAvgPooling3D", node.inputs, node.attr, node.name
    )


def _map_keras_global_max_pooling_1d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras global max pooling 1d operation."""
    return builder.make_node(
        "Custom_KerasGlobalMaxPooling1D", node.inputs, node.attr, node.name
    )


def _map_keras_global_max_pooling_2d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras global max pooling 2d operation."""
    return builder.make_node(
        "Custom_KerasGlobalMaxPooling2D", node.inputs, node.attr, node.name
    )


def _map_keras_global_max_pooling_3d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras global max pooling 3d operation."""
    return builder.make_node(
        "Custom_KerasGlobalMaxPooling3D", node.inputs, node.attr, node.name
    )


def _map_keras_global_avg_pooling_1d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras global avg pooling 1d operation."""
    return builder.make_node(
        "Custom_KerasGlobalAvgPooling1D", node.inputs, node.attr, node.name
    )


def _map_keras_global_avg_pooling_2d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras global avg pooling 2d operation."""
    return builder.make_node(
        "Custom_KerasGlobalAvgPooling2D", node.inputs, node.attr, node.name
    )


def _map_keras_global_avg_pooling_3d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map keras global avg pooling 3d operation."""
    return builder.make_node(
        "Custom_KerasGlobalAvgPooling3D", node.inputs, node.attr, node.name
    )
...
def _map_keras_bidirectional(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasBidirectional", node.inputs, node.attr, node.name
    )
...
def _map_keras_batch_normalization(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_keras_layer_normalization(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_keras_activation(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasActivation", node.inputs, node.attr, node.name
    )
...
def _map_keras_repeat_vector(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    return builder.make_node(
        "Custom_KerasRepeatVector", node.inputs, node.attr, node.name
    )
...
def _map_keras_cropping(dim: str) -> Callable:
    ...
    def _impl(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
        """Executes the  impl operation."""
        return builder.make_node(
            f"Custom_KerasCropping{dim}", node.inputs, node.attr, node.name
        )
...
def _map_keras_upsampling(dim: str) -> Callable:
    ...
    def _impl(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
        """Executes the  impl operation."""
        return builder.make_node(
            f"Custom_KerasUpSampling{dim}", node.inputs, node.attr, node.name
        )
...
def _map_keras_zero_padding(dim: str) -> Callable:
    ...
    def _impl(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
        """Executes the  impl operation."""
        return builder.make_node(
            f"Custom_KerasZeroPadding{dim}", node.inputs, node.attr, node.name
        )
