...
class TFImporter:
    ...
    def convert_nhwc_to_nchw(self, input_name: str, node_name: str) -> str:
        ...
        self.graph.tensors[t_out] = Tensor(
            name=t_out, dtype=DType.FLOAT32.value, shape=()
        )
        return t_out
    ...
    def parse(self, graph_def: Dict[str, Any]) -> Graph:
        ...
        for node in graph_def.get("node", []):
            ...
            if op == "Placeholder":
                ...
                self.graph.tensors[w_out] = Tensor(
                    name=w_out, dtype=DType.FLOAT32.value, shape=()
                )
...
def load_tf(model_path_or_dict: Any) -> Graph:
    ...
    raise NotImplementedError("Parsing physical file not implemented in mock")
