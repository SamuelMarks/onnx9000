...
# TFLite op codes are typically integer enums in FlatBuffers,
# but we assume the parser extracts them as uppercase strings like "CONV_2D"
...
def _map_tflite_pool(op_type: str) -> Callable:
    ...
    def _impl(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
        """Executes the  impl operation."""
        # In TFLite, pools are typically NHWC.
        # This mirrors TF logic, but parameters might be structured differently.
        return builder.make_node(
            f"Custom_TFLite{op_type}", node.inputs, node.attr, node.name
        )
...
def _map_tflite_depthwise_conv2d(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map tflite depthwise conv2d operation."""
    return builder.make_node(
        "Custom_TFLiteDepthwiseConv2D", node.inputs, node.attr, node.name
    )


def _map_tflite_fully_connected(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
    """Executes the  map tflite fully connected operation."""
    # MatMul + BiasAdd usually
    return builder.make_node(
        "Custom_TFLiteFullyConnected", node.inputs, node.attr, node.name
    )
...
def _map_tflite_resize_bilinear(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_tflite_softmax(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
    ...
    # TFLite softmax implies axis=-1 (last dimension)
...
def _map_tflite_embedding_lookup(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_tflite_l2_normalization(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_tflite_space_to_depth(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_tflite_depth_to_space(
    builder: TFToONNXGraphBuilder, node: TFNode
) -> List[str]:
...
def _map_tflite_custom_subgraph(op: str) -> Callable:
    ...
    def _impl(builder: TFToONNXGraphBuilder, node: TFNode) -> List[str]:
        """Executes the  impl operation."""
        return builder.make_node(
            f"Custom_TFLite{op}", node.inputs, node.attr, node.name
        )
