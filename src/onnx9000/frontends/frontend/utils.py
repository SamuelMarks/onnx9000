...
def infer_elementwise_shape(
    ...
    for d1, d2 in zip(s1, s2):
        if d1 == 1:
            ...
            out_shape.append(d2 if d1 == 1 else d1)  # simplified dynamic dim
        else:
            raise ValueError(
...
def record_op(op_type: str, inputs: list[Any], attributes: dict = None) -> Any:
    ...
    from onnx9000.frontends.frontend.builder import get_active_builder
    ...
    if op_type in ("Add", "Sub", "Mul", "Div"):
        ...
        perm = attributes.get("perm", None) if attributes else None
        if perm:
