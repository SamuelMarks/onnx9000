...
def add_mse_loss(
    ...
    if reduction == "mean":
        ...
        graph.add_node(
            Node("Identity", [sq], [loss_out], {}, name=f"{loss_out}_identity")
        )
...
def add_crossentropy_loss(
    ...
    # SoftmaxCrossEntropyLoss is available in ONNX natively for training or via Softmax + NLLLoss
    graph.add_node(
        Node(
...
def add_bce_with_logits_loss(
    ...
    graph.add_node(
        Node("Sigmoid", [logits], [f"{loss_out}_sig"], {}, name=f"{loss_out}_sig_node")
    )
    # P * log(Q) + (1-P) * log(1-Q) mock
    graph.add_node(
        Node(
...
def add_l1_loss(
    ...
    if reduction == "mean":
        ...
        graph.add_node(
            Node("Identity", [abs_diff], [loss_out], {}, name=f"{loss_out}_identity")
        )
