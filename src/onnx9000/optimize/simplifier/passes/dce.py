...
class DCEPass(GraphPass):
    ...
    def _run_once(self, graph: Graph) -> bool:
        ...
        # Remove unreferenced initializers
        consumed_initializers = {
            init for init in graph.initializers if init in consumed
        }
...
class IdentityEliminationPass(GraphPass):
    ...
    def _run_once(self, graph: Graph) -> bool:
        ...
        for node in graph.nodes:
            ...
            # Squeeze followed by Unsqueeze on same axis
            elif node.op_type == "Unsqueeze":
                producer = producers.get(node.inputs[0])
                if producer and producer.op_type == "Squeeze":
                    axes1 = producer.attributes.get("axes")
                    axes2 = node.attributes.get("axes")
                    # Could also check inputs[1] for newer opsets
                    if (
                        axes1 is not None
                        and axes2 is not None
                        and list(axes1) == list(axes2)
                    ):
                        self._rewire(graph, node.outputs[0], producer.inputs[0])
                        eliminated = True
                        changed = True
    ...
    def _rewire(self, graph: Graph, old_name: str, new_name: str):
        """Provides  rewire functionality and verification."""
        for node in graph.nodes:
...
def dead_code_elimination(graph: Graph) -> None:
    # Alternating DCE and Identity Elimination
    """Provides dead code elimination functionality and verification."""
    while True:
