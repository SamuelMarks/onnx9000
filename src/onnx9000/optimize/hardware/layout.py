...
class LayoutOptimizer:
    ...
    @staticmethod
    def inject_transpose(
        graph: Graph, input_name: str, output_name: str, perm: List[int]
    ) -> Node:
        """Helper to inject a Transpose node."""
        node_name = f"Transpose_{input_name}_{output_name}"
        t_node = Node(
            "Transpose", [input_name], [output_name], {"perm": perm}, name=node_name
        )
    ...
    @staticmethod
    def inject_transposes_for_layout(
        graph: Graph, target_layout: str = "NHWC"
    ) -> Graph:
            ...
            if LayoutOptimizer.supports_layout_attribute(node):
                ...
                if current_layout != target_layout:
                    if target_layout == "NHWC" and current_layout == "NCHW":
                        # Needs Transpose from NCHW -> NHWC before, and NHWC -> NCHW after
                        new_inputs = []
                        ...
                        new_graph.add_node(
                            Node(
                                node.op_type, new_inputs, new_outputs, attrs, node.name
                            )
                        )
            ...
            if node.op_type == "Transpose":
                ...
                if len(cons) == 1 and cons[0].op_type == "Transpose":
                    ...
                    if p1 and p2 and len(p1) == len(p2):
                        ...
                        # if fused_perm is identity, we can remove both if safe
                        is_identity = all(
                            fused_perm[i] == i for i in range(len(fused_perm))
                        )
                        if is_identity:
                            # Identity cancellation, map input of first to output of second
                            # In real graph we would rename edges or replace in consumers.
                            # For simplicity we inject an Identity node to maintain connections.
                            ident_node = Node(
                                "Identity",
                                [node.inputs[0]],
                                [next_node.outputs[0]],
    ...
    def push_transposes_down(graph: Graph) -> Graph:
        ...
        for node in graph.nodes:
            ...
            if node.op_type in elementwise_ops:
                pushed_inputs = [
                    inp for inp in node.inputs if inp in transposes_to_push
                ]
                if pushed_inputs:
                    perm = transposes_to_push[pushed_inputs[0]].attributes.get(
                        "perm", []
                    )
                    new_inputs = []
                    for inp in node.inputs:
    ...
    def optimal_ios_coreml_layout() -> str:
        ...
        return "NCHW"  # Typically NCHW or specific channel-padded shapes
    ...
    def pad_to_alignment(shape: Tuple[Any, ...], alignment: int = 4) -> Tuple[Any, ...]:
        ...
        if len(shape) < 1:
            return shape
        # Assume channel is the last dimension for packing or index 1 depending on layout, here we just pad the last dimension
        last_dim = shape[-1]
    ...
    def align_tensor_shapes_pass(graph: Graph, alignment: int = 4) -> Graph:
        ...
        for name, t in graph.tensors.items():
            ...
            new_t = Tensor(
                t.name, new_shape, t.dtype, t.is_initializer, t.requires_grad, t.data
            )
    ...
    def update_parameters_for_alignment(node: Node, pad_amount: int) -> Node:
        ...
        if node.op_type == "Conv":
            ...
            if "shape" in attrs:
                ...
                attrs["shape"] = shape
        return Node(
            node.op_type, node.inputs.copy(), node.outputs.copy(), attrs, node.name
        )
    ...
    def unfold_constants(graph: Graph) -> Graph:
        ...
        for name, t in graph.tensors.items():
            if t.is_initializer and t.data is not None:
                ...
                new_t = Tensor(
                    t.name, (len(flat_data),), t.dtype, True, t.requires_grad, flat_data
                )
                new_graph.add_tensor(new_t)
            else:
                new_graph.add_tensor(t)
