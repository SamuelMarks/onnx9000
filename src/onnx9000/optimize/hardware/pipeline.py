...
class PipelineOptimizer:
    ...
    def identify_independent_paths(graph: Graph) -> List[List[Node]]:
        ...
        # Simple topological level assignment to find independent nodes
        levels: Dict[str, int] = {}
    ...
    def partition_for_webworkers(graph: Graph, num_workers: int = 4) -> List[Graph]:
        ...
        # very simple partitioning by round robin
    ...
    def inject_alloc_free(graph: Graph) -> Graph:
        ...
        for node in graph.nodes:
            ...
            # inject Free for consumed inputs
            for inp in node.inputs:
                free_node = Node(
                    "Free",
                    [f"{inp}_ptr"],
    ...
    def device_placement_pass(graph: Graph) -> Graph:
        ...
        for node in graph.nodes:
            ...
            attrs["device"] = PipelineOptimizer.cpu_vs_gpu_heuristic(
                payload, node.op_type
            )
            new_graph.add_node(
                Node(
                    node.op_type,
    ...
    @staticmethod
    def precalculate_static_shapes(
        graph: Graph, input_bounds: Dict[str, Tuple[int, ...]]
    ) -> Graph:
    ...
    @staticmethod
    def genetic_algorithm_auto_tuner(
        graph: Graph, generations: int = 5
    ) -> Dict[str, Any]:
        ...
        population = [
            [random.randint(0, 1) for _ in range(num_layers)]
