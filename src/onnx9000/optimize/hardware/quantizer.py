...
class Quantizer:
    ...
    def calculate_scale_zero_point(
        ...
        if symmetric:
            ...
            # handle div by zero later
            zp = np.round(qmin - min_arr / np.where(scale == 0, 1e-8, scale)).astype(
                np.int32
            )
    ...
    def dequantize_linear(
        ...
        return (
            quantized.astype(np.float32) - zp_br.astype(np.float32)
        ) * scale_br.astype(np.float32)
    ...
    def qlinear_conv(
        ...
        conv_out = Quantizer.conv_integer(
            x, w, x_zero_point, w_zero_point, pads, strides
        )
    ...
    def activation_clipping(
        tensor: np.ndarray, min_val: float, max_val: float
    ) -> np.ndarray:
    ...
    def quantize_bf16(tensor: np.ndarray) -> np.ndarray:
        ...
        bf16_bytes = fp32_bytes & 0xFFFF0000
    ...
    def convert_initializer_to_int8(tensor_proto: Any) -> Any:
        ...
        if not hasattr(tensor_proto, "data_type") or not hasattr(
    ...
    def pack_int4(tensor: np.ndarray, little_endian: bool = True) -> np.ndarray:
        ...
        flat = flat.astype(np.uint8) & 0x0F
    ...
    @staticmethod
    def unpack_int4(
        packed: np.ndarray, length: int, little_endian: bool = True
    ) -> np.ndarray:
        """Unpack INT4 weights."""
    ...
    @staticmethod
    def analyze_sparsity(tensor: np.ndarray, threshold: float = 1e-5) -> float:
