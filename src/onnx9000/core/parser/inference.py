...
def infer_reshape(node: Node, graph: "Graph") -> None:
    ...
    if shape_tensor and shape_tensor.data is not None:
        ...
        target_shape = node.attributes["shape"]
...
def infer_transpose(node: Node, graph: Graph) -> None:
    ...
    if perm:
        out_shape = tuple(in_tensor.shape[i] for i in perm)
    else:
...
def infer_cast(node: Node, graph: Graph) -> None:
    ...
    if node.op_type == "Cast":
        ...
        if not target_tensor:
            raise CompilationError("Target tensor not found.")
    ...
    for i, out_name in enumerate(node.outputs):
        ...
        if i == 0:
            ...
            if len(in_tensor.shape) > 1:
                ...
                stat_shape = (DynamicDim(-1),)
            if out_tensor:
                out_tensor.shape = stat_shape
                out_tensor.dtype = in_tensor.dtype
            else:
                out_tensor = Tensor(
...
def infer_sequence_ops_mock(node: Node, graph: Graph) -> None:
    ...
    if not node.outputs:
        raise CompilationError(f"{node.op_type} expects 1 output.")
...
def infer_constant(node: Node, graph: Graph) -> None:
    ...
    if not node.outputs:
        raise CompilationError("Constant expects exactly 1 output.")
    ...
    if (
        ...
        for i, axis in enumerate(axes):
            ...
            if step > 0:
                ...
                out_dim = max(0, (start - end - step - 1) // (-step))
...
def infer_scatter_elements(node: Node, graph: "Graph") -> None:
    ...
    for out in node.outputs:
        if out not in graph.tensors:
            ...
            graph.tensors[out].shape = in_tensor.shape
            graph.tensors[out].dtype = in_tensor.dtype
...
def infer_spacetodepth(node: Node, graph: "Graph") -> None:
    ...
    print("INFER DEPTH TO SPACE INPUT SHAPE:", in_tensor.shape)
    if len(out_shape) == 4:
        c = out_shape[1].value if hasattr(out_shape[1], "value") else out_shape[1]
        ...
        if c != -1:
            out_shape[1] = c * blocksize * blocksize
        if h != -1:
            out_shape[2] = h // blocksize
        if w != -1:
            out_shape[3] = w // blocksize
...
def infer_compress(node: Node, graph: "Graph") -> None:
    ...
    in_tensor = graph.tensors.get(node.inputs[0])
    cond_tensor = graph.tensors.get(node.inputs[1])
...
def infer_random_normal(node: Node, graph: "Graph") -> None:
    ...
    for out in node.outputs:
        if out not in graph.tensors:
            ...
            graph.tensors[out].shape = tuple(out_shape)
            graph.tensors[out].dtype = DType.FLOAT32
...
def infer_deform_conv(node: Node, graph: Graph) -> None:
    ...
    if not x or not w:
        raise CompilationError("DeformConv inputs not found.")
...
def infer_layer_normalization(node: Node, graph: Graph) -> None:
    ...
    for i in range(1, 3):
        if len(node.outputs) > i and node.outputs[i]:
            ...
            if out_tensor_aux:
                out_tensor_aux.shape = (1,)
                out_tensor_aux.dtype = out_dtype
            else:
...
def infer_topk(node: Node, graph: Graph) -> None:
    ...
    if out_vals_tensor:
        out_vals_tensor.shape = out_shape
        out_vals_tensor.dtype = out_dtype_vals
    else:
        graph.add_tensor(
    ...
    if out_idx_tensor:
        out_idx_tensor.shape = out_shape
        out_idx_tensor.dtype = DType.INT64
    else:
...
def infer_unique(node: Node, graph: Graph) -> None:
    ...
    for i in range(1, 4):
        if len(node.outputs) > i and node.outputs[i]:
            ...
            if aux_tensor:
                aux_tensor.shape = (DynamicDim(-1),)
                aux_tensor.dtype = DType.INT64
            else:
                graph.add_tensor(
