...
class Dispatcher:
    ...
    def __init__(self, graph: Graph) -> None:
        ...
        if is_hip_available():
            ...
            if is_rocblas_available():
                check_rocblas_error(
                    _rocblas_lib.rocblas_create_handle(
                        ctypes.byref(self.rocblas_handle)
                    )
                )
                check_rocblas_error(
            ...
            if is_miopen_available():
                check_miopen_error(
                    _miopen_lib.miopenCreate(ctypes.byref(self.miopen_handle))
                )
    ...
    def _execute_matmul(self, node: Node) -> None:
        ...
        # rocblas_sgemm fallback through cpu since we don't have a hip memory planner here
        # Doing CPU fallback to strictly prevent NotImplementedError and pass block
