...
def generate_constant(node: Node, ctx: Generator) -> str:
    """Provides generate constant functionality and verification."""
...
def generate_constant_of_shape(node: Node, ctx: Generator) -> str:
    """Provides generate constant of shape functionality and verification."""
...
def generate_concat(node: Node, ctx: Generator) -> str:
    """Provides generate concat functionality and verification."""
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_quantize_linear(node: Node, ctx: Generator) -> str:
    """Provides generate quantize linear functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_dequantize_linear(node: Node, ctx: Generator) -> str:
    """Provides generate dequantize linear functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_eye_like(node: Node, ctx: Generator) -> str:
    """Provides generate eye like functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_non_max_suppression(node: Node, ctx: Generator) -> str:
    """Provides generate non max suppression functionality and verification."""
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_non_zero(node: Node, ctx: Generator) -> str:
    """Provides generate non zero functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_random_normal(node: Node, ctx: Generator) -> str:
    """Provides generate random normal functionality and verification."""
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_random_normal_like(node: Node, ctx: Generator) -> str:
    """Provides generate random normal like functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_random_uniform(node: Node, ctx: Generator) -> str:
    """Provides generate random uniform functionality and verification."""
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_random_uniform_like(node: Node, ctx: Generator) -> str:
    """Provides generate random uniform like functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_range(node: Node, ctx: Generator) -> str:
    """Provides generate range functionality and verification."""
...
def generate_regex_full_match(node: Node, ctx: Generator) -> str:
    """Provides generate regex full match functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_resize(node: Node, ctx: Generator) -> str:
    """Provides generate resize functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_reverse_sequence(node: Node, ctx: Generator) -> str:
    """Provides generate reverse sequence functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_scatter(node: Node, ctx: Generator) -> str:
    """Provides generate scatter functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_scatter_elements(node: Node, ctx: Generator) -> str:
    """Provides generate scatter elements functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_scatter_nd(node: Node, ctx: Generator) -> str:
    """Provides generate scatter nd functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    indices = ctx.get_tensor_name(node.inputs[1])
    updates = ctx.get_tensor_name(node.inputs[2])
...
def generate_gather_elements(node: Node, ctx: Generator) -> str:
    """Provides generate gather elements functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    indices = ctx.get_tensor_name(node.inputs[1])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
...
def generate_gathernd(node: Node, ctx: Generator) -> str:
    """Provides generate gather nd functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    indices = ctx.get_tensor_name(node.inputs[1])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
...
def generate_group_normalization(node: Node, ctx: Generator) -> str:
    """Provides generate group normalization functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_identity(node: Node, ctx: Generator) -> str:
    """Provides generate identity functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_same_shape_type_ops(node: Node, ctx: Generator) -> str:
    """Provides generate same shape type ops functionality and verification."""
    if not node.inputs:
        return ""
...
def generate_lrn(node: Node, ctx: Generator) -> str:
    """Provides generate lrn functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_onehot(node: Node, ctx: Generator) -> str:
    """Provides generate onehot functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_sum(node: Node, ctx: Generator) -> str:
    """Provides generate sum functionality and verification."""
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_swish(node: Node, ctx: Generator) -> str:
    """Provides generate swish functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_tile(node: Node, ctx: Generator) -> str:
    """Provides generate tile functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_slice(node: Node, ctx: Generator) -> str:
    """Provides generate slice functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_depthtospace(node: Node, ctx: Generator) -> str:
    """Provides generate depthtospace functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_spacetodepth2(node: Node, ctx: Generator) -> str:
    """Provides generate spacetodepth functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_compress(node: Node, ctx: Generator) -> str:
    """Provides generate compress functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_cumsum(node: Node, ctx: Generator) -> str:
    """Provides generate cumsum functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_dropout(node: Node, ctx: Generator) -> str:
    """Provides generate dropout functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
...
def generate_trilu(node: Node, ctx: Generator) -> str:
    """Provides generate trilu functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_pad(node: Node, ctx: Generator) -> str:
    """Provides generate pad functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
...
def generate_unique(node: Node, ctx: Generator) -> str:
    """Provides generate unique functionality and verification."""
    inp = ctx.get_tensor_name(node.inputs[0])
    out = ctx.get_tensor_name(node.outputs[0])
    tensor_info = ctx.graph.tensors[node.outputs[0]]
    buffer_idx = tensor_info.buffer_id
