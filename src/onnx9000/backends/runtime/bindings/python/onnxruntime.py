...
from onnx9000.frontends.jit import compile as compile_model
...
class RunOptions:
    ...
    def __init__(self) -> None:
        """Execute the   init   process and return the computed results."""
        self.run_log_severity_level: int = 2
        self.run_log_verbosity_level: int = 0
        self.run_tag: str = ""
        self.terminate: bool = False
        self.only_execute_path_to_fetches: bool = False
...
class OrtValue:
    ...
    def ortvalue_from_shape_and_type(
        ...
        dt = dtype_map.get(element_type, np.float32)
        return OrtValue(np.zeros(shape, dtype=dt))
...
class IOBinding:
    ...
    def __init__(self, session: "InferenceSession") -> None:
        ...
        self.inputs: Dict[str, OrtValue] = {}
        self.outputs: Dict[str, OrtValue] = {}
    ...
    def bind_input(
        ...
        self.inputs[name] = OrtValue.ortvalue_from_shape_and_type(shape, element_type)
    ...
    def bind_output(
        ...
        self.outputs[name] = OrtValue.ortvalue_from_shape_and_type(shape, element_type)
...
class InferenceSession:
    ...
    def __init__(
        ...
        if isinstance(path_or_bytes, bytes):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".onnx") as f:
                ...
                temp_path = f.name
            try:
                self._compiled_model = compile_model(temp_path)
            finally:
                os.remove(temp_path)
    ...
    def run(
        ...
        if run_options and run_options.terminate:
            raise RuntimeError("Execution cancelled by RunOptions.terminate")
        ...
        try:
            ...
            raise RuntimeError(f"onnx9000 execution failed: {e}")
    ...
    def run_with_iobinding(
        ...
        inputs = {k: v.numpy() for k, v in iobinding.inputs.items()}
        res = self.run(self._output_names, inputs, run_options)
        for name, val in zip(self._output_names, res):
            if name in iobinding.outputs:
                iobinding.outputs[name]._data = val
