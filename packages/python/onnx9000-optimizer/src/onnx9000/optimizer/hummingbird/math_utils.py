...
def map_count_vectorizer(g: Graph, input_name: str, vocab: List[str]):
    """Map CountVectorizer to native Equal, Cast, and ReduceSum ops."""
    pass


def map_tfidf_vectorizer(g: Graph, input_name: str, vocab: List[str], idf: List[float]):
    """Map TfidfVectorizer to native ONNX math ops using sparse dictionaries."""
    pass


def map_polynomial_expansion(g: Graph, input_name: str, degree: int):
    """Map polynomial expansions using Pow and Mul combinations."""
    pass


def map_murmurhash3(g: Graph, input_name: str):
    """Implement MurmurHash3 purely in ONNX ops for FeatureHasher support."""
    pass


def optimize_sigmoid(g: Graph, input_name: str, output_name: str, use_fast_math: bool = False):
...
def fold_scaler_into_linear(weights, bias, mean, scale):
...
def map_knn_distances(g: Graph):
    """Map KNN distances to ReduceSumSquare, TopK, and Gather natively."""
    pass


def replace_mod(g: Graph, input_A: str, input_B: str, output_name: str):
...
def replace_where_with_arithmetic_mask(g: Graph, mask: str, A: str, B: str, output_name: str):
...
def clamp_nan_to_zero(g: Graph, input_name: str, output_name: str):
...
def division_by_zero_guard(g: Graph, denom: str, epsilon: str, output_name: str):
    """Implement robust division by zero guards Add(x, epsilon)."""
    g.nodes.append(Node("Add", inputs=[denom, epsilon], outputs=[output_name]))


def ensure_softmax_stability(g: Graph, input_name: str, output_name: str):
