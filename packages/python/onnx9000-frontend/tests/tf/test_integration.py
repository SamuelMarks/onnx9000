import time
from onnx9000.frontend.tf.api import convert_keras_to_onnx
...
def test_integration_tf_mnist() -> None:
    from onnx9000.frontend.tf.api import _convert_tfgraph
    from onnx9000.frontend.tf.parsers import TFGraph, TFNode
...
def test_integration_resnet_mobilenet_mock() -> None:
    ...
    for i in range(50):
        nodes.append(TFNode(f"conv{i}", "Conv2D", inputs=[f"conv{i - 1}" if i > 0 else "in"]))
