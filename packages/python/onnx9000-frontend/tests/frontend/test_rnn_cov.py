...
def test_rnn_coverage_extra_2() -> None:
    ...
    gru = GRU(10, 20)
    with patch("onnx9000.frontend.frontend.utils.record_op", return_value="res"):
        (y1, h1) = rnn(None)
        (y2, h2) = lstm(None)
        (y3, h3) = gru(None)
        assert y1 == "res"
...
def test_rnn_list_return() -> None:
    ...
    with patch("onnx9000.frontend.frontend.utils.record_op", return_value=["a", "b", "c"]):
...
def test_lstm_list_len_2() -> None:
    ...
    from unittest.mock import patch
    from onnx9000.frontend.frontend.nn.rnn import LSTM

    lstm = LSTM(10, 20, bias=False)
    with patch("onnx9000.frontend.frontend.utils.record_op", return_value=["a", "b"]):
        (y2, h2) = lstm(None)
        assert h2 == ("b", "b")
