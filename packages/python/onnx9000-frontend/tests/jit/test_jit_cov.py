...
def test_jit_wasm_target(tmp_path) -> None:
    ...
    g = ir.Graph("dummy")
    with patch("onnx9000.frontend.jit.load", return_value=g):
        with patch("onnx9000.frontend.jit.plan_memory"):
            with patch("onnx9000.frontend.jit.compile_wasm", return_value="wasm_ok"):
...
def test_jit_initializer_no_data() -> None:
    ...
    with patch("onnx9000.frontend.jit.load", return_value=g):
        with patch("onnx9000.frontend.jit.plan_memory"):
            with patch("onnx9000.frontend.jit.compile_cpp", return_value="lib"):
                with patch("onnx9000.frontend.jit.hash_graph", return_value="hash"):
                    with patch("onnx9000.frontend.jit.load_module", return_value=DummyMod()):
