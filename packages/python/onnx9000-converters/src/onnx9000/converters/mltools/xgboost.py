"""XGBoost JSON parser for pure-Python ONNX conversion."""
...
def parse_xgboost_json(json_str: str) -> Graph:
    """Parse an XGBoost JSON dump and return an ONNX Graph."""
    data = json.loads(json_str)
    return parse_xgboost_dict(data)


def parse_xgboost_dict(data: dict[str, Any]) -> Graph:
    """Parse an XGBoost dict representation."""
    graph = Graph("XGBoost_Model")
    graph.opset_imports = {"": 14, "ai.onnx.ml": 3}

    learner = data.get("learner", {})
    objective = learner.get("objective", {}).get("name", "")

    num_features = int(learner.get("learner_model_param", {}).get("num_feature", 0))
    ...
    for tree_id, tree in enumerate(trees):
        ...
        for node_id in range(num_nodes):
            ...
            if left_id == -1:  # Leaf node
                ...
                target_ids.append(0)
                target_weights.append(split_conditions[node_id])
            else:
                nodes_featureids.append(split_indices[node_id])
                nodes_values.append(split_conditions[node_id])
                nodes_hitrates.append(1.0)
                nodes_modes.append(b"BRANCH_LT")
                nodes_truenodeids.append(left_id)
                nodes_falsenodeids.append(right_id)

                missing = 1 if default_left[node_id] == 1 else 0
    ...
    if objective.startswith("binary:"):
        ...
        if objective == "binary:logistic":
            attrs["post_transform"] = Attribute(b"LOGISTIC", "STRING")
        else:
