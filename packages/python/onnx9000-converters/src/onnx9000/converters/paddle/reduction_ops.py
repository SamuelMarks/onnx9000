...
def _map_allclose(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    rtol = builder.extract_attr(node, "rtol", 1e-05)
    atol = builder.extract_attr(node, "atol", 1e-08)
    a = inputs[0]
    b = other[0]
    ...
    less_eq = builder.make_node("LessOrEqual", [abs_diff, tol], {}, f"{node.name}_le")[0]
...
REDUCTION_OPS_MAPPING: dict[str, Callable[[PaddleToONNXGraphBuilder, PaddleNode], list[str]]] = {
    ...
    "cumsum": _map_cumsum,
    "cumprod": _map_custom("Custom_Paddle_cumprod"),
}
