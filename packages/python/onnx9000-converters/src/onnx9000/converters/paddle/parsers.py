...
class PaddleProtobufParser:
    ...
    def read_string(self) -> str:
        """read_string implementation."""
        return self.read_bytes().decode("utf-8")

    def parse_op_desc(self, limit: int) -> PaddleNode:
        """parse_op_desc implementation."""
        op_type = ""
            ...
            if field_num == 3:
                ...
                while self.offset < sub_limit:
                    ...
                    if s_field == 1:
                        ...
                        a_val = self.read_varint()
                    elif s_field == 4:
                        a_val = struct.unpack("<f", self.data[self.offset : self.offset + 4])[0]
                        self.offset += 4
                    elif s_field == 5:
                        a_val = self.read_string()
                    elif s_field == 10:
                        a_val = bool(self.read_varint())
                    elif s_field in [6, 7, 8, 9, 11]:
                        self.skip_field(s_wire)
                    else:
                        self.skip_field(s_wire)
                attrs[a_name] = a_val
    ...
    def parse_block_desc(self, limit: int) -> PaddleBlock:
        """parse_block_desc implementation."""
        idx = 0
        parent_idx = -1
        vars_dict = {}
        ops = []
    ...
    def parse_program_desc(self, limit: int) -> PaddleGraph:
        """parse_program_desc implementation."""
        graph = PaddleGraph()
        while self.offset < limit:
            tag = self.read_varint()
            field_num = tag >> 3
    ...
    def parse_framework(self) -> PaddleGraph:
        """parse_framework implementation."""
        while self.offset < len(self.data):
            tag = self.read_varint()
            tag >> 3
            tag & 7
