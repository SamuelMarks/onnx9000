...
def _map_generate_proposals(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    """Generate Proposals mapping."""
    scores = node.inputs.get("Scores", [])
    bboxes = node.inputs.get("BboxDeltas", [])
    node.inputs.get("ImInfo", [])
    node.inputs.get("Anchors", [])
    scores_flat = builder.make_node("Flatten", scores, {"axis": 1}, f"{node.name}_flatten_scores")[
        0
    ]
    ...
    rpn_rois = builder.make_node("Gather", [bboxes, nms_out], {"axis": 1}, f"{node.name}_rpn_rois")[
        0
    ]
...
def _map_multiclass_nms(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    max_output_boxes_per_class = builder.add_constant(f"{node.name}_max_out", [nms_top_k], 7, [1])
    iou_threshold = builder.add_constant(f"{node.name}_iou", [nms_threshold], 1, [1])
    score_threshold_t = builder.add_constant(f"{node.name}_score", [score_threshold], 1, [1])
    nms_out = builder.make_node(
        "NonMaxSuppression",
...
def _map_box_coder(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    add_out = builder.make_node("Add", [prior_box, target_box], {}, f"{node.name}_add")[0]
    return [add_out]
...
def _map_grid_sampler(builder: PaddleToONNXGraphBuilder, node: PaddleNode) -> list[str]:
    ...
    mode = "bilinear" if builder.extract_attr(node, "mode", "bilinear") == "bilinear" else "nearest"
...
VISION_OPS_MAPPING: dict[str, Callable[[PaddleToONNXGraphBuilder, PaddleNode], list[str]]] = {
