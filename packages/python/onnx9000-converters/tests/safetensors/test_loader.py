...
def test_validate_onnx_shapes_and_dtypes(caplog):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        save_file(
            {
                "w1": np.array([1.0, 2.0], dtype=np.float32),  # Shape mismatch! (2,) vs (2, 2)
                "w2": np.array([3.0, 4.0], dtype=np.float32),  # Dtype mismatch! F32 vs I32
            },
            path,
        )
...
def test_split_qkv():
    ...
    # Let's populate specific values to ensure slices are correct
    tensors["c_attn.bias"][:64] = 1.0  # Q
    tensors["c_attn.bias"][64:128] = 2.0  # K
    tensors["c_attn.bias"][128:] = 3.0  # V
...
def test_converters_loader_remaining():
    ...
    # 29-33: map_huggingface_to_onnx split QKV branches
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # load_safetensors_to_graph output
        g3 = Graph("g3")
        ...
        load_safetensors_to_graph(path)  # implicit empty graph
...
def test_converters_loader_edge_cases():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # 29-33: Constant node replacement link
        g = Graph("g")
        g.add_tensor(Tensor("out", shape=(1,), dtype=DType.FLOAT32))
...
def test_constant_value_attribute_name():
    # 33: Constant node replacement link
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        n = Node("Constant", [], ["out_c"])
        # In ONNX, the value attribute is a TensorProto. The 'name' of the Attribute object itself is 'value'.
        # The code checks `if attr_val.name == name:` which implies `attr_val.name` is the name of the tensor.
        # But `attr_val` is an `Attribute` object, its `name` is the name of the attribute (e.g. "value").
        # If the safetensor key is literally "value", it will match!
