...
def generate_shape_op(
    ...
    # If the arena planner correctly alias this, this node doesn't actually need to copy memory
    # However, if it's not strictly aliased, we must copy it.
    if out_name != in_name:
...
def generate_transpose(
    ...
    if ndim == 2:
        ...
        # Mapping out dims directly into the string
        out_dims = [0, 0, 0]
        out_dims[P0] = 0
        out_dims[P1] = 1
        out_dims[P2] = 2

        # Find which loop variable corresponds to which output dimension
...
def generate_concat(
    ...
    for in_idx, in_tensor in enumerate(in_tensors):
        ...
        # Calculate full index strings for readability and correctness
...
def generate_pad(
    ...
    if (
        ...
        # strict C89 fallback without explicit dynamic pads parsing currently
...
def generate_slice(b, node, out_tensor, in_tensors, in_names, out_name):
    ...
    # 124: Handle static starts, ends, axes, steps
    # We must generate a nested loop for each dimension
    idx_vars = []
    ...
    # Check if starts/ends are dynamic or static. Usually they are constant inputs.
    # If they are dynamic (inputs 1-4), we must compute them.
    # To satisfy 125, we emit C variables that read from the input arrays.
    ...
    # Map axes to actual dimension bounds
    ...
    b.emit(
        "real_starts[ax] = starts[i] < 0 ? starts[i] + " + str(ndims) + " : starts[i];"
    )  # Simplify clamp
...
def generate_gather(b, node, out_tensor, in_tensors, in_names, out_name):
    ...
    from onnx9000.core.profiler import resolve_volume
    # A gather loop has out_idx and maps to the indices tensor
    # 126: Gather C code
    # 127: OOB indexing clamp safely

    # Pre-calculate strides for the input tensor
    strides = []
    stride = 1
    ...
    # The output is shape: in_shape[:axis] + indices_shape + in_shape[axis+1:]
    # For every element in output, we must find the index in input

    b.emit(f"for (int i = 0; i < {resolve_volume(out_shape)}; ++i) {{")
    b.push_indent()

    # Deconstruct i into the output dimensions, then reconstruct input index
    b.emit("int in_idx = 0;")
    b.emit("int temp = i;")

    # 1D or ND logic is heavily math dependent. Let's do a generic fast div/mod mapping
    out_strides = []
    ostride = 1
    ...
    # Variables for each output dimension
    idx_vars = []
    ...
    # Now reconstruct the input index!
    # Output indices map to:
    # out_dims[:axis] -> in_dims[:axis]
    # out_dims[axis:axis+len(indices_shape)] -> indices -> in_dims[axis]
    # out_dims[axis+len(indices_shape):] -> in_dims[axis+1:]
    ...
    # The indices lookup
    # Find flat index into the `indices` tensor
    indices_strides = []
    istride = 1
...
def generate_gathernd(b, node, out_tensor, in_tensors, in_names, out_name):
    ...
    # 240: Map GatherND multi-dim loops

    # Calculate the flat index into the `indices` tensor
    # The output dimensions are: out_shape = in_shape[:batch_dims] + indices_shape[batch_dims:-1] + in_shape[batch_dims + indices_shape[-1]:]
    ...
    # Loop over the `indices_shape[-1]` elements which contain the actual gather dimensions
    k = indices_shape[-1]
    ...
    # Remaining dimensions map directly
...
def generate_scatternd(b, node, out_tensor, in_tensors, in_names, out_name):
    ...
    # 130 / 241: ScatterND Memory Updates
    indices_shape[-1]

    # The number of updates is the volume of the updates tensor.
    # However, one "update" might be a slice or a scalar.
    # A single index tuple indices[i_0...i_M] corresponds to updates[i_0...i_M]
    # The shape of the slice is in_shape[k:]
    # And updates_shape is indices_shape[:-1] + in_shape[k:]
...
def generate_cumsum(b, node, out_tensor, in_tensors, in_names, out_name):
    ...
    from onnx9000.core.profiler import resolve_volume
    # The axis is given by the second input (tensor of 1 element)
    # However we'll assume it's statically parsed by the frontend, but if it's dynamic we have to read it.
    ...
    # We calculate the block sizes
    b.emit("int outer = 1;")
    b.emit("int inner = 1;")
    b.emit("int dim = " + str(in_shape[0]) + ";")  # Fallback dummy
