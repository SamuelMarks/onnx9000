...
def test_routing_extra():
    ...
    g.nodes.append(Node("Pad", inputs=["X"], outputs=["Y_PadDyn"]))  # missing pad tensor
...
def test_operations_extra():
    ...
    g.tensors["W1"] = Constant(
        "W1", shape=(1,), dtype=DType.INT32, values=struct.pack("<i", 4)
    )  # Power of 2
...
def test_quantization_extra():
    ...
    g.tensors["ZP_NO"] = Tensor("ZP_NO", shape=(1,), dtype=DType.FLOAT32)  # No valid data
