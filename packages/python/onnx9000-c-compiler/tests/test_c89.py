...
def test_phase_3():
    ...
    # Add dummy tensors for broadcasting
...
def test_phase_4():
    ...
    g.tensors.update(
        {
            ...
            "C": Tensor("C", shape=(2, 4), dtype=DType.FLOAT32),  # MatMul
    ...
    n3 = Node("MatMul", inputs=["G", "H"], outputs=["I"])  # GEMV detection
    n4 = Node("MatMul", inputs=["J", "K"], outputs=["L"])  # Unrolled detection
...
def test_phase_6():
    ...
    g.tensors.update(
        {
            "X1": Tensor("X1", shape=(1, 3, 224, 224), dtype=DType.FLOAT32),
            "Y1": Tensor("Y1", shape=(1, 3, 112, 112), dtype=DType.FLOAT32),  # MaxPool
            "X2": Tensor("X2", shape=(1, 64, 112, 112), dtype=DType.FLOAT32),
            "Y2": Tensor("Y2", shape=(1, 64, 1, 1), dtype=DType.FLOAT32),  # GlobalAveragePool
            "X3": Tensor("X3", shape=(1, 128), dtype=DType.FLOAT32),
            "Y3": Tensor("Y3", shape=(1, 1), dtype=DType.FLOAT32),  # ReduceSum
            "X4": Tensor("X4", shape=(1, 10), dtype=DType.FLOAT32),
            "Y4": Tensor("Y4", shape=(1,), dtype=DType.INT64),  # ArgMax
        }
    )
    ...
    g.tensors.update(
        {
            ...
            "Y1": Tensor("Y1", shape=(1, 10), dtype=DType.FLOAT32),  # Relu
            "X2": Tensor("X2", shape=(1, 10), dtype=DType.FLOAT32),
            "Y2": Tensor("Y2", shape=(1, 10), dtype=DType.FLOAT32),  # Softmax
            ...
            "Y3": Tensor("Y3", shape=(1, 3, 2, 2), dtype=DType.FLOAT32),  # BatchNorm
            "X4": Tensor("X4", shape=(1, 3, 2, 2), dtype=DType.FLOAT32),
            "Y4": Tensor("Y4", shape=(1, 3, 2, 2), dtype=DType.FLOAT32),  # LayerNorm
            "X5": Tensor("X5", shape=(1, 10), dtype=DType.FLOAT32),
            "Y5": Tensor("Y5", shape=(1, 10), dtype=DType.FLOAT32),  # Gelu
        }
    )
    ...
    g.tensors.update(
        {
            ...
            "Y1": Tensor("Y1", shape=(1, 12), dtype=DType.FLOAT32),  # Reshape
            "X2": Tensor("X2", shape=(1, 3, 2, 2), dtype=DType.FLOAT32),
            "Y2": Tensor("Y2", shape=(1, 2, 3, 2), dtype=DType.FLOAT32),  # Transpose
            "X3": Tensor("X3", shape=(1, 3, 2, 2), dtype=DType.FLOAT32),
            "X4": Tensor("X4", shape=(1, 4, 2, 2), dtype=DType.FLOAT32),
            "Y3": Tensor("Y3", shape=(1, 7, 2, 2), dtype=DType.FLOAT32),  # Concat
            ...
            "Y5": Tensor("Y5", shape=(1, 4), dtype=DType.FLOAT32),  # Pad
        }
    )
    ...
    g.tensors.update(
        {
            ...
            "Y1": Tensor("Y1", shape=(1, 5), dtype=DType.BOOL),  # Equal
            "X3": Tensor("X3", shape=(1, 5), dtype=DType.BOOL),
            "X4": Tensor("X4", shape=(1, 5), dtype=DType.BOOL),
            "Y2": Tensor("Y2", shape=(1, 5), dtype=DType.BOOL),  # And
            ...
            "Y3": Tensor("Y3", shape=(1, 5), dtype=DType.FLOAT32),  # Where
        }
    )
    ...
    g.tensors.update(
        {
            ...
            "Y1": Tensor("Y1", shape=(1, 5), dtype=DType.UINT8),  # QuantizeLinear
            ...
            "Y2": Tensor("Y2", shape=(1, 5), dtype=DType.FLOAT32),  # DequantizeLinear
        }
    )
    ...
    g.tensors.update(
        {
            ...
            "Y1": Tensor("Y1", shape=(1, 2), dtype=DType.UINT8),  # QLinearMatMul
            "X2": Tensor("X2", shape=(1, 1, 4, 4), dtype=DType.UINT8),
            "W2": Tensor("W2", shape=(2, 1, 2, 2), dtype=DType.UINT8),
            "Y2": Tensor("Y2", shape=(1, 2, 3, 3), dtype=DType.UINT8),  # QLinearConv
        }
    )
...
def test_einsum_coverage():
    ...
    # 3: valid with string instead of bytes
        ...
        with patch("onnx9000.c_compiler.cli.os.path.exists", return_value=True):
            ...
            with patch("onnx9000.c_compiler.cli.open", m_open):
                with patch("onnx9000.c_compiler.cli.os.makedirs"):
                    # Custom graph with NO Float to trigger math.h stripping
                    g = Graph("t")
                    g.tensors["A"] = Tensor(
