...
def test_ast_builder_edges():
    ...
    b = C89Builder()
    assert b._sanitize("1name") == "v1name"  # line 15
    b.push_indent()
    b.pop_indent()
    b.pop_indent()  # line 40 - popping below 0
...
def test_data_unpacker_edges():
    ...
    # non-matching exact size fallback
    res = unpack_bytes_to_str(b"\x01\x02\x03", DType.FLOAT32)  # size 3 not multiple of 4
...
def test_activations_coverage():
    ...
    compiler = C89Compiler(g, use_math_h=False)  # trigger use_math_h = False branches
    h, c = compiler.generate()
...
def test_pooling_and_reduction_coverage():
    ...
    for rop in ["ReduceMean", "ReduceSum", "ReduceMax", "ReduceMin", "ReduceProd"]:
        ...
        # Force "flat" reduction by making axes cover everything, or "complex" by making it partial
        # We'll use partial to hit the complex fallback
...
def test_routing_coverage():
    ...
    assert "out_idx =" in c  # 3D transpose logic
...
def test_spatial_coverage():
    ...
    # ConvTranspose unsupported dims
