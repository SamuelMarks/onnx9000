...
def test_pooling_and_reduction_more():
    ...
    g.nodes.append(Node("MaxPool", inputs=["X_3D"], outputs=["Y_Max3D"]))  # unsupported dim
...
def test_qlinear_conv():
    ...
    # Check unsupported dim fallback
...
def test_missing_ops():
    ...
    g.nodes.append(Node("MatMul", inputs=["X3D_A", "X3D_B"], outputs=["Y_BM"]))  # BatchMatMul
...
def test_missing_quantization_and_spatial():
    ...
    # QuantizeLinear and DequantizeLinear without zero_point
...
def test_more_ops_and_broadcasting():
    ...
    g.tensors["X_B2"] = Tensor(
        "X_B2", shape=(3, 1), dtype=DType.FLOAT32
    ...
    # Check that broadcast dimensions logic hits `elif in_stride == 0`
    ...
    # Gemm with Alpha/Beta != 1 and transA/B
    ...
    # Matmul symbolic broadcasting checking dim type
...
def test_missing_math_and_targets():
    ...
    # test arduino & baremetal targets, emit_cpp, UINT8 constants
    g.tensors["UINT8_CONST"] = Constant("UINT8_CONST", shape=(2,), dtype=DType.UINT8, values=b"")
...
def test_missing_quant_and_pool():
    ...
    g.nodes.append(Node("AveragePool", inputs=["X2D"], outputs=["Y_Avg2D"]))  # hit 2D average pool
    ...
    g.tensors["X_Q"] = Tensor("X_Q", shape=(1, 5), dtype=DType.FLOAT32)
    g.tensors["Y_Q"] = Tensor("Y_Q", shape=(1, 5), dtype=DType.INT8)  # NOT UINT8
    g.nodes.append(Node("QuantizeLinear", inputs=["X_Q", "Scale"], outputs=["Y_Q"]))
...
def test_phase_13_nlp_vision():
    ...
    # test emit_bpe_tokenizer separately
    b = C89Builder()
...
def test_missing_nlp_and_rnn():
    ...
    g.nodes.append(
        Node("NonMaxSuppression", inputs=["X", "X", "X", "X"], outputs=["Y_NMS2"])
    )  # missing 5th
