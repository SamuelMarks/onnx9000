...
export class ModelExporter {
  ...
  async exportModel(): Promise<Uint8Array> {
    ...
    for (const init of this.mutator.graph.initializers) {
      ...
      if (t?.data) totalBytes += t.data.byteLength;
    }
    if (totalBytes > MEMORY_THRESHOLD_MB * 1024 * 1024) {
      ...
      if (!proceed)
        throw new Error(
          "Export aborted by user due to memory threshold warning.",
        );
    }
  ...
  saveSessionToLocalStorage() {
    ...
    try {
      const skeleton = {
        ...
        edges:
          this.mutator.graph.inputs.length + this.mutator.graph.outputs.length,
      };
  ...
  promptChangesBeforeExport(): boolean {
    ...
    const edits =
      (this.mutator as ReturnType<typeof JSON.parse>).deletedNodeCount || 0; // assuming we track this
    return confirm(
