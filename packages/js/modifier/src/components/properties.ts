/**
 * @fileoverview properties.ts
 * Provides properties functionality for the modifier package.
 */
...
export class PropertiesPanel {
  ...
  renderNode(node: Node, _graph: Graph) {
    ...
    if (
      (node as ReturnType<typeof JSON.parse>)._profiling_time_ms !== undefined
    ) {
      ...
      const t = (
        node as ReturnType<typeof JSON.parse>
      )._profiling_time_ms.toFixed(3);
      const b = (
        (node as ReturnType<typeof JSON.parse>)._profiling_bandwidth_mb || 0
      ).toFixed(2);
    ...
    // Outputs
    const outputsGroup = this._createFormGroup(
    ...
    const isCustom = renderCustomEditor({
    ...
    if (vi) {
      ...
      vi.shape.forEach((dim, idx) => {
        ...
        dimInput.addEventListener("change", () => {
          ...
          newShape[idx] = Number.isNaN(Number(val)) ? val : Number(val);
  ...
  renderGraphProperties(graph: Graph) {
    ...
    createInput("Model Version", graph.modelVersion, (val) => {
      graph.modelVersion = Number.isNaN(Number(val)) ? 0 : Number(val);
    });
