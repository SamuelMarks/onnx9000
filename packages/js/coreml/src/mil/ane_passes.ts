/**
 * @fileoverview ane_passes.ts
 * Provides ane_passes functionality for the coreml package.
 */
...
export function optimizeForANE(block: Block): void {
  ...
  for (const op of block.operations) {
    if (op.opType === "cast") {
      ...
      if (
        out &&
        out.type instanceof TensorType &&
  ...
  for (let i = 0; i < newOps.length; i++) {
    ...
    if (op.opType === "cast") {
      const sourceVar = Array.isArray(op.inputs["x"])
        ? op.inputs["x"][0]
        : op.inputs["x"];
      if (
        sourceVar &&
  ...
  for (const op of block.operations) {
    if (op.opType === "matmul") {
      ...
      if (consecutiveMatMuls > 20) {
        ...
        console.warn(
          new ThermalThrottlingWarning(
            "Deep consecutive MatMul sequences without normalization",
          ).message,
        );
      }
      ...
      for (const k in op.inputs) {
        ...
        if (input && !Array.isArray(input)) {
          if (input.type instanceof TensorType) {
            ...
            if (shape.length > 4) {
              console.warn(
                `Warning: ANE generally does not support >4D tensors for ${op.opType}`,
              );
            }
            ...
            if (shape.some((d) => typeof d === "number" && d > 65536)) {
              console.warn(
                new ANELimitsExceededWarning(
                  `Dimension size exceeds 65536 in ${op.opType}`,
                ).message,
              );
            }
          }
