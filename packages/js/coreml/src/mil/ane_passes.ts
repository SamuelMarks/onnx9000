...
import { ANELimitsExceededWarning, ThermalThrottlingWarning } from './errors.js';
...
export function optimizeForANE(block: Block): void {
  ...
  for (let i = 0; i < newOps.length; i++) {
    ...
    if (op.opType === 'conv' || op.opType === 'conv_transpose') {
      const weight = Array.isArray(op.inputs.weight) ? op.inputs.weight[0] : op.inputs.weight;
