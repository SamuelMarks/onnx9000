...
export function detectAndMapGenAITopologies(graph: Graph, block: Block): void {
  ...
  const isLlama = graph.nodes.some(
    (n) => n.opType === "MatMul" && n.name.includes("q_proj"),
  );
  if (isLlama) {
    console.log(
    ...
    for (const op of block.operations) {
      if (
        op.opType === "concat" &&
        op.outputs[0]?.name.includes("present_key")
      ) {
  ...
  if (isWhisper) {
    console.log(
      "Detected Whisper topology. Optimizing 1D Convolutions for ANE.",
    );
    // Specifically ANE favors 2D convs with height=1. We would rewrite them here.
  }

  // 235. Map Stable Diffusion UNets
  const isSD = graph.nodes.some(
    (n) => n.opType === "Conv" && n.name.includes("down_blocks"),
  );
  if (isSD) {
    console.log(
      "Detected Stable Diffusion UNet topology. Optimizing spatial shapes.",
    );
  }
}
