...
describe("JSZip Sandboxing", () => {
  it("Validates clean structure without errors", () => {
    ...
    files.set(
      "Manifest.json",
      new TextEncoder().encode(JSON.stringify({ test: 123 })),
    );
  ...
  it("Detects and blocks script injections in Manifest.json", () => {
    ...
    files.set(
      ...
      new TextEncoder().encode(
        JSON.stringify({ author: "<script>alert(1)</script>" }),
      ),
    );
    expect(() => validateZipInputData(files)).toThrowError(
  ...
  it("Detects and blocks path traversal attempts", () => {
    ...
    expect(() => validateZipInputData(files2)).toThrowError(
      /Directory traversal detected/,
    );
  });
});
