...
describe("MIL Errors", () => {
  it("covers error instantiation", () => {
    ...
    expect(new UnsupportedOpError("FakeOp", "Because").name).toBe(
      "UnsupportedOpError",
    );
    expect(new UnsupportedOpError("FakeOp").message).toContain("FakeOp");
    expect(new ThermalThrottlingWarning("MatMul").name).toBe(
      "ThermalThrottlingWarning",
    );
    expect(new ANELimitsExceededWarning("Too big").name).toBe(
      "ANELimitsExceededWarning",
    );
...
describe("MIL Builder", () => {
  it("covers missing builder paths", () => {
    ...
    const _fn = builder.createFunction("test", [], []);
    ...
    // mul, sub, relu
    const v1 = builder.createVar(
      "v1",
      builder.tensor(MILDataType.FLOAT32, [1]),
    );
    const v2 = builder.createVar(
      "v2",
      builder.tensor(MILDataType.FLOAT32, [1]),
    );
    builder.sub(v1, v2);
...
describe("MIL Printer", () => {
  it("prints an entire program", () => {
    ...
    const _fn = b.createFunction("main", [inVar], [outVar]);
