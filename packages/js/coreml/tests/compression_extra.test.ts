...
describe("Compression Extras", () => {
  it("Handles mixed precision dictionary and gathers stats", () => {
    ...
    const x = builder.createVar(
      "w1",
      builder.tensor(MILDataType.FLOAT32, [100]),
    );
    const y = builder.createVar(
      "w2",
      builder.tensor(MILDataType.FLOAT32, [100]),
    );
    const z = builder.createVar(
      "w3",
      builder.tensor(MILDataType.FLOAT32, [100]),
    );
    ...
    expect(ops[0]!.attributes["ane_hint_dynamic_quant_stats_gathered"]).toBe(
      true,
    );
