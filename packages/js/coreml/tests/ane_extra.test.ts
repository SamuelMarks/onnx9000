...
describe("ANE Optimization Passes (Extended)", () => {
  ...
  it("annotates attention and layer_norm for ANE", () => {
    ...
    expect(block.operations[1]?.attributes.ane_hint_decompose_layer_norm).toBe(
      true,
    );
    expect(block.operations[2]?.attributes.ane_hint_decompose_einsum).toBe(
      true,
    );
  });
  ...
  it("optimizes gather with constants", () => {
    ...
    optimizeForANE(block);
    expect(block.operations[1]?.attributes.ane_hint_precompute_gather).toBe(
      true,
    );
  });
  ...
  it("detects thermal throttling on consecutive matmuls", () => {
    ...
    for (let i = 0; i < 22; i++) {
      ...
      b.addOp("matmul", { x, y: x }, [
        b.createVar(`y${i}`, b.tensor(MILDataType.FLOAT32, [1])),
      ]);
    }
    ...
    b.addOp(
      "add",
      { x: b.createVar("x", b.tensor(MILDataType.FLOAT32, [1])) },
      [],
    );
