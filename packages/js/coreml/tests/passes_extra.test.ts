...
import { Builder, fuseAdjacentOps, MILDataType } from "../src/index.js";
...
describe("Passes Extras", () => {
  it("fuses adjacent split to concat", () => {
    ...
    const s1 = builder.createVar(
      "s1",
      builder.tensor(MILDataType.FLOAT32, [2]),
    );
    const s2 = builder.createVar(
      "s2",
      builder.tensor(MILDataType.FLOAT32, [2]),
    );
    ...
    const _concatOp = builder.addOp("concat", { values: [s1, s2] }, [y]);
  ...
  it("fuses slice with adjacent pad", () => {
    ...
    const x = builder.createVar("x", builder.tensor(MILDataType.FLOAT32, [4]));
    const p1 = builder.createVar(
      "p1",
      builder.tensor(MILDataType.FLOAT32, [6]),
    );
    const s1 = builder.createVar(
      "s1",
      builder.tensor(MILDataType.FLOAT32, [4]),
    );

    builder.addOp("pad", { x }, [p1], { pad_amounts: [1, 1] });
    const _sliceOp = builder.addOp("slice_by_index", { x: p1 }, [s1]);
