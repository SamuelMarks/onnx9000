...
import { MILDataType, ScalarType, TensorType } from "../src/mil/types.js";
...
describe("Coverage tests", () => {
  ...
  it("covers rewriter setting shape for general ops", () => {
    const block = new Block("test_block");
    const outVar = new Var(
      "out1",
      new TensorType(MILDataType.FLOAT32, [1, 2, 3]),
    );
    const op = new Operation("add", {}, [outVar], {});
    block.addOperation(op);
    inferShapes(block);
    expect(outVar.type instanceof TensorType && outVar.type.shape).toEqual([
      1, 2, 3,
    ]);
  });
  ...
  it("covers ane redundant cast pass array branch", () => {
    ...
    const op = new Operation("cast", { x: [inVar] }, [outVar], {
      dtype: "fp16",
    });
    block.addOperation(op);
