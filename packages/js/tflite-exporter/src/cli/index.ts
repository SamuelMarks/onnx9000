...
export async function onnx2tfCli(args: string[]) {
  ...
  let _outputFile = "model.tflite";
  if (outputFlagIndex !== -1 && args[outputFlagIndex + 1]) {
    _outputFile = args[outputFlagIndex + 1]!;
  }
  ...
  const externalWeights =
    externalWeightsIdx !== -1 && args[externalWeightsIdx + 1]
      ? args[externalWeightsIdx + 1]!
      : "";

  const dynamicBatchIdx = args.indexOf("-b");
  let _dynamicBatch = -1;
  if (dynamicBatchIdx !== -1 && args[dynamicBatchIdx + 1]) {
    _dynamicBatch = parseInt(args[dynamicBatchIdx + 1]!, 10);
  }
  ...
  if (isSavedModel) {
    ...
    console.log(
      `[onnx2tf] Compiling to TFLite... (keepNchw=${keepNchw}, quantMode=${quantMode})`,
    );
    if (isMicro) {
      console.log(
