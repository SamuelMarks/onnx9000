    ...
    for (let i = 0; i < this.graph.nodes.length; i++) {
      ...
      if (node.opType === 'Conv') {
        ...
        if (
          ...
          const _isDepthwise =
            groupAttr !== undefined && groupAttr > 1 && groupAttr === numChannels;
  ...
  private foldConstants() {
    ...
    for (let i = 0; i < this.graph.nodes.length; i++) {
      ...
      if (node.opType === 'Gemm') {
        ...
        if (weightTensor?.isInitializer && weightTensor.data && weightTensor.shape.length === 2) {
      ...
      if (node.opType === 'Conv' || node.opType === 'ConvTranspose') {
        ...
        const weightTensor = this.graph.tensors[weightName];
        if (weightTensor?.isInitializer && weightTensor.data && weightTensor.shape.length === 4) {
          const groupAttr = node.attributes.group?.value as number;
