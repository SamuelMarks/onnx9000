...
describe("Quantizer Coverage", () => {
  it("should handle customQuantizationMap and fused_activation", () => {
    ...
    node1.attributes.fused_activation = new Attribute(
      "fused_activation",
      "STRING",
      "Relu",
    );
    ...
    node2.attributes.fused_activation = new Attribute(
      "fused_activation",
      "STRING",
      "Relu6",
    );
    ...
    expect(
      (quantizer as any).quantizationMap.get("custom_tensor"),
    ).toBeDefined();
    ...
    const offset = quantizer.getQuantizationOffset(
      builder as any,
      new Tensor("y", [1]),
    );
    ...
    // Test empty map
    const offset3 = quantizer.getQuantizationOffset(
      builder as any,
      new Tensor("none", [1]),
    );
