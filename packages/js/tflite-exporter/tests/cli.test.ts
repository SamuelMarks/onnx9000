...
import { onnx2tfCli } from "../src/cli/index.js";
import fs from "fs";
...
describe("TFLite Compiler - CLI", () => {
  it("should error without inputs", async () => {
    ...
    expect(mockError).toHaveBeenCalledWith(
      "Error: Must provide an input .onnx file.",
    );
  ...
  it("should process normal file with output and flags", async () => {
    ...
    expect(mockLog).toHaveBeenCalledWith(
      expect.stringContaining(
        "Compiling to TFLite... (keepNchw=true, quantMode=fp16)",
      ),
    );
  ...
  it("should process int8 mode", async () => {
    ...
    expect(mockLog).toHaveBeenCalledWith(
      expect.stringContaining("quantMode=int8"),
    );
