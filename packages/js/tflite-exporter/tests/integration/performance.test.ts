...
describe("End-to-End Testing & Regression Validations", () => {
  it("should measure compilation time under 5 seconds for a large dummy model", () => {
    ...
    console.log(
      `[onnx2tf] Compiled ~40MB dummy model in ${diffMs.toFixed(2)}ms`,
    );
  ...
  it("should compile an emulated ResNet50 topology", () => {
    ...
    graph.nodes.push(
      new Node("Relu", ["Conv1_Out"], ["Relu1_Out"], {}, "relu1"),
    );
    ...
    // Global Average Pool
    graph.nodes.push(
      new Node("GlobalAveragePool", ["Pool1_Out"], ["Gap_Out"], {}, "gap"),
    );
    ...
    graph.nodes.push(
      new Node("Gemm", ["Reshape_Out", "FC_W"], ["Output"], {}, "gemm"),
    );
  ...
  it("should compile an emulated YOLOv8 topology", () => {
    ...
    graph.nodes.push(
      new Node("Add", ["Split1", "Split2"], ["Add_Out"], {}, "add_residual"),
    );
    graph.nodes.push(
      new Node(
        "Concat",
    ...
    graph.nodes.push(
      new Node("Conv", ["Concat_Out", "Head_W"], ["Head_Out"], {}, "head_conv"),
    );
  ...
  it("should compile an emulated Whisper topology", () => {
    ...
    // Gelu
    graph.nodes.push(
      new Node("Gelu", ["Conv1_Out"], ["Gelu_Out"], {}, "gelu1"),
    );
  ...
  it("should compile an emulated DeepLabV3 topology", () => {
    ...
    graph.outputs.push(
      new ValueInfo("Concat_Out", [1, 512, 257, 257], "float32"),
    );
