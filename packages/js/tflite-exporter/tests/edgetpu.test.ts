...
describe("TFLite Compiler - EdgeTPU", () => {
  it("should apply EdgeTPU optimizations and generate warnings", () => {
    ...
    expect(warnings.some((w) => w.includes("Replaced 1 1D Convolutions"))).toBe(
      true,
    );
    ...
    expect(
      warnings.some((w) => w.includes("Dynamic StridedSlice detected")),
    ).toBe(true);
    expect(
      warnings.some((w) =>
        w.includes("Injected Zero-Padding into 1 Convolutions"),
      ),
    ).toBe(true);
    expect(
      warnings.some((w) =>
        w.includes("Operation Loop (loop1) breaks strict NNAPI"),
      ),
    ).toBe(true);
    expect(
      warnings.some((w) => w.includes("Rewrote 1 Softmax operations")),
    ).toBe(true);
  });
});
