...
function validateKServeRequest(
  ...
  for (const input of body.inputs) {
    ...
    if (!input.datatype || !VALID_DATATYPES.has(input.datatype)) {
      throw new Error(
        `Input ${input.name} has invalid datatype: ${input.datatype}`,
      );
    }
    if (input.data && Array.isArray(input.data)) {
      const expectedLength = input.shape.reduce(
...
export function addKServeRoutes(_server: Onnx9000Server, router: Router) {
  ...
  router.get("/v2/models/:model_name", async (_req, params) => {
    return new Response(
      JSON.stringify({
  ...
  router.post("/v2/models/:model_name/infer", async (req, params) => {
    ...
    if (
      ...
      const headerLengthStr = req.headers.get(
      ...
      if (headerLengthStr) {
        ...
        const combined = new Uint8Array(
          outJsonBuffer.length + outDataBuffer.byteLength,
        );
