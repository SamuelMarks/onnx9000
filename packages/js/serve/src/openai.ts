...
import type { Onnx9000Server } from "./index";
import { safeJsonParse } from "./middleware";
import type { Router } from "./router";
export function addOpenAIRoutes(_server: Onnx9000Server, router: Router) {
  ...
  router.post("/v1/chat/completions", async (req) => {
    ...
    for (const msg of messages) {
      _promptString += `<|im_start|>${msg.role}\n${msg.content}<|im_end|>\n`;
    }

    const _maxTokens = body.max_tokens || 128;
    const _temperature = body.temperature ?? 1.0;
    const _top_p = body.top_p ?? 1.0;
    const _top_k = body.top_k ?? 50;
    const _stopSequences = body.stop || [];
    const _tools = body.tools || [];
    const _presencePenalty = body.presence_penalty || 0.0;
    ...
    if (encodingFormat === "base64") {
      ...
      embeddingData =
        typeof btoa !== "undefined"
          ? btoa(b64)
          : Buffer.from(b8).toString("base64");
    }
  ...
  router.post("/v1/audio/transcriptions", async (req) => {
    ...
    const _model = formData.get("model") || "whisper-1";
