...
describe("Coverage OpenAI & KServe", () => {
  ...
  it("OpenAI - /v1/audio/transcriptions", async () => {
    let req = new Request("http://localhost/v1/audio/transcriptions", {
      method: "POST",
    });
  ...
  it("KServe - health", async () => {
    expect(
      (await server.fetch(new Request("http://localhost/v2/health/ready")))
        .status,
    ).toBe(200);
    expect(
      (await server.fetch(new Request("http://localhost/v2/health/live")))
        .status,
    ).toBe(200);
  });
  ...
  it("KServe - models", async () => {
    expect(
      (await server.fetch(new Request("http://localhost/v2/models"))).status,
    ).toBe(200);
    expect(
      (await server.fetch(new Request("http://localhost/v2/models/m1"))).status,
    ).toBe(200);
    expect(
      (
        await server.fetch(
          new Request("http://localhost/v2/models/m1/versions/1"),
        )
      ).status,
    ).toBe(200);
  });
