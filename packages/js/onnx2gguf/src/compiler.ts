...
import { GGUFTensorType, GGUFValueType, GGUFWriter } from "./builder";
...
export function compileGGUF(
  ...
  const generalKvs: Record<string, ReturnType<typeof JSON.parse>> = {
    ...
    "general.author":
      (graph as ReturnType<typeof JSON.parse>).producerName || "onnx9000",
    "general.version": Number(
      (graph as ReturnType<typeof JSON.parse>).modelVersion || 1,
    ),
  ...
  for (const initName of graph.initializers) {
    ...
    if (t) {
      ...
      const shape = t.shape
        .map((s) => (typeof s === "number" ? BigInt(s) : 1n))
        .reverse();
      ...
      for (const kv of writer.kvs) {
        if (
          kv.key === "general.alignment" &&
          kv.type === GGUFValueType.UINT32
        ) {
  ...
  for (const t of tensorDefs) {
    if (t.tensor.data) {
      u8View.set(
        new Uint8Array(
          t.tensor.data.buffer,
          t.tensor.data.byteOffset,
          t.tensor.data.byteLength,
        ),
        cursor,
      );
      let items = 1n;
