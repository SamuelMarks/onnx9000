...
import { SafeTensors } from "../src/parser/safetensors.js";
import { inferShapes } from "../src/shape_inference/infer.js";
import { unpackData } from "../src/sparse.js";
...
describe("Final Coverage Gaps", () => {
  ...
  it("unpackData coverage for int32/int64 and iterables", () => {
    ...
    const t32 = new Tensor(
      "t32",
      [1],
      "int32",
      true,
      false,
      new Uint8Array(data32.buffer),
    );
    expect(unpackData(t32)).toEqual([42]);

    const data64 = new BigInt64Array([42n]);
    const t64 = new Tensor(
      "t64",
      [1],
      "int64",
      true,
      false,
      new Uint8Array(data64.buffer),
    );
  ...
  it("SafeTensors coverage for errors and alignment", async () => {
    ...
    new DataView(badJsonBuf.buffer).setBigUint64(
      0,
      BigInt(badJsonBytes.byteLength),
      true,
    );
