...
import { sparseToCoo, unpackData } from "../src/sparse.js";
...
describe("Extra Coverage Gaps", () => {
  describe("sparse.ts fallback coverage", () => {
    ...
    it("unpackData fallback for non-iterable data", () => {
      const t = new Tensor("t", [1], "float32", true, false, {
        notIterable: true,
      } as any);
  ...
  describe("ops/index.ts non-Float32Array coverage", () => {
    ...
    it("AddOp with Uint8Array data", () => {
      ...
      const inputA = new Tensor(
        "a",
        [2],
        "float32",
        true,
        false,
        new Uint8Array(dataA.buffer),
      );
      const inputB = new Tensor(
        "b",
        [2],
        "float32",
        true,
        false,
        new Uint8Array(dataB.buffer),
      );
  ...
  describe("safetensors.ts coverage", () => {
    ...
    it("should cover swapEndianness directly", () => {
      ...
      expect(new Uint8Array(data.buffer)).toEqual(
        new Uint8Array([0x02, 0x01, 0x04, 0x03]),
      );
    });
    ...
    it("should trigger swapEndianness in getTypedArray for BE", () => {
      ...
      new Uint8Array(buffer, 8 + headerBytes.byteLength).set([
        0x02, 0x01, 0x04, 0x03,
      ]);
