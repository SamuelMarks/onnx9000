...
describe("BufferReader", () => {
  it("should read bytes and skip", async () => {
    ...
    await expect(reader.readBytes(1)).rejects.toThrow(
...
describe("Primitives", () => {
  it("should read varints", async () => {
    const data = new Uint8Array([
      0x01, 0x80, 0x01, 0xff, 0xff, 0xff, 0xff, 0x0f,
    ]);
    const reader = new BufferReader(data);
...
describe("skipField", () => {
  it("should skip various wire types", async () => {
    ...
    await expect(skipField(reader, 99)).rejects.toThrow(
      "Unsupported wire type: 99",
    );
  });
});
