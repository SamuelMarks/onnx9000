...
describe("WebNNCompiler remaining coverage", () => {
  ...
  it("Conv with autoPad SAME_UPPER and explicit pads", async () => {
    ...
    n.attributes["auto_pad"] = new Attribute(
      "auto_pad",
      "STRING",
      "SAME_UPPER",
    );
    ...
    n2.attributes["auto_pad"] = new Attribute(
      "auto_pad",
      "STRING",
      "SAME_LOWER",
    );
  ...
  it("ConvTranspose with autoPad SAME_UPPER and explicit pads", async () => {
    ...
    n.attributes["output_padding"] = new Attribute(
      "output_padding",
      "INTS",
      [1, 1],
    );
  ...
  it("Shape error", async () => {
    ...
    await expect(compileNode(n)).rejects.toThrow(
      "Shape should be evaluated on CPU statically",
    );
  });

  it("Squeeze / Unsqueeze / Tile errors", async () => {
    await expect(
      compileNode(new Node("Squeeze", ["in1"], ["out"])),
    ).rejects.toThrow("Squeeze implemented via reshape mapping");
    await expect(
      compileNode(new Node("Unsqueeze", ["in1"], ["out"])),
    ).rejects.toThrow("Unsqueeze implemented via reshape mapping");
    await expect(
      compileNode(new Node("Tile", ["in1"], ["out"])),
    ).rejects.toThrow("Tile requires expanding/concatenating");
  });
