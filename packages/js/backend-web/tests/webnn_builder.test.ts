...
describe("KerasWebNNCompiler", () => {
  it("should build Conv2DBNRelu", () => {
    ...
    expect(builder.conv2d).toHaveBeenCalledWith(
      "in",
      "w",
      expect.objectContaining({ bias: "b" }),
    );
  ...
  it("should build SeparableConv2D", () => {
    const builder = {
      conv2d: vi
        .fn()
        .mockReturnValueOnce("depth_out")
        .mockReturnValueOnce("point_out"),
    };
    const compiler = new KerasWebNNCompiler(builder as any, {} as any);
  ...
  it("should executeAsync", async () => {
    ...
    const outputs = await compiler.executeAsync(
      { out: "op" } as any,
      { in: "buf" } as any,
    );
