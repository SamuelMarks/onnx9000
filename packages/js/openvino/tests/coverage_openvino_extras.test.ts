...
describe("Coverage OpenVINO", () => {
  it("exhausts all operator handlers", () => {
    ...
    graph.addNode(
      new Node("Gelu", ["e"], ["f"], { approximate: { value: "tanh" } } as any),
    );
    graph.addNode(
      new Node("Softmax", ["f"], ["g"], { axis: { value: 1 } } as any),
    );
    graph.addNode(
    ...
    graph.addNode(
      new Node("Gather", ["h", "idx"], ["i"], { axis: { value: 1 } } as any),
    );
    ...
    graph.addNode(
      new Node("SpaceToDepth", ["m"], ["n"], {
        blocksize: { value: 2 },
      } as any),
    );
    graph.addNode(
      new Node("NonMaxSuppression", ["n"], ["o"], {
        center_point_box: { value: 1 },
      } as any),
    );
    graph.addNode(
      new Node("RoiAlign", ["o"], ["p"], { mode: { value: "avg" } } as any),
    );
    ...
    graph.addNode(
      new Node("LayerNormalization", ["r"], ["s"], {
        epsilon: { value: 1e-5 },
      } as any),
    );
    graph.addNode(
      new Node("BatchNormalization", ["s"], ["t"], {
        epsilon: { value: 1e-5 },
      } as any),
    );
    ...
    graph.addNode(
      new Node("Cast", ["v"], ["w"], { to: { value: "float32" } } as any),
    );
    graph.addNode(
    ...
    graph.addNode(
      new Node("Flatten", ["y"], ["z"], { axis: { value: 1 } } as any),
    );
    graph.addNode(
      new Node("Transpose", ["z"], ["aa"], { perm: { value: [1, 0] } } as any),
    );
    graph.addNode(
      new Node("GatherElements", ["aa", "idx2"], ["bb"], {
        axis: { value: 1 },
      } as any),
    );

    const tVal = new Tensor(
      "val",
      [1],
      "float32",
      false,
    ...
    graph.addNode(
      new Node("ConstantOfShape", ["bb"], ["cc"], {
        value: { value: tVal },
      } as any),
    );
    ...
    graph.addNode(
      new Node("Pad", ["g"], ["h2"], { mode: { value: "constant" } } as any),
    );
    graph.addNode(
      new Node("ReduceMean", ["j"], ["k2"], { keepdims: { value: 0 } } as any),
    );
