...
import { createOnnxJsonReplacer, extractJson } from "../src/index.js";
...
describe("JSON Extract SDK", () => {
  ...
  it("should handle ArrayBuffer and ArrayBufferView when dropBuffers is true", () => {
    ...
    const json = JSON.stringify(
      obj,
      createOnnxJsonReplacer({ dropBuffers: true }),
    );
    const parsed = JSON.parse(json);
  ...
  it("should allow custom bufferReplacer", () => {
    ...
    const replacer = (val: ArrayBuffer | ArrayBufferView) =>
      `[Dropped: ${val.byteLength}]`;
    const json = JSON.stringify(
      obj,
  ...
  it("should keep buffers when dropBuffers is false", () => {
    ...
    const json = JSON.stringify(
      obj,
      createOnnxJsonReplacer({ dropBuffers: false }),
    );
    const parsed = JSON.parse(json);
