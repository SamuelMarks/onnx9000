...
const add = (a: Tensor) =>
  new Tensor("Add_out", [], a.dtype, false, false, new Float32Array());
  ...
  parse(model: object | null): Graph {
    ...
    if (Array.isArray(jaxpr.invars)) {
      for (const invar of jaxpr.invars) {
        ...
        graph.inputs.push(
          new ValueInfo(nameStr, shapeArr, this.mapJaxType(typeStr)),
        );
        graph.tensors[nameStr] = t;
      }
    }
    ...
    if (Array.isArray(jaxpr.eqns)) {
      for (const eqn of jaxpr.eqns) {
        ...
        const nodeName =
          outputs.length > 0 ? `${primStr}_${firstOutput}` : primStr;
        const rawParams =
        ...
        if (Array.isArray(e.outvars)) {
          for (const outvar of e.outvars) {
            ...
            const outShapeArr = Array.isArray(o.shape)
              ? (o.shape as number[])
              : [];
            const t = new Tensor(
              outNameStr,
              outShapeArr,
              this.mapJaxType(outTypeStr),
            );
