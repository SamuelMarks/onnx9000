...
import type { DarknetLayer } from "./parser.js";
...
export class DarknetMapper {
  ...
  public map(layers: DarknetLayer[]) {
    ...
    this.graph.inputs.push(
      new ValueInfo(
        currentInput,
        ["batch_size", currentChannels, height, width],
        "float32",
      ),
    );
    ...
    for (let i = 0; i < executionLayers.length; i++) {
      ...
      if (layer.type === "convolutional") {
        ...
        const routeLayers = Array.isArray(layer.layers)
          ? layer.layers
          : [layer.layers];
        currentChannels = 0;
    ...
    const yoloLayers = executionLayers.filter(
      (l) => l.type === "yolo" || l.type === "region",
    );
    if (yoloLayers.length > 0) {
      for (let i = 0; i < executionLayers.length; i++) {
        if (
          executionLayers[i]?.type === "yolo" ||
          executionLayers[i]?.type === "region"
        ) {
  ...
  private readWeights(count: number): Float32Array {
    const _end = this.weightOffset + count;
    ...
    if (available > 0) {
      slice = this.weights.slice(
        this.weightOffset,
        this.weightOffset + available,
      );
    } else {
  ...
  private mapLayer(
    ...
    switch (layer.type) {
      case "convolutional": {
        ...
        let _biases: Float32Array;
        let _scales: Float32Array | undefined;
        let _rollingMean: Float32Array | undefined;
        let _rollingVariance: Float32Array | undefined;
        let _convWeights: Float32Array;

        const weightCount = filters * inChannels * size * size;

        if (batchNormalize) {
          _biases = this.readWeights(filters);
          _scales = this.readWeights(filters);
          _rollingMean = this.readWeights(filters);
          _rollingVariance = this.readWeights(filters);
          _convWeights = this.readWeights(weightCount);
        } else {
          _biases = this.readWeights(filters);
          _convWeights = this.readWeights(weightCount);
        }
      ...
      case "avgpool": {
        nodes.push(
          new Node("AveragePool", [input], [`${layerName}_out`], {}, layerName),
        );
        this.channelsOutput[layerIdx] = inChannels;
        break;
      }
      ...
      case "connected": {
        ...
        const t_W = new Tensor(
          `${layerName}_W`,
          [outputSize, inChannels],
          "float32",
          true,
        );
      ...
      case "route": {
        const layersList = Array.isArray(layer.layers)
          ? layer.layers
          : [layer.layers];
