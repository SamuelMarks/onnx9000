 ...
 */
// @ts-nocheck
import { BufferReader, Graph } from "@onnx9000/core";
import type { FileLoader } from "../file-loader.js";
import type { MMDNNReporter } from "../reporter.js";

// mocked coreml
const importCoreML = (_a: object) => new Graph("coreml_imported");
const parseModel = async (_a: object) => ({});
const MLPackageLoader = { parseMILProgram: (_m: object) => ({}) };
  ...
  async parse(loader: FileLoader, reporter: MMDNNReporter): Promise<Graph> {
    ...
    try {
      ...
      if (zipFile) {
        ...
        reporter.warn(
          "No .mlmodel or .zip found. Returning a mock graph for testing.",
        );
        graph = new Graph("mock_coreml_graph");
      }
    } catch (e) {
      reporter.warn(
        `CoreML parsing failed: ${e}. Returning fallback mock graph.`,
      );
