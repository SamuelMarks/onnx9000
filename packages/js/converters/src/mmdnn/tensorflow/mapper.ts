...
import type { TFNodeDef } from "./parser.js";
...
export class TFMapper {
  ...
  map(node: TFNodeDef, graph: Graph): Node[] {
    ...
    if (opType === "Placeholder") {
      ...
      let shape = [1];
      if (tfAttr?.tensor?.shape && tfAttr.tensor.shape.length > 0) {
      ...
      if (
        ...
        attrs.strides = new Attribute("strides", "INTS", [
          tfStrides[1],
          tfStrides[2],
        ]);
      }
      ...
      if (
        ...
        attrs.kernel_shape = new Attribute("kernel_shape", "INTS", [
          ksize[1],
          ksize[2],
        ]);
        delete attrs.ksize;
