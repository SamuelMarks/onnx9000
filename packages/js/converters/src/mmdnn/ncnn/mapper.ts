...
import { NcnnBinParser, type NcnnNode, type NcnnParam } from "./parser.js";
...
export class NcnnMapper {
  ...
  private mapConvolution(node: NcnnNode) {
    ...
    onnxNode.attributes.strides = new Attribute("strides", "INTS", [
      strideH,
      strideW,
    ]);
    onnxNode.attributes.dilations = new Attribute("dilations", "INTS", [
      dilationH,
      dilationW,
    ]);
    onnxNode.attributes.pads = new Attribute("pads", "INTS", [
      padH,
      padW,
      padH,
      padW,
    ...
    const wTensor = new Tensor(
      wName,
      [numOutput, inC, kernelH, kernelW],
      "float32",
      true,
    );
  ...
  private mapGeneric(node: NcnnNode) {
    ...
    for (const [k, v] of Object.entries(node.attrs)) {
      onnxNode.attributes[`ncnn_attr_${k}`] = new Attribute(
        `ncnn_attr_${k}`,
        "STRING",
        v,
      );
