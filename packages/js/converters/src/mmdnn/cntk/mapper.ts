...
import type { CNTKNode } from "./parser.js";
...
export class CNTKMapper {
  ...
  mapConvolution(layer: CNTKNode, _graph: Graph): Node[] {
    ...
    if (layer.attributes.strides) {
      node.attributes.strides = new Attribute(
        "strides",
        "INTS",
        layer.attributes.strides,
      );
    }
    if (layer.attributes.pads) {
      node.attributes.pads = new Attribute(
        "pads",
        "INTS",
        layer.attributes.pads,
      );
    }
    if (layer.attributes.dilations) {
      node.attributes.dilations = new Attribute(
        "dilations",
        "INTS",
        layer.attributes.dilations,
      );
    }
    if (layer.attributes.group) {
      node.attributes.group = new Attribute(
        "group",
        "INT",
        layer.attributes.group,
      );
    }
  ...
  mapPooling(layer: CNTKNode, _graph: Graph): Node[] {
    const poolType =
      layer.attributes.poolingType === "Average" ? "AveragePool" : "MaxPool";
  ...
  mapBatchNormalization(layer: CNTKNode, _graph: Graph): Node[] {
    const node = new Node(
      "BatchNormalization",
      layer.inputs,
      [layer.uid],
    ...
    if (layer.attributes.epsilon !== undefined) {
      node.attributes.epsilon = new Attribute(
        "epsilon",
        "FLOAT",
        layer.attributes.epsilon,
      );
    }
    if (layer.attributes.momentum !== undefined) {
      node.attributes.momentum = new Attribute(
        "momentum",
        "FLOAT",
        layer.attributes.momentum,
      );
    }
  ...
  mapSplice(layer: CNTKNode, _graph: Graph): Node[] {
    const node = new Node("Concat", layer.inputs, [layer.uid], {}, layer.name);

    const axis =
      layer.attributes.axis !== undefined ? layer.attributes.axis : -1;
  ...
  mapReshape(layer: CNTKNode, _graph: Graph): Node[] {
  ...
  mapTranspose(layer: CNTKNode, _graph: Graph): Node[] {
    const node = new Node(
      "Transpose",
      layer.inputs,
      [layer.uid],
    ...
    if (layer.attributes.perm) {
      node.attributes.perm = new Attribute(
        "perm",
        "INTS",
        layer.attributes.perm,
      );
    }
  ...
  mapAveragePooling(layer: CNTKNode, _graph: Graph): Node[] {
    const node = new Node(
      "AveragePool",
      layer.inputs,
      [layer.uid],
    ...
    const count_include_pad =
      layer.attributes.count_include_pad !== undefined
        ? layer.attributes.count_include_pad
        : 0;
