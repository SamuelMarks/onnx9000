...
// @ts-nocheck
// @ts-nocheck
import { Attribute, type Graph, Node, type Shape } from "@onnx9000/core";
...
export class PaddleMapper {
  ...
  mapConv2d(layer: object, _graph: Graph): Node[] {
  ...
  mapPool2d(layer: object, _graph: Graph): Node[] {
    const attrs = layer.attrs || {};
    const pads = attrs.paddings || [0, 0, 0, 0];
    const strides = attrs.strides || [1, 1];
    ...
    const pooling_type =
      attrs.pooling_type === "avg" ? "AveragePool" : "MaxPool";
    ...
    const node = new Node(
      ...
      outputs,
      nodeAttrs,
      layer.name || pooling_type,
    );
    return [node];
  }

  @register_paddle_op("paddle", "elementwise_add")
  mapElementwiseAdd(layer: object, _graph: Graph): Node[] {
    const inputs = [];
    if (layer.inputs) {
  ...
  mapMul(layer: object, _graph: Graph): Node[] {
    // Paddle's 'mul' is matrix multiplication, often flattened first if rank > 2.
    // X * Y
    const attrs = layer.attrs || {};
    const _x_num_col_dims = attrs.x_num_col_dims || 1;
    const _y_num_col_dims = attrs.y_num_col_dims || 1;
  ...
  mapMatMul(layer: object, _graph: Graph): Node[] {
