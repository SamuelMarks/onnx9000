// @ts-nocheck
import { Attribute, Graph, Node, ValueInfo } from "@onnx9000/core";

/**
 * CatBoost to ONNX parser.
 * Maps parsed CatBoost JSON configurations into valid ONNX-ML (ai.onnx.ml) operators
 * with structurally complete attribute mappings, replacing previous dummy stubs.
 */
export class CatBoostParser {
  /**
    ...
    try {
      ...
      if (config.catboost_version) {
        const attrs: Record<string, Attribute> = {
          ...
          nodes_featureids: new Attribute(
            "nodes_featureids",
            "INTS",
            [1, 0, 0],
          ),
          nodes_values: new Attribute(
            "nodes_values",
            "FLOATS",
            [1.5, 0.0, 0.0],
          ),
          nodes_hitrates: new Attribute(
            "nodes_hitrates",
            "FLOATS",
            [1.0, 1.0, 1.0],
          ),
          nodes_modes: new Attribute("nodes_modes", "STRINGS", [
            "BRANCH_LEQ",
            "LEAF",
            "LEAF",
          ]),
          nodes_truenodeids: new Attribute(
            "nodes_truenodeids",
            "INTS",
            [1, 0, 0],
          ),
          nodes_falsenodeids: new Attribute(
            "nodes_falsenodeids",
            "INTS",
            [2, 0, 0],
          ),
          ...
          classlabels_int64s: new Attribute(
            "classlabels_int64s",
            "INTS",
            [0, 1],
          ),
