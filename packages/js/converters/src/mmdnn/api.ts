...
import type { CaffeLayer, MxNetSymbol, PaddleModel } from "./types.js";
...
export async function convertToCoreML(
  ...
  return convert("onnx", "coreml", files, options) as Promise<
    string | JsonObject
  >;
}
...
export async function convertToPaddle(
  ...
  return convert("onnx", "paddle", files, options) as Promise<
    string | JsonObject
  >;
}
...
export async function convert(
  ...
  if (source === "onnx") {
    ...
    if (source === "caffe") {
      const { parsePrototxt, parseCaffemodel } =
        await import("./caffe/parser.js");
      ...
      try {
        ...
        if (files.length > 1) {
          ...
          const weightsParsed = (await parseCaffemodel(
      ...
      try {
        const text = await file0.text();
        const parsed = parsePbtxt(text) as {
          node: import("./tensorflow/parser.js").TFNodeDef[];
        };
      ...
      try {
        ...
        graph.nodes.push(
          new Node("Identity", ["features"], ["prediction"], {}, "identity"),
        );
      }
    } else if (source === "h2o") {
  ...
  return {
    content: `# Exported ${target} content for ${graph.name}`,
  } as JsonObject;
}
