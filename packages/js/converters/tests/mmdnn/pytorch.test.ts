...
describe("MMDNN - PyTorch Code Generation", () => {
  describe("Generator", () => {
    ...
    it("should generate MaxPool and AveragePool", () => {
      ...
      expect(code).toContain(
        "nn.AvgPool2d(kernel_size=(2, 2), stride=(1, 1), padding=(0, 0))",
      );
    });
    ...
    it("should format BatchNormalization with defaults", () => {
      ...
      const code = new PyTorchGenerator(graph).generate();
      expect(code).toContain(
        "nn.BatchNorm2d(num_features=64, eps=0.00001, momentum=0.1)",
      );
    });
    ...
    it("should handle scalar math nodes (Add, Mul) and Concat", () => {
      const graph = createGraph();
      graph.inputs = [
        new ValueInfo("a", [1], "float32"),
        new ValueInfo("b", [1], "float32"),
      ];
    ...
    it("should handle Reshape with dynamic shape", () => {
      const graph = createGraph();
      graph.inputs = [
        new ValueInfo("x", [2, 3], "float32"),
        new ValueInfo("shape", [2], "int64"),
      ];
      const reshape = new Node("Reshape", ["x", "shape"], ["y"]);
      graph.nodes = [reshape];
      const code = new PyTorchGenerator(graph).generate();
    ...
    it("should handle fallback for unknown op", () => {
      ...
      const code = new PyTorchGenerator(graph).generate();
      expect(code).toContain(
        "y = getattr(torch, 'unknownop')(x) # WARNING: Unmapped op",
      );
    });
    ...
    it("should register buffers for standalone initializers in non-Sequential nodes", () => {
      const graph = createGraph();
      graph.inputs = [new ValueInfo("x", [1], "float32")];
      graph.tensors.standalone_init = new Tensor(
        "standalone_init",
        [1],
        "float32",
      );
      ...
      const code = new PyTorchGenerator(graph).generate();
      expect(code).toContain(
        "self.register_buffer('standalone_init', torch.empty((1,)))",
      );
  ...
  describe("Serializer", () => {
    it("should correctly map dtypes to storage classes", () => {
      ...
      expect(PyTorchSerializer.getStorageClass("float64")).toBe(
        "DoubleStorage",
      );
      expect(PyTorchSerializer.getStorageClass("float16")).toBe("HalfStorage");
      expect(PyTorchSerializer.getStorageClass("bfloat16")).toBe(
        "BFloat16Storage",
      );
      ...
      expect(PyTorchSerializer.getStorageClass("unknown" as any)).toBe(
        "FloatStorage",
      );
    });
