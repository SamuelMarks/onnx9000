...
import {
  parseMxNetParams,
  parseMxNetSymbol,
} from "../../src/mmdnn/mxnet/parser.js";
...
test("MxNetMapper - Activation variations", () => {
  ...
  expect(
    mapper.map(
      { op: "Activation", name: "act", attrs: { act_type: "sigmoid" } },
      graph,
    )[0].opType,
  ).toBe("Sigmoid");
  expect(
    mapper.map(
      { op: "Activation", name: "act", attrs: { act_type: "tanh" } },
      graph,
    )[0].opType,
  ).toBe("Tanh");
  expect(
    mapper.map(
      { op: "Activation", name: "act", attrs: { act_type: "unknown" } },
      graph,
    )[0].opType,
  ).toBe("Relu");
  expect(mapper.map({ op: "Activation", name: "act" }, graph)[0].opType).toBe(
    "Relu",
  );
});
...
test("MxNetMapper - Pooling variations", () => {
  ...
  expect(mapper.map({ op: "Pooling", name: "pool" }, graph)[0].opType).toBe(
    "MaxPool",
  );
  expect(
    mapper.map(
      { op: "Pooling", name: "pool", attrs: { pool_type: "unknown" } },
      graph,
    )[0].opType,
  ).toBe("MaxPool");
});
...
test("MxNetMapper - BatchNorm variations", () => {
  const mapper = new MxNetMapper();
  const graph = new Graph();
  expect(mapper.map({ op: "BatchNorm", name: "bn" }, graph)[0].opType).toBe(
    "BatchNormalization",
  );
...
test("MxNetMapper - Reshape variations", () => {
  const mapper = new MxNetMapper();
  const graph = new Graph();
  expect(mapper.map({ op: "Reshape", name: "resh" }, graph)[0].opType).toBe(
    "Reshape",
  );
  expect(
    mapper.map(
      { op: "Reshape", name: "resh", attrs: { shape: "(1, 2, 3)" } },
      graph,
    )[0].opType,
  ).toBe("Reshape");
});
...
test("MxNetMapper - Concat variations", () => {
  const mapper = new MxNetMapper();
  const graph = new Graph();
  expect(mapper.map({ op: "Concat", name: "concat" }, graph)[0].opType).toBe(
    "Concat",
  );
});
...
test("MxNetMapper - LeakyReLU variations", () => {
  ...
  expect(
    mapper.map(
      { op: "LeakyReLU", name: "lr", attrs: { act_type: "leaky" } },
      graph,
    )[0].opType,
  ).toBe("LeakyRelu");
  expect(
    mapper.map(
      { op: "LeakyReLU", name: "lr", attrs: { act_type: "elu" } },
      graph,
    )[0].opType,
  ).toBe("Elu");
  expect(
    mapper.map(
      { op: "LeakyReLU", name: "lr", attrs: { act_type: "prelu" } },
      graph,
    )[0].opType,
  ).toBe("PRelu");
  expect(
    mapper.map(
      { op: "LeakyReLU", name: "lr", attrs: { act_type: "unknown" } },
      graph,
    )[0].opType,
  ).toBe("LeakyRelu");
  expect(mapper.map({ op: "LeakyReLU", name: "lr" }, graph)[0].opType).toBe(
    "LeakyRelu",
  );
});
...
test("MxNetMapper - UpSampling variations", () => {
  ...
  expect(
    mapper.map(
      { op: "UpSampling", name: "us", attrs: { sample_type: "nearest" } },
      graph,
    )[0].attributes.mode.value,
  ).toBe("nearest");
  expect(
    mapper.map({ op: "UpSampling", name: "us" }, graph)[0].attributes.mode
      .value,
  ).toBe("linear");
});
...
test("MxNetMapper - SliceChannel variations", () => {
  ...
  expect(
    mapper.map(
      { op: "SliceChannel", name: "sc", attrs: { axis: "2" } },
      graph,
    )[0].opType,
  ).toBe("Split");
  expect(mapper.map({ op: "SliceChannel", name: "sc" }, graph)[0].opType).toBe(
    "Split",
  );
});
...
test("MxNetMapper - Deconvolution", () => {
  const mapper = new MxNetMapper();
  const graph = new Graph();
  expect(mapper.map({ op: "Deconvolution", name: "dc" }, graph)[0].opType).toBe(
    "ConvTranspose",
  );
});
...
test("MxNetMapper - FullyConnected", () => {
  ...
  const graph = new Graph();
  expect(
    mapper.map({ op: "FullyConnected", name: "fc1" }, graph)[0].opType,
  ).toBe("Gemm");
});
...
test("MxNetMapper - Dropout", () => {
  const mapper = new MxNetMapper();
  const graph = new Graph();
  expect(mapper.map({ op: "Dropout", name: "drop" }, graph)[0].opType).toBe(
    "Identity",
  );
});
...
test("MxNetMapper - Misc Ops", () => {
  ...
  const graph = new Graph();
  expect(
    mapper.map({ op: "elemwise_add", name: "eadd" }, graph)[0].opType,
  ).toBe("Add");
  expect(
    mapper.map({ op: "elemwise_sub", name: "esub" }, graph)[0].opType,
  ).toBe("Sub");
  expect(
    mapper.map({ op: "elemwise_mul", name: "emul" }, graph)[0].opType,
  ).toBe("Mul");
  expect(
    mapper.map({ op: "broadcast_add", name: "badd" }, graph)[0].opType,
  ).toBe("Add");
  expect(
    mapper.map({ op: "broadcast_mul", name: "bmul" }, graph)[0].opType,
  ).toBe("Mul");
  expect(mapper.map({ op: "SoftmaxOutput", name: "sm" }, graph)[0].opType).toBe(
    "Softmax",
  );
  expect(mapper.map({ op: "Flatten", name: "flat" }, graph)[0].opType).toBe(
    "Flatten",
  );
});
...
function buildMxNetParamBuffer(options: object) {
  ...
  for (const arr of arrays) {
    ...
    const elements = arr.shape.reduce((a: number, b: number) => a * b, 1);
    const bytesPerElement = arr.dtypeCode === 1 ? 8 : 4;
    size += elements * bytesPerElement;
  }
  size += 8;
  for (const name of names) size += 8 + new TextEncoder().encode(name).length;
...
test("parseMxNetParams - Error Invalid params magic", () => {
  ...
  expect(() => parseMxNetParams(buf)).toThrowError(
    /Invalid MXNet params magic/,
  );
});
...
test("parseMxNetParams - Error Invalid NDArray block magic", () => {
  const buf = buildMxNetParamBuffer({
    ...
    count: 1,
    arrays: [
      { tensorMagic: 0x99999999, shape: [1], dtypeCode: 0, data: [1.0] },
    ],
    numNames: 1,
    names: ["test1"],
  });
  expect(() => parseMxNetParams(buf)).toThrowError(
    /Invalid MXNet NDArray block magic/,
  );
});
...
test("parseMxNetParams - Error Names count mismatch", () => {
  ...
  expect(() => parseMxNetParams(buf)).toThrowError(
    /Names count 2 does not match arrays count 1/,
  );
});
