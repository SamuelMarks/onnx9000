...
import {
  CaffeMapper,
  register_caffe_op,
} from "../../src/mmdnn/caffe/mapper.js";
import {
  parseCaffemodel,
  parsePrototxt,
} from "../../src/mmdnn/caffe/parser.js";
    ...
    it("should parse binary caffe model", async () => {
      ...
      const blobBytes = [
        ...
        ...encodeLengthDelimited(
          5,
          Array.from(new Uint8Array(new Float32Array([2.5]).buffer)),
        ), // data (LENGTH_DELIMITED)
...
describe("MMDNN - Caffe Importer Mapping", () => {
  ...
  it("should map Eltwise", () => {
    expect(
      mapper.map({ type: "Eltwise", eltwise_param: { operation: 0 } }, graph)[0]
        .opType,
    ).toBe("Mul");
    expect(
      mapper.map({ type: "Eltwise", eltwise_param: { operation: 1 } }, graph)[0]
        .opType,
    ).toBe("Add");
    expect(
      mapper.map({ type: "Eltwise", eltwise_param: { operation: 2 } }, graph)[0]
        .opType,
    ).toBe("Max");
