...
import { type CNTKNode, CNTKParser } from "../../src/mmdnn/cntk/parser.js";
...
describe("CNTK Mapper", () => {
  ...
  it("maps Pooling with attributes", () => {
    const layer = createNode("Pooling", {
      poolingType: "Average",
      kernel_shape: [3, 3],
      strides: [2, 2],
      pads: [1, 1, 1, 1],
  ...
  it("maps BatchNormalization with and without attributes", () => {
    let layer = createNode("BatchNormalization", {
      epsilon: 1e-5,
      momentum: 0.9,
    });
    let nodes = mapper.map(layer, graph);
  ...
  it("handles unknown OP gracefully", () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "Unsupported CNTK node type: UnknownOp",
    );
