...
import {
  generateTFJSCode,
  isLinearGraph,
} from "../../src/mmdnn/tfjs/generator.js";
...
describe("TFJS Generator - isLinearGraph", () => {
  ...
  test("multiple inputs/outputs", () => {
    ...
    graph1.inputs.push(
      new ValueInfo("in1", [1], "float32"),
      new ValueInfo("in2", [1], "float32"),
    );
...
describe("TFJS Generator - generateTFJSCode", () => {
  test("sequential API for linear model", () => {
    ...
    const _code = generateTFJSCode(graph);
    ...
    expect(() => generateTFJSCode(graph2)).toThrow(
      "Unsupported operator: UnknownOp",
    );
  });
...
describe("TFJS Serializer", () => {
  test("serializeTFJSWeights", () => {
    ...
    expect((modelJson as any).weightsManifest[0].weights[0].shape).toEqual([
      2, 2,
    ]);
