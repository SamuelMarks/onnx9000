...
import { fetchRemoteUrl } from "../src/keras/browser-io.js";
import {
  emitAttention,
  emitEmbedding,
...
describe("Keras Module", () => {
  ...
  it("should test optimizers applyQuantization", () => {
    const weights = [
      { name: "w", dtype: "float32", data: new Float32Array(10) },
    ];
  ...
  it("should test weight-loader downloadWeightShards", async () => {
    ...
    const weights = await downloadWeightShards(
      manifest,
      "http://test.com",
      fetcher,
    );
  ...
  it("should test weight-loader unsupported string dtype", async () => {
    ...
    await expect(
      downloadWeightShards(manifest, "http://test.com", fetcher),
    ).rejects.toThrow();
  });
  ...
  it("should test weight-loader default fetcher success and failure", async () => {
    ...
    await expect(
      downloadWeightShards(manifest, "http://test.com"),
    ).rejects.toThrow();
  });
        ...
        config: {
          layers: [
            ...
            },
            {
              class_name: "Flatten",
              config: {},
              inboundNodes: [["gmaxpool_1"]],
            },
            {
              class_name: "Dropout",
              config: { rate: 0.5 },
              inboundNodes: [["flatten_1"]],
            },
            {
              class_name: "LeakyReLU",
              config: { alpha: 0.2 },
              inboundNodes: [["dropout_1"]],
            },
            {
              class_name: "PReLU",
              config: {},
              inboundNodes: [["leaky_relu_1"]],
            },
            {
              class_name: "ELU",
              config: { alpha: 1.0 },
              inboundNodes: [["prelu_1"]],
            },
            {
            ...
            },
            {
              class_name: "Softmax",
              config: { axis: -1 },
              inboundNodes: [["thresholded_relu_1"]],
            },
            {
              class_name: "QConv2D",
              config: {
            ...
            },
            {
              class_name: "Add",
              config: {},
              inboundNodes: [["centercrop_1"], ["resizing_1"]],
            },
            {
              class_name: "Subtract",
              config: {},
              inboundNodes: [["add_1"], ["centercrop_1"]],
            },
            {
              class_name: "Multiply",
              config: {},
              inboundNodes: [["sub_1"], ["add_1"]],
            },
            {
              class_name: "Minimum",
              config: {},
              inboundNodes: [["mul_1"], ["sub_1"]],
            },
            {
              class_name: "Maximum",
              config: {},
              inboundNodes: [["min_1"], ["mul_1"], ["sub_1"]],
            },
            {
            ...
            },
            {
              class_name: "Average",
              config: {},
              inboundNodes: [["concat_1"], ["max_1"]],
            },
            {
              class_name: "Dot",
              config: { axes: 1 },
              inboundNodes: [["avg_1"], ["concat_1"]],
            },
            {
  ...
  it("should test emitters-norm", () => {
    ...
    emitLayerNormalization("in", "out", "gamma", "beta", "name", {
      epsilon: 1e-5,
      axis: -1,
    });
  ...
  it("should test emitters-tfjs", () => {
    ...
    expect(() =>
      mapTfjsOpToOnnx("UnsupportedOp", ["in"], "out", "name"),
    ).toThrow("Unsupported TF.js Op: UnsupportedOp");
  });

  it("should test emitters-attention", () => {
    emitAttention("q", "v", "k", "out", "name", {
      useScale: true,
      causal: false,
    });
    emitAttention("q", "v", "k", "out", "name", {
      ...
      modelTopology: {
        ...
        config: {
          layers: [
            {
              class_name: "GlobalAveragePooling2D",
              config: { name: "g_pool", keepdims: true },
            },
