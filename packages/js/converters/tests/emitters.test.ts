...
describe("emitters", () => {
  it("emitActivation", () => {
    ...
    expect(emitActivation("selu", "in", "out", "n")).toEqual([
      {
        ...
        attributes: [
          {
            name: "alpha",
            f: 1.6732632423543772,
            type: "FLOAT",
          },
          {
            name: "gamma",
            f: 1.0507009873554805,
            type: "FLOAT",
          },
        ],
      },
    ...
    expect(
      emitActivation("leaky_relu", "in", "out", "n", { alpha: 0.1 }),
    ).toEqual([
      {
        opType: "LeakyRelu",
        inputs: ["in"],
    ...
    expect(
      emitActivation("prelu", "in", "out", "n", { alphaWeightName: "aw" }),
    ).toEqual([
      {
        opType: "PRelu",
        inputs: ["in", "aw"],
    ...
    expect(
      emitActivation("thresholded_relu", "in", "out", "n", { theta: 0.5 }),
    ).toEqual([
      {
