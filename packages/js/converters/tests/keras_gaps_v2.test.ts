...
describe("Keras2OnnxConverter Extra Gaps V2", () => {
  it("should cover signature mapping, mixed precision, and complex shape mapping", async () => {
    ...
    converter.convert();
    expect(
      converter._test_finalNodes.find((n) => n.opType === "Cast"),
    ).toBeDefined();
    expect(
      converter._test_finalNodes.find((n) => n.outputs.includes("output_sig")),
    ).toBeDefined();
  });
  ...
  it("should cover Multiply/Average multi-node and masking", async () => {
    ...
    registerCustomKerasLayer(
      "MaskingLayerFinal",
      (_nodeName, _layerName, inputs, outName, _config) => {
        return [
          {
            opType: "Gather",
    ...
    converter.convert();
    expect(
      converter._test_finalNodes.find((n) => n.opType === "Mean"),
    ).toBeDefined();
    expect(
      converter._test_finalNodes.find((n) => n.opType === "Equal"),
    ).toBeDefined();
  });
});
