...
import {
  emitAttention,
  emitEmbedding,
} from "../src/keras/emitters-attention.js";
import { emitConv, emitSeparableConv } from "../src/keras/emitters-conv.js";
import { emitConcat, emitDot, emitMerge } from "../src/keras/emitters-merge.js";
import {
...
import { emitGlobalPool, emitPool } from "../src/keras/emitters-pool.js";
import {
  emitBidirectional,
  emitRNNBase,
  reorderGRUGates,
...
describe("emitters-pool", () => {
  test("emitPool Max/Average", () => {
    ...
    expect(nodesMax[0].attributes?.find((a) => a.name === "auto_pad")?.s).toBe(
      "VALID",
    );
    ...
    expect(nodesAvg[0].attributes?.find((a) => a.name === "auto_pad")?.s).toBe(
      "SAME_UPPER",
    );
  });

  test("emitGlobalPool", () => {
    const nodesKeep = emitGlobalPool("Max", "in", "out", "gpool1", {
      keepDims: true,
    });
    expect(nodesKeep.length).toBe(1);
    expect(nodesKeep[0].opType).toBe("GlobalMaxPool");

    const nodesSqueeze = emitGlobalPool("Average", "in", "out", "gpool2", {
      keepDims: false,
    });
...
describe("emitters-norm", () => {
  ...
  test("emitLayerNormalization", () => {
    const nodes = emitLayerNormalization(
      "in",
      "out",
      "gamma",
      "beta",
  ...
  test("emitReshape, emitFlatten, emitTranspose, emitPad", () => {
    expect(emitReshape("in", "shape", "out", "reshape")[0].opType).toBe(
      "Reshape",
    );
    expect(emitFlatten("in", "out", 1, "flatten")[0].opType).toBe("Flatten");
    expect(emitTranspose("in", "out", [0, 2, 1], "transpose")[0].opType).toBe(
      "Transpose",
    );
  ...
  test("emitGroupNormalization", () => {
    const nodes = emitGroupNormalization(
      "in",
      "out",
      32,
      "gamma",
    ...
    expect(nodes[0].attributes?.find((a) => a.name === "num_groups")?.i).toBe(
      32,
    );
...
describe("emitters-merge", () => {
  ...
  test("emitConcat & emitDot", () => {
    expect(emitConcat(["in1", "in2"], "out", 1, "concat")[0].opType).toBe(
      "Concat",
    );
...
describe("emitters-tfjs", () => {
  test("mapTfjsOpToOnnx valid ops", () => {
    expect(mapTfjsOpToOnnx("Add", ["a", "b"], "out", "add")[0].opType).toBe(
      "Add",
    );
    expect(mapTfjsOpToOnnx("AddV2", ["a", "b"], "out", "add")[0].opType).toBe(
      "Add",
    );
    expect(mapTfjsOpToOnnx("Sub", ["a", "b"], "out", "sub")[0].opType).toBe(
      "Sub",
    );
    expect(mapTfjsOpToOnnx("Mul", ["a", "b"], "out", "mul")[0].opType).toBe(
      "Mul",
    );
    expect(mapTfjsOpToOnnx("RealDiv", ["a", "b"], "out", "div")[0].opType).toBe(
      "Div",
    );
    expect(mapTfjsOpToOnnx("Div", ["a", "b"], "out", "div2")[0].opType).toBe(
      "Div",
    );
    expect(mapTfjsOpToOnnx("MatMul", ["a", "b"], "out", "mm")[0].opType).toBe(
      "MatMul",
    );
    expect(mapTfjsOpToOnnx("Square", ["a"], "out", "sq")[0].opType).toBe("Pow");
    expect(mapTfjsOpToOnnx("Sqrt", ["a"], "out", "sqrt")[0].opType).toBe(
      "Sqrt",
    );
    expect(mapTfjsOpToOnnx("Exp", ["a"], "out", "e")[0].opType).toBe("Exp");
    expect(mapTfjsOpToOnnx("Log", ["a"], "out", "l")[0].opType).toBe("Log");
    expect(mapTfjsOpToOnnx("Maximum", ["a", "b"], "out", "max")[0].opType).toBe(
      "Max",
    );
    expect(mapTfjsOpToOnnx("Minimum", ["a", "b"], "out", "min")[0].opType).toBe(
      "Min",
    );
    expect(mapTfjsOpToOnnx("Sum", ["a"], "out", "s")[0].opType).toBe(
      "ReduceSum",
    );
    expect(mapTfjsOpToOnnx("Mean", ["a"], "out", "m")[0].opType).toBe(
      "ReduceMean",
    );
    expect(mapTfjsOpToOnnx("Max", ["a"], "out", "ma")[0].opType).toBe(
      "ReduceMax",
    );
    expect(mapTfjsOpToOnnx("Min", ["a"], "out", "mi")[0].opType).toBe(
      "ReduceMin",
    );
    expect(mapTfjsOpToOnnx("ArgMax", ["a"], "out", "am")[0].opType).toBe(
      "ArgMax",
    );
    expect(mapTfjsOpToOnnx("ArgMin", ["a"], "out", "ami")[0].opType).toBe(
      "ArgMin",
    );
    expect(mapTfjsOpToOnnx("Split", ["a", "b"], "out", "sp")[0].opType).toBe(
      "Split",
    );
    expect(
      mapTfjsOpToOnnx("SplitV", ["a", "b", "c"], "out", "spv")[0].opType,
    ).toBe("Split");
    expect(mapTfjsOpToOnnx("Concat", ["a", "b"], "out", "c")[0].opType).toBe(
      "Concat",
    );
    expect(
      mapTfjsOpToOnnx("ConcatV2", ["a", "b", "c"], "out", "cv2")[0].opType,
    ).toBe("Concat");
    expect(
      mapTfjsOpToOnnx("Slice", ["a", "b", "c"], "out", "sl")[0].opType,
    ).toBe("Slice");
    expect(
      mapTfjsOpToOnnx("StridedSlice", ["a", "b", "c", "d"], "out", "ssl")[0]
        .opType,
    ).toBe("Slice");
    expect(mapTfjsOpToOnnx("Gather", ["a", "b"], "out", "g")[0].opType).toBe(
      "Gather",
    );
    expect(
      mapTfjsOpToOnnx("GatherV2", ["a", "b", "c"], "out", "gv2")[0].opType,
    ).toBe("Gather");
    expect(
      mapTfjsOpToOnnx("GatherNd", ["a", "b"], "out", "gnd")[0].opType,
    ).toBe("GatherND");
    expect(
      mapTfjsOpToOnnx("Where", ["a", "b", "c"], "out", "w")[0].opType,
    ).toBe("Where");
    expect(
      mapTfjsOpToOnnx("TensorScatterUpdate", ["a", "b", "c"], "out", "tsu")[0]
        .opType,
    ).toBe("ScatterND");
    expect(
      mapTfjsOpToOnnx("ResizeBilinear", ["a", "b"], "out", "rb")[0].opType,
    ).toBe("Resize");
    expect(
      mapTfjsOpToOnnx("ResizeNearestNeighbor", ["a", "b"], "out", "rnn")[0]
        .opType,
    ).toBe("Resize");
  });
...
describe("emitters-rnn", () => {
  test("emitRNNBase", () => {
    ...
    expect(
      nodesState[0].attributes?.find((a) => a.name === "direction")?.s,
    ).toBe("reverse");
    ...
    expect(nodesStateWithInitial[0].inputs).toEqual([
      "in",
      "w",
      "r",
      "",
  ...
  test("emitBidirectional", () => {
    ...
    expect(
      nodesConcat[0].attributes?.find((a) => a.name === "direction")?.s,
    ).toBe("bidirectional");
    ...
    expect(
      nodesSum[1].attributes?.find((a) => a.name === "axes")?.ints,
    ).toEqual([0]);
    ...
    expect(
      nodesSumSeq[1].attributes?.find((a) => a.name === "axes")?.ints,
    ).toEqual([2]);
    ...
    expect(
      nodesAve[1].attributes?.find((a) => a.name === "axes")?.ints,
    ).toEqual([2]);
    ...
    expect(
      nodesAveState[1].attributes?.find((a) => a.name === "axes")?.ints,
    ).toEqual([0]);
