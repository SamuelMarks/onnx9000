...
describe("Triton Generator Final", () => {
  it("should generate Triton code for all op types and hit coverage", () => {
    ...
    for (const op of ops) {
      ...
      if (op === "Cast")
        node.attributes.to = new Attribute("to", "STRING", "float16");
      if (op === "Constant")
        node.attributes.value = new Attribute("value", "FLOAT", 1.0);
      if (op === "BitShift")
        node.attributes.direction = new Attribute(
          "direction",
          "STRING",
          "LEFT",
        );
  ...
  it("should throw on large BLOCK_M", () => {
    const g = new Graph("g");
    expect(() => generateTriton(g, { blockM: 3000 })).toThrow(
      "BLOCK_M too large",
    );
  });
});
