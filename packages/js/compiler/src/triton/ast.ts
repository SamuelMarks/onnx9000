...
export function generateTriton(
  graph: Graph,
  config: TritonConfig = {},
): string {
  ...
  ast.pushLine(
    "# Analysis: This kernel is likely compute-bound due to high dot-product density.",
  );
  ast.pushLine(
  ...
  for (const output of graph.outputs) {
    const outName =
      typeof output === "string"
        ? output
        : (output as ReturnType<typeof JSON.parse>).name;
    args.push(outName);
  }
  ...
  ast.pushLine(
    'tl.device_assert(offsets_m < BLOCK_M * 1024, "Out of bounds access")',
  );
  ...
  for (const input of graph.inputs) {
    ...
    if (input.shape && input.shape.length === 2) {
      ...
      ast.pushLine(
        `${input.name}_tile = tl.load(${input.name}_ptrs, mask=mask_m, other=0.0)`,
      );
    }
  }
  ...
  const getOpName = (name: string) =>
    inputNames.has(name) ? `${name}_tile` : name;
  ...
  for (const node of graph.nodes) {
    ...
    if (domain !== "" && domain !== "ai.onnx" && domain !== "ai.onnx.ml") {
      ast.pushLine(
        `# WARNING: Custom domain ${domain} might have different semantics.`,
      );
    }
    ...
    if (node.opType.includes("String")) {
      ...
      ast.pushLine(
        `# WARNING: String tensors are unsupported in Triton (${node.opType})`,
      );
      continue;
    }
    if (!supportedOps.has(node.opType)) {
      ...
      ast.pushLine(
        `# WARNING: Unsupported op ${node.opType} (159. Emit fallback comments)`,
      );
      continue;
    }
    const out = sanitize(node.outputs[0] || ""); // 156. Sanitize all node names.
    const in0 = node.inputs[0]
      ? getOpName(node.inputs[0]).replace(/[^a-zA-Z0-9_]/g, "_")
      : "";
    const in1 = node.inputs[1]
      ? getOpName(node.inputs[1]).replace(/[^a-zA-Z0-9_]/g, "_")
      : "";
    ...
    if (node.opType === "Add") {
      ...
      ast.pushLine(
        `${out} = tl.where(${in0} > 0, 1.0, tl.where(${in0} < 0, -1.0, 0.0))`,
      );
      ...
      ast.pushLine(
        `a_tile = tl.load(${node.inputs[0]}_ptrs + k * stride_${node.inputs[0]}_1)`,
      );
      ast.pushLine(
        `b_tile = tl.load(${node.inputs[1]}_ptrs + k * stride_${node.inputs[1]}_0)`,
      );
      ...
      ast.pushLine(
        `${out} = 0.5 * ${in0} * (1.0 + tl.math.erf(${in0} / 1.41421356))`,
      );
      ...
      ast.pushLine(
        `${out}_var = tl.sum(${out}_diff * ${out}_diff, axis=0) / BLOCK_N`,
      );
  ...
  for (const output of graph.outputs) {
    ...
    if ((output as ReturnType<typeof JSON.parse>).dtype === "string") {
      ...
      ast.pushLine(
        `# WARNING: String outputs are unsupported in Triton (${outName})`,
      );
      continue;
    }
    if ((output as ReturnType<typeof JSON.parse>).dtype === "sequence") {
      ...
      ast.pushLine(
        `# WARNING: Sequence outputs are unsupported in Triton (${outName})`,
      );
      continue;
    }
  ...
  for (const output of graph.outputs) {
    const outName =
      typeof output === "string"
        ...
        : (output as ReturnType<typeof JSON.parse>).name;
    ast.pushLine(
      `${outName} = torch.empty_like(${graph.inputs[0]?.name || "in_0"})`,
    );
  }

  // 107. Generate grid = lambda META: ... logic.
  ast.pushLine(
    "grid = lambda META: (triton.cdiv(x.shape[0], META['BLOCK_M']), )",
  );
