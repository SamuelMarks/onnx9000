 ...
 * Provides generator functionality for the c-compiler package.
 */
...
export class CGenerator {
  ...
  constructor(
    graph: Graph,
    prefix: string = "model_",
    emitCpp: boolean = false,
  ) {
    this.graph = graph;
  ...
  public generateSource(): string {
    ...
    for (const intermediate of intermediates) {
      ...
      if (tensor?.data) {
        const maxVals = Math.min(
          size,
          1024,
          Math.floor(tensor.data.byteLength / 4),
        ); // Limit inline weights for demo
        const values: string[] = [];
    ...
    for (const node of this.graph.nodes) {
      ...
      const in1Size =
        inputs.length > 0 ? this.getTensorSize(node.inputs[0] || "") : 256;
