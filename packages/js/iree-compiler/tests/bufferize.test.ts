...
describe("Bufferization Pass", () => {
  it("should bufferize web.tensor.empty, web.linalg.fill, and web.linalg.matmul", () => {
    ...
    const mappedGenericOp = block.operations.find(
      (o) => o.opcode === "web.some.tensor.op",
    );
...
it("should cover fallback branches for fill and matmul", () => {
  ...
  const fillMapped = block.operations.find(
    (o) => o.opcode === "web.linalg.fill",
  );
  expect(fillMapped?.operands[1]).toBe(unmappedOut);

  const matmulMapped = block.operations.find(
    (o) => o.opcode === "web.linalg.matmul",
  );
