...
describe("Lower Linalg to HAL", () => {
  it("should lower memref.alloc, linalg.matmul, linalg.fill, and generic ops", () => {
    ...
    const fillOp = new Operation(
      "web.linalg.fill",
      [fillVal, allocOp.results[0]!],
      [tensorType],
    );
...
it("should fall back for unmapped operands", () => {
  ...
  const matmul = new Operation(
    "web.linalg.matmul",
    [unmappedVal, unmappedVal, unmappedVal],
    [],
  );
  ...
  const dispatchOp = block.operations.find(
    (o) => o.opcode === "web.hal.command_buffer.dispatch",
  );
  expect(dispatchOp).toBeDefined();
  expect(dispatchOp?.operands.includes(unmappedVal)).toBe(true);

  const fillOp = block.operations.find(
    (o) => o.opcode === "web.hal.command_buffer.fill_buffer",
  );
