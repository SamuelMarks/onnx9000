...
import { Block, Operation, Region, type Type, Value } from "../src/ir/core.js";
...
describe("Web MHLO Dialect", () => {
  ...
  it("should create structural ops", () => {
    ...
    expect(mhlo.broadcastInDim(val1, [0], type).opcode).toBe(
      "web.mhlo.broadcast_in_dim",
    );
    expect(mhlo.reshape(val1, type).opcode).toBe("web.mhlo.reshape");
    expect(mhlo.transpose(val1, [1, 0], type).opcode).toBe(
      "web.mhlo.transpose",
    );
    expect(mhlo.concatenate([val1, val2], 0, type).opcode).toBe(
      "web.mhlo.concatenate",
    );
    expect(mhlo.slice(val1, [0], [5], [1], type).opcode).toBe("web.mhlo.slice");
    expect(mhlo.dynamicSlice(val1, [val2], [5], type).opcode).toBe(
      "web.mhlo.dynamic_slice",
    );
    expect(mhlo.gather(val1, val2, {}, [5], type).opcode).toBe(
      "web.mhlo.gather",
    );
  });
  ...
  it("should create regions ops", () => {
    ...
    expect(mhlo.reduce([val1], [val2], [0], region, [type]).opcode).toBe(
      "web.mhlo.reduce",
    );
    expect(
      mhlo.reduceWindow([val1], [val2], [2], [1], [1], [1], [[0, 0]], region, [
        type,
      ]).opcode,
    ).toBe("web.mhlo.reduce_window");
    expect(mhlo.scatter(val1, val2, val1, region, {}, type).opcode).toBe(
      "web.mhlo.scatter",
    );
  });
});
