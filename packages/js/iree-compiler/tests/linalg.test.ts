...
import { type Operation, Region, Value } from "../src/ir/core.js";
...
describe("Web Linalg Dialect", () => {
  it("should create linalg ops", () => {
    ...
    expect(linalg.matmul(val1, val2, val3, type).opcode).toBe(
      "web.linalg.matmul",
    );
    expect(linalg.batchMatmul(val1, val2, val3, type).opcode).toBe(
      "web.linalg.batch_matmul",
    );
    expect(
      linalg.conv2dNhwcHwcf(val1, val2, val3, [1, 1], [1, 1], type).opcode,
    ).toBe("web.linalg.conv_2d_nhwc_hwcf");
    expect(
      linalg.poolingNhwcMax(val1, val2, val3, [1, 1], [1, 1], type).opcode,
    ).toBe("web.linalg.pooling_nhwc_max");
...
describe("Web Memref Dialect", () => {
  it("should create memref ops", () => {
    ...
    expect(memref.load(val1, [val2, val2], { id: "float32" }).opcode).toBe(
      "web.memref.load",
    );
    expect(memref.store(val1, val1, [val2, val2]).opcode).toBe(
      "web.memref.store",
    );
  });
});
