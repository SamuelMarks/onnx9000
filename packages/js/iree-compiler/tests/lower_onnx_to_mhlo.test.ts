...
describe("lowerONNXToMHLO", () => {
  it("should lower all basic ops", () => {
    ...
    // Add Transpose
    g.nodes.push(
      new Node("Transpose", ["in1"], ["t_trans"], {
        perm: { type: "INTS", value: [1, 0] },
      }),
    );

    // Add Concat
    g.nodes.push(
      new Node("Concat", ["in1", "init"], ["t_cat"], {
        axis: { type: "INT", value: 0 },
      }),
    );

    // Add Slice
    g.nodes.push(
      new Node("Slice", ["in1", "init", "init", "init", "init"], ["out"]),
    );
  ...
  it("should throw on unknown node", () => {
    ...
    expect(
      lowerONNXToMHLO(g).blocks[0].operations.some(
        (o) => o.opcode === "web.mhlo.custom_call",
      ),
    ).toBe(true);
  });
...
it("should throw on missing output", () => {
  ...
  g.inputs = [{ name: "in1", dtype: "float32", shape: [2, 2], id: "in1" }];
  g.outputs = [
    { name: "out_missing", dtype: "float32", shape: [2, 2], id: "out_missing" },
  ];
  g.nodes.push(
    new Node("Add", ["in1", "in1"], ["out1", "tensor_out", "empty_out"]),
  );
...
it("covers string dimensions in shape for type inference", () => {
  const g = new Graph("test");
  g.inputs = [
    { name: "in1", dtype: "float32", shape: ["batch", 2], id: "in1" },
  ];
  g.outputs = [
    { name: "out", dtype: "float32", shape: ["batch", 2], id: "out" },
  ];
...
it("covers string dimensions in shape for type inference in graph.tensors", () => {
  ...
  g.nodes.push(
    new Node("Add", ["tensor_only", "tensor_only"], ["tensor_only"]),
  );
