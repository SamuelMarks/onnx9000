...
describe("@onnx9000/optimum", () => {
  const createMockGraph = () => {
    ...
    graph.inputs = [
      { name: "input_1", type: "tensor", shape: [1, 3, 224, 224] },
    ] as any;
    graph.outputs = [
    ...
    const deadNode = new Node(
      "Add",
      ["relu_out", "other_weight"],
      ["dead_out"],
    );

    // Dropout node
    const dropoutNode = new Node(
      "Dropout",
      ["relu_out"],
      ["drop_out", "drop_mask"],
    );
  ...
  it("should optimize graph by eliminating Identity, Dropout, and DCE", async () => {
    ...
    const outputId = optGraph.nodes.find((n) =>
      n.outputs.includes("identity_output"),
    );
  ...
  it("should simplify graph similarly without fusions", async () => {
    ...
    expect(
      simpGraph.nodes.find(
        (n) => n.opType === "Identity" && n.outputs.includes("identity_output"),
      ),
    ).toBeDefined();
  });
