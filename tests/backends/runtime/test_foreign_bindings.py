...
def test_csharp_bindings():
    ...
    p = Path("src/onnx9000/backends/runtime/bindings/csharp/onnxruntime.cs")
    assert p.exists()
    content = p.read_text()
...
def test_java_bindings():
    ...
    p = Path("src/onnx9000/backends/runtime/bindings/java/OrtEnvironment.java")
    assert p.exists()
    content = p.read_text()
...
def test_js_bindings():
    ...
    p = Path("src/onnx9000/backends/runtime/bindings/js/onnxruntime-web.js")
    assert p.exists()
    content = p.read_text()
    assert "class InferenceSession" in content
