...
def test_sequence_ops(temp_dir: Path):
    ...
    @onnx9000.jit
    def seq_model(x, pos):
        """Provides seq model functionality and verification."""
        seq = onnx9000.core.ops.sequence_construct([x])
        out = onnx9000.core.ops.sequence_at(seq, pos)
        s_e = onnx9000.core.ops.sequence_empty(1)
        s_del = onnx9000.core.ops.sequence_erase(seq, pos)
        s_ins = onnx9000.core.ops.sequence_insert(seq, x, pos)
        s_len = onnx9000.core.ops.sequence_length(seq)
        s_map = onnx9000.core.ops.sequence_map(seq)
        c = onnx9000.core.ops.concat_from_sequence(seq, axis=0)
        s = onnx9000.core.ops.split_to_sequence(x, axis=0)
