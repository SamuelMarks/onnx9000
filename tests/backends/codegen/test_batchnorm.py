...
def test_batchnorm_opsets(temp_dir: Path):
    ...
    def batchnorm_model(x, scale, b, mean, var):
        """Provides batchnorm model functionality and verification."""
        from onnx9000.frontends.frontend.utils import record_op

        if hasattr(onnx9000.core.ops, "batch_normalization"):
            return onnx9000.core.ops.batch_normalization(x, scale, b, mean, var)
        else:
            return record_op(
