...
def test_rev_scatter(temp_dir: Path):
    ...
    def rs_model(x, lengths, indices, updates):
        """Provides rs model functionality and verification."""
        r = onnx9000.core.ops.reverse_sequence(x, lengths)
        s = onnx9000.core.ops.scatter(x, indices, updates)
        se = onnx9000.core.ops.scatter_elements(x, indices, updates)
        snd = onnx9000.core.ops.scatter_nd(x, indices, updates)
        return r, s, se, snd
