...
def test_paddle_builder_infer_shape():
    builder = PaddleToONNXGraphBuilder()
...
def test_paddle_builder_extract_attr():
    builder = PaddleToONNXGraphBuilder()
...
def test_paddle_builder_broadcasting():
    builder = PaddleToONNXGraphBuilder()
    assert builder.resolve_broadcasting((1, 2, 3), (2, 3)) == (1, 2, 3)


def test_paddle_builder_flatten_lod():
    builder = PaddleToONNXGraphBuilder()
...
def test_paddle_builder_make_node():
    builder = PaddleToONNXGraphBuilder()
...
def test_paddle_builder_make_node_optional():
    builder = PaddleToONNXGraphBuilder()
    outs = builder.make_node_optional_inputs("Concat", ["in", None], {}, "concat")
    assert builder.graph.nodes[-1].inputs == ["in", ""]


def test_paddle_builder_replace():
    builder = PaddleToONNXGraphBuilder()
