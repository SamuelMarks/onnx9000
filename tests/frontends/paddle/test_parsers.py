...
def test_paddlegraph_topological_sort():
    n1 = PaddleNode("feed", "feed", outputs={"Out": ["n1_out"]})
    n2 = PaddleNode(
        "relu", "relu", inputs={"X": ["n1_out"]}, outputs={"Out": ["n2_out"]}
    )
...
def test_paddlegraph_extract_inputs_outputs():
    ...
    b = PaddleBlock(
        0, -1, {"in1": v1, "weight1": v2, "feed": v3, "out1": v4, "fetch": v5}
    )
    graph = PaddleGraph([b])
...
def test_paddle_protobuf_parser_var_desc():
    data = b"\n\x02v1\x18\x01"
    parser = PaddleProtobufParser(data)
...
def test_paddle_protobuf_parser_var_desc_deep():
...
def test_paddle_protobuf_parser_parse_block_and_program():
...
def test_paddle_protobuf_parser_attr_types():
    import struct
...
def test_paddle_load_model_feed_fetch():
    op1_sub = b"\n\x03Out\x12\x03in1"
...
def test_log_unsupported_paddle_node(caplog):
    n = PaddleNode("n1", "some_op")
