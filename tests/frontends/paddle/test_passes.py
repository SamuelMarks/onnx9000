...
from onnx9000.frontends.paddle.passes import paddle_optimize_graph
...
def test_paddle_optimize_graph_identity_dropout_dce():
    g = _create_graph()
    n1 = Node(
        op_type="Identity", inputs=["in1"], outputs=["out1"], name="n1", attributes={}
    )
    n2 = Node(
        op_type="Dropout", inputs=["out1"], outputs=["out2"], name="n2", attributes={}
    )
    n3 = Node(
