import pytest
import time
from onnx9000.frontends.paddle.api import convert_paddle_to_onnx
from onnx9000.frontends.paddle.parsers import (
    PaddleNode,
    PaddleVar,
    PaddleBlock,
    PaddleGraph,
...
def test_integration_paddle_linear_regression():
    from onnx9000.frontends.paddle.parsers import PaddleGraph, PaddleNode, PaddleBlock
    from onnx9000.frontends.paddle.api import _convert_paddle_graph
    ...
    ops = [
        ...
        PaddleNode(
            "rm", "reduce_mean", inputs={"X": ["sqr1"]}, outputs={"Out": ["loss"]}
        ),
...
def test_integration_paddle_resnet_mock():
    from onnx9000.frontends.paddle.api import _convert_paddle_graph
...
def test_integration_paddle_ocr_mock():
    from onnx9000.frontends.paddle.api import _convert_paddle_graph
    ...
    ops = [
        ...
        PaddleNode(
            "lstm", "lstm", inputs={"Input": ["relu1"]}, outputs={"Out": ["lstm1"]}
        ),
        PaddleNode(
            "db", "sigmoid", inputs={"X": ["lstm1"]}, outputs={"Out": ["prob_map"]}
        ),
    ]
...
def test_paddle_dynamic_shapes_and_lod():
    from onnx9000.frontends.paddle.api import _convert_paddle_graph

    ops = [
        PaddleNode("n", "feed", outputs={"Out": ["in"]}),
        PaddleNode(
            "lod", "lod_array_length", inputs={"X": ["in"]}, outputs={"Out": ["len"]}
        ),
        PaddleNode(
            "rnn", "rnn", inputs={"Input": ["in"]}, outputs={"Out": ["seq_out"]}
        ),
    ]
    pg = PaddleGraph([PaddleBlock(0, -1, {}, ops)])
    graph = _convert_paddle_graph(pg)
...
def test_paddle_security_audit_parsing():
