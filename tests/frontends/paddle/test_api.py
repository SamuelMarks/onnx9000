...
def test_convert_paddle_to_onnx_empty():
...
def test_convert_paddle_to_onnx_fallback(caplog):
    op = b"\x1a\x07unknown"
...
def test_convert_paddle_to_onnx_basic_graph():
    op1 = b"\x1a\x04feed\n\x0b\n\x03Out\x12\x04in_1"
    op2 = b"\x1a\x04relu\n\x0c\n\x01X\x12\x04in_1\x12\x0b\n\x03Out\x12\x04out1"
    block_data = (
        b'\x08\x00\x10\x00"' + bytes([len(op1)]) + op1 + b'"' + bytes([len(op2)]) + op2
    )
    program_data = b"\n" + bytes([len(block_data)]) + block_data
...
def test_convert_paddle_to_onnx_else_branch():
    op = b"\x1a\x07unknown"
    block_data = b'\x08\x00\x10\x00"' + bytes([len(op)]) + op
    program_data = b"\n" + bytes([len(block_data)]) + block_data
    ...
    def mock_make_node(self, op_type, inputs, attributes, name_prefix, outputs=None):
        out = original_make_node(
            self, op_type, inputs, attributes, name_prefix, outputs
        )
        self.graph.tensors.clear()
        return out

    with patch(
        "onnx9000.frontends.paddle.builder.PaddleToONNXGraphBuilder.make_node",
        new=mock_make_node,
    ):
        g = convert_paddle_to_onnx(program_data)
