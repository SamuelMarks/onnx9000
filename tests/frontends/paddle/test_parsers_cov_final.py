...
def test_paddle_graph_empty_blocks():
    g = PaddleGraph()
    assert g.get_main_block().idx == 0
    assert g.topological_sort() == []


def test_paddle_parsers_skip_field():
...
def test_paddle_parsers_op_desc_extras():
...
def test_paddle_parsers_var_desc():
    name = b"\n\x01X"
...
def test_paddle_parsers_block_desc():
    idx = b"\x08\x01"
...
def test_paddle_parsers_program_desc():
...
def test_paddle_parsers_framework():
    block_data = b"\x08\x01\x10\x00"
    block_msg = b"\n" + bytes([len(block_data)]) + block_data
...
def test_paddle_parsers_framework_loop():
    prog_msg = b"\n\x00"
