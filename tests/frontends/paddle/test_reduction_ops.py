...
def test_paddle_reduce_ops():
    builder = PaddleToONNXGraphBuilder()
    n = PaddleNode(
        "n", "reduce_sum", inputs={"X": ["a"]}, attrs={"dim": [1], "keep_dim": True}
    )
...
def test_paddle_arg_ops():
    builder = PaddleToONNXGraphBuilder()
    n = PaddleNode(
        "n", "arg_max", inputs={"X": ["a"]}, attrs={"axis": 1, "keepdims": True}
    )
    outs = REDUCTION_OPS_MAPPING["arg_max"](builder, n)
    ...
    outs = REDUCTION_OPS_MAPPING["arg_min"](builder, n2)
...
def test_paddle_cumsum():
    ...
    outs = REDUCTION_OPS_MAPPING["cumsum"](builder, n)
