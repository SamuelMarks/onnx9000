...
def test_paddle_tensor_ops_reshape_transpose_flatten():
    builder = PaddleToONNXGraphBuilder()
    n = PaddleNode("n", "reshape", inputs={"X": ["a"], "Shape": ["s"]})
    outs = TENSOR_OPS_MAPPING["reshape"](builder, n)
    ...
    outs = TENSOR_OPS_MAPPING["transpose2"](builder, n)
...
def test_paddle_tensor_ops_squeeze_unsqueeze():
    ...
    outs = TENSOR_OPS_MAPPING["unsqueeze"](builder, n)
    assert builder.graph.nodes[-1].op_type == "Unsqueeze"


def test_paddle_tensor_ops_concat_stack():
    builder = PaddleToONNXGraphBuilder()
...
def test_paddle_tensor_ops_slice_gather_scatter():
    builder = PaddleToONNXGraphBuilder()
    n = PaddleNode("n", "slice", inputs={"X": ["a"]})
    outs = TENSOR_OPS_MAPPING["slice"](builder, n)
    assert (
...
def test_paddle_tensor_ops_slice_input_fb():
    from onnx9000.frontends.paddle.builder import PaddleToONNXGraphBuilder
    from onnx9000.frontends.paddle.parsers import PaddleNode
    from onnx9000.frontends.paddle.tensor_ops import TENSOR_OPS_MAPPING
