...
import subprocess
from onnx9000.frontends.jit.compiler import (
    _get_compiler,
    compile_cpp,
    compile_wasm,
    load_module,
...
def test_get_compiler(monkeypatch):
    ...
    with patch("sys.platform", "linux"):
        with patch(
            "shutil.which", side_effect=lambda x: True if x == "clang++" else False
        ):
            assert _get_compiler() == "clang++"
    with patch("sys.platform", "linux"):
        with patch("shutil.which", side_effect=lambda x: True if x == "g++" else False):
            assert _get_compiler() == "g++"
    with patch("sys.platform", "linux"):
        with patch("shutil.which", side_effect=lambda x: True if x == "c++" else False):
...
@patch("onnx9000.frontends.jit.compiler.hash_graph", return_value="test_hash")
@patch("onnx9000.frontends.jit.compiler.Generator")
@patch("onnx9000.frontends.jit.compiler.Environment")
@patch("builtins.open")
@patch("subprocess.run")
def test_compile_cpp(
    mock_run, mock_open, mock_env, mock_gen, mock_hash, monkeypatch, tmp_path
):
    ...
    with (
        ...
        patch("onnx9000.frontends.jit.compiler._get_compiler", return_value="clang++"),
    ):
...
@patch("onnx9000.frontends.jit.compiler.hash_graph", return_value="test_wasm_hash")
@patch("onnx9000.frontends.jit.compiler.Generator")
@patch("onnx9000.frontends.jit.compiler.Environment")
@patch("builtins.open")
@patch("subprocess.run")
...
def test_load_module(mock_mod_from_spec, mock_spec_from_file):
    mock_spec = MagicMock()
