...
def test_control_flow_ops_noop():
    builder = TFToONNXGraphBuilder()
    for op in ["Enter", "Exit", "Merge", "Switch", "NextIteration", "LoopCond"]:
        outs = CONTROL_FLOW_OPS_MAPPING[op](
            builder, TFNode(f"n_{op}", op, inputs=["a"])
        )
        assert builder.graph.nodes[-1].op_type == f"Custom_TF{op}"


def test_control_flow_ops_if_while():
    builder = TFToONNXGraphBuilder()
    outs = CONTROL_FLOW_OPS_MAPPING["If"](
        builder,
        TFNode(
            "n_if",
            "If",
    ...
    outs = CONTROL_FLOW_OPS_MAPPING["StatelessIf"](
        builder,
        TFNode(
            "n_if",
            "If",
    ...
    assert builder.graph.nodes[-1].op_type == "If"
    outs = CONTROL_FLOW_OPS_MAPPING["While"](
        builder,
        TFNode(
            "n_while",
            "While",
    ...
    outs = CONTROL_FLOW_OPS_MAPPING["StatelessWhile"](
        builder,
        TFNode(
            "n_while",
            "While",
...
def test_control_flow_ops_tensor_array():
    builder = TFToONNXGraphBuilder()
    ...
    outs = CONTROL_FLOW_OPS_MAPPING["TensorArrayWriteV3"](
        builder,
        TFNode(
            "n_write", "TensorArrayWriteV3", inputs=["handle", "index", "value", "flow"]
        ),
    )
...
def test_control_flow_ops_rnn():
    ...
    outs = CONTROL_FLOW_OPS_MAPPING["GRUBlockCell"](
        builder, TFNode("n_gru", "GRUBlockCell", inputs=["a"])
    )
