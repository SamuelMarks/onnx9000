...
def test_tfgraph_topological_sort():
...
def test_tfgraph_resolve_duplicate_names():
    graph = TFGraph(
        [TFNode("n1", "Placeholder"), TFNode("n1", "Relu"), TFNode("n1", "MatMul")]
    )
    graph.resolve_duplicate_names()
    assert [n.name for n in graph.nodes] == ["n1", "n1_1", "n1_2"]


def test_tfgraph_extract_inputs_outputs():
...
def test_tfgraph_extract_subgraph():
    n1 = TFNode("n1", "Placeholder")
    n2 = TFNode("n2", "Relu", inputs=["n1:0"])
    ...
    assert set(n.name for n in sub.nodes) == {"n1", "n2", "n3"}


def test_protobuf_parser_varint():
...
def test_protobuf_parser_bytes_string():
...
def test_protobuf_parser_node_def():
...
def test_protobuf_parser_graph_def():
...
def test_protobuf_parser_skip_fields():
    data = b"\x00\x08"
...
def test_protobuf_incomplete_varint():
    parser = ProtobufParser(b"\x80")
    assert parser.read_varint() == 0


def test_protobuf_parser_node_def_unknown_field():
    data = b"*\x03unk"
    parser = ProtobufParser(data)
    node = parser.parse_node_def(len(data))
    assert node.name == ""
...
def test_protobuf_parser_graph_def_unknown_field():
    data = b"*\x03unk"
    parser = ProtobufParser(data)
    graph = parser.parse_graph_def()
...
def test_log_unsupported_node(caplog):
    node = TFNode("n", "op")
