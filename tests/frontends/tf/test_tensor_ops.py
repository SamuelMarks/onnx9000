...
def test_tensor_ops_mapping_simple():
...
def test_tensor_ops_reshape_squeeze_expand():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["Reshape"](
        builder, TFNode("n", "Reshape", inputs=["a", "b"])
    )
    assert builder.graph.nodes[-1].op_type == "Reshape"
    outs = TENSOR_OPS_MAPPING["Squeeze"](builder, TFNode("n", "Squeeze", inputs=["a"]))
    assert builder.graph.nodes[-1].op_type == "Squeeze"
    assert len(builder.graph.nodes[-1].inputs) == 1
    outs = TENSOR_OPS_MAPPING["Squeeze"](
        builder, TFNode("n", "Squeeze", inputs=["a"], attr={"squeeze_dims": [1, 2]})
    )
    assert builder.graph.nodes[-1].op_type == "Squeeze"
    assert len(builder.graph.nodes[-1].inputs) == 2
    outs = TENSOR_OPS_MAPPING["ExpandDims"](
        builder, TFNode("n", "ExpandDims", inputs=["a", "dim"])
    )
    assert builder.graph.nodes[-1].op_type == "Unsqueeze"


def test_tensor_ops_transpose():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["Transpose"](
        builder, TFNode("n", "Transpose", inputs=["a", "perm"])
    )
    assert builder.graph.nodes[-1].op_type == "Transpose"
    outs = TENSOR_OPS_MAPPING["ConjugateTranspose"](
...
def test_tensor_ops_concat_pack_unpack():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["Concat"](
        builder, TFNode("n", "Concat", inputs=["dim", "a", "b"])
    )
...
def test_tensor_ops_split_slice_tile():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["Split"](
        builder, TFNode("n", "Split", inputs=["dim", "a"], attr={"num_split": 3})
    )
    assert builder.graph.nodes[-1].op_type == "Custom_TFSplit"
    outs = TENSOR_OPS_MAPPING["Slice"](
        builder, TFNode("n", "Slice", inputs=["a", "b", "c"])
    )
    assert builder.graph.nodes[-1].op_type == "Slice"
    outs = TENSOR_OPS_MAPPING["Tile"](builder, TFNode("n", "Tile", inputs=["a", "b"]))
    assert builder.graph.nodes[-1].op_type == "Tile"


def test_tensor_ops_pad():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["Pad"](builder, TFNode("n", "Pad", inputs=["a", "b"]))
    assert builder.graph.nodes[-1].op_type == "Pad"
    assert builder.graph.nodes[-1].attributes["mode"] == "constant"
    outs = TENSOR_OPS_MAPPING["PadV2"](
        builder, TFNode("n", "PadV2", inputs=["a", "b", "c"])
    )
    assert builder.graph.nodes[-1].op_type == "Pad"
    assert builder.graph.nodes[-1].attributes["mode"] == "constant"
    outs = TENSOR_OPS_MAPPING["MirrorPad"](
...
def test_tensor_ops_gather_scatter():
    builder = TFToONNXGraphBuilder()
    for op, onnx_op in [
        ("Gather", "Gather"),
        ("GatherNd", "GatherND"),
        ...
        outs = TENSOR_OPS_MAPPING[op](builder, TFNode(f"n_{op}", op, inputs=["a", "b"]))
        assert builder.graph.nodes[-1].op_type == onnx_op
    outs = TENSOR_OPS_MAPPING["TensorScatterAdd"](
...
def test_tensor_ops_space_depth():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["SpaceToBatchND"](
        builder, TFNode("n", "SpaceToBatchND", inputs=["a", "b", "c"])
    )
    assert builder.graph.nodes[-1].op_type == "SpaceToDepth"
    outs = TENSOR_OPS_MAPPING["BatchToSpaceND"](
        builder, TFNode("n", "BatchToSpaceND", inputs=["a", "b", "c"])
    )
    assert builder.graph.nodes[-1].op_type == "DepthToSpace"


def test_tensor_ops_reverse_roll_diag():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["Reverse"](
        builder, TFNode("n", "Reverse", inputs=["a", "b"])
    )
    assert builder.graph.nodes[-1].op_type == "ReverseSequence"
    outs = TENSOR_OPS_MAPPING["Roll"](
        builder, TFNode("n", "Roll", inputs=["a", "b", "c"])
    )
    assert builder.graph.nodes[-1].op_type == "Custom_Roll"
    outs = TENSOR_OPS_MAPPING["MatrixDiag"](
        builder, TFNode("n", "MatrixDiag", inputs=["a"])
    )
    assert builder.graph.nodes[-1].op_type == "Custom_MatrixDiag"


def test_tensor_ops_cast_shape_size():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["Cast"](
        builder, TFNode("n", "Cast", inputs=["a"], attr={"DstT": 3})
    )
    ...
    outs = TENSOR_OPS_MAPPING["ShapeN"](
        builder, TFNode("n", "ShapeN", inputs=["a", "b"])
    )
    assert len(outs) == 2
    assert builder.graph.nodes[-1].op_type == "Shape"
...
def test_tensor_ops_like_fill_broadcast_where():
    builder = TFToONNXGraphBuilder()
    outs = TENSOR_OPS_MAPPING["ZerosLike"](
        builder, TFNode("n", "ZerosLike", inputs=["a"])
    )
    assert builder.graph.nodes[-1].op_type == "ConstantOfShape"
    assert builder.graph.nodes[-1].attributes["value"] == 0.0
    outs = TENSOR_OPS_MAPPING["OnesLike"](
        builder, TFNode("n", "OnesLike", inputs=["a"])
    )
    assert builder.graph.nodes[-1].op_type == "ConstantOfShape"
    assert builder.graph.nodes[-1].attributes["value"] == 1.0
    outs = TENSOR_OPS_MAPPING["Fill"](builder, TFNode("n", "Fill", inputs=["a", "b"]))
    assert builder.graph.nodes[-1].op_type == "Custom_Fill"
    outs = TENSOR_OPS_MAPPING["BroadcastTo"](
        builder, TFNode("n", "BroadcastTo", inputs=["a", "b"])
    )
    assert builder.graph.nodes[-1].op_type == "Expand"
    outs = TENSOR_OPS_MAPPING["Where"](builder, TFNode("n", "Where", inputs=["a"]))
    assert builder.graph.nodes[-1].op_type == "Where"
    outs = TENSOR_OPS_MAPPING["Select"](
        builder, TFNode("n", "Select", inputs=["a", "b", "c"])
    )
    assert builder.graph.nodes[-1].op_type == "Where"
    outs = TENSOR_OPS_MAPPING["SelectV2"](
        builder, TFNode("n", "SelectV2", inputs=["a", "b", "c"])
    )
    assert builder.graph.nodes[-1].op_type == "Where"
