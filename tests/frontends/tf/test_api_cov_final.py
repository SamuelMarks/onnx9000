...
from onnx9000.frontends.tf.api import convert_tf_to_onnx


def test_convert_tf_to_onnx_else_branch():
    op = b"\x1a\x07unknown"
    node_data = b"\n" + bytes([len(op)]) + op
    ...
    def mock_make_node(self, op_type, inputs, attributes, name_prefix, num_outputs=1):
        out = original_make_node(
            self, op_type, inputs, attributes, name_prefix, num_outputs
        )
        self.graph.tensors.clear()
        return out

    with patch(
        "onnx9000.frontends.tf.builder.TFToONNXGraphBuilder.make_node",
        new=mock_make_node,
    ):
