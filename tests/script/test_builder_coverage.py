...
def test_rename_all_initializers():
...
def test_infer_shapes():
    ...
    node2 = Node(
        "Constant", [], ["o2"], {"value": np.array([5.0], dtype=np.float32)}, name="n2"
    )
...
def test_control_flow():
    ...
    mgr_loop = gb.Loop(max_trip, cond, 1)


def test_to_onnx_extras():
...
def test_extract_subgraph_with_graph():
