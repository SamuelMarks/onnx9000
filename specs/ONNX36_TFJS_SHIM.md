...
- [ ] 001. Implement global `tf` namespace object.
- [ ] 002. Implement `tf.setBackend(backendName)` interceptor.
- [ ] 003. Map `tf.setBackend('webgl')` natively to `onnx9000` WebGPU (with fallback to WASM).
- [ ] 004. Map `tf.setBackend('wasm')` natively to `onnx9000` WASM SIMD.
- [ ] 005. Map `tf.setBackend('cpu')` natively to `onnx9000` pure JS/WASM scalar fallback.
- [ ] 006. Implement `tf.getBackend()` returning the active simulated backend.
- [ ] 007. Implement `tf.ready()` resolving a Promise when `onnx9000` WASM/WebGPU is initialized.
- [ ] 008. Implement `tf.env()` configuration stub.
- [ ] 009. Implement `tf.enableProdMode()` (suppressing ONNX compilation warnings).
- [ ] 010. Implement `tf.enableDebugMode()` (enabling ONNX verbose trace logging).
- [ ] 011. Implement `tf.memory()` returning an object matching `{ numBytes, numTensors, numDataBuffers }`.
- [ ] 012. Map `tf.memory()` metrics dynamically to the `onnx9000` internal WebGPU buffer tracker.
- [ ] 013. Implement `tf.profile(f)` wrapping `onnx9000.profile()`.
- [ ] 014. Implement `tf.time(f)` executing the ONNX JIT and measuring wall clock time.
- [ ] 015. Implement `tf.disposeVariables()` clearing the `onnx9000` global context.
- [ ] 016. Emulate TF.js global registry to track active tensors for memory leak detection.
- [ ] 017. Support standard `tf.version.tfjs` matching the latest shimmed version (e.g., `4.10.0`).
- [ ] 018. Support standard `tf.version.core` strings.

### Phase 2: Tensor Creation & Lifecycle Management (`tf.tensor`)
- [ ] 019. Implement `tf.tensor(values, shape, dtype)` mapped to `onnx9000.Tensor`.
- [ ] 020. Implement `tf.tensor1d(values, dtype)`.
- [ ] 021. Implement `tf.tensor2d(values, shape, dtype)`.
- [ ] 022. Implement `tf.tensor3d(values, shape, dtype)`.
- [ ] 023. Implement `tf.tensor4d(values, shape, dtype)`.
- [ ] 024. Implement `tf.tensor5d(values, shape, dtype)`.
- [ ] 025. Implement `tf.tensor6d(values, shape, dtype)`.
- [ ] 026. Implement `tf.scalar(value, dtype)`.
- [ ] 027. Implement `tf.buffer(shape, dtype, values)` mapping to mutable JS arrays.
- [ ] 028. Implement `tf.clone(x)`.
- [ ] 029. Implement `tf.complex(real, imag)` (mapping to `Float32` pairs if ONNX lacks native complex support).
- [ ] 030. Implement `tf.diag(x)`.
- [ ] 031. Implement `tf.eye(numRows, numColumns, batchShape, dtype)`.
- [ ] 032. Implement `tf.fill(shape, value, dtype)`.
- [ ] 033. Implement `tf.imag(complexTensor)`.
- [ ] 034. Implement `tf.linspace(start, stop, num)`.
- [ ] 035. Implement `tf.ones(shape, dtype)`.
- [ ] 036. Implement `tf.onesLike(x)`.
- [ ] 037. Implement `tf.print(x, verbose)` wrapping `console.log(tensor.numpy())`.
- [ ] 038. Implement `tf.range(start, stop, step, dtype)`.
- [ ] 039. Implement `tf.real(complexTensor)`.
- [ ] 040. Implement `tf.zeros(shape, dtype)`.
- [ ] 041. Implement `tf.zerosLike(x)`.

### Phase 3: The `tf.tidy` Memory Engine
- [ ] 042. Implement `tf.tidy(nameOrFn, fn)` scoping block.
- [ ] 043. Track all `onnx9000.Tensor` allocations created inside the `tf.tidy` closure.
- [ ] 044. Prevent intermediate `onnx9000` WebGPU Buffers from leaking out of the closure.
- [ ] 045. Allow the returned `Tensor` (or array of Tensors) to escape the `tidy` block safely.
- [ ] 046. Implement `tf.keep(x)` to exempt a tensor from `tf.tidy` cleanup.
- [ ] 047. Implement `tf.dispose(tensors)` translating to `onnx9000.Tensor.dispose()`.
- [ ] 048. Handle deep arrays and dictionaries of tensors correctly inside `tf.dispose`.
- [ ] 049. Map tensor disposal directly to WebGPU `buffer.destroy()` to ensure VRAM is released immediately.
- [ ] 050. Gracefully catch and ignore double-dispose calls.

### Phase 4: Basic Math & Elementwise Operations
- [ ] 051. Implement `tf.add(a, b)` -> ONNX `Add`.
- [ ] 052. Implement `tf.sub(a, b)` -> ONNX `Sub`.
- [ ] 053. Implement `tf.mul(a, b)` -> ONNX `Mul`.
- [ ] 054. Implement `tf.div(a, b)` -> ONNX `Div`.
- [ ] 055. Implement `tf.divNoNan(a, b)`.
- [ ] 056. Implement `tf.floorDiv(a, b)` -> ONNX `Div` + `Floor`.
- [ ] 057. Implement `tf.maximum(a, b)` -> ONNX `Max`.
- [ ] 058. Implement `tf.minimum(a, b)` -> ONNX `Min`.
- [ ] 059. Implement `tf.mod(a, b)` -> ONNX `Mod`.
- [ ] 060. Implement `tf.pow(base, exp)` -> ONNX `Pow`.
- [ ] 061. Implement `tf.squaredDifference(a, b)` -> ONNX `Sub` + `Pow(2)`.
- [ ] 062. Implement `tf.addN(tensors)` -> ONNX chained `Add`.
- [ ] 063. Implement `tf.abs(x)` -> ONNX `Abs`.
- [ ] 064. Implement `tf.acos(x)` -> ONNX `Acos`.
- [ ] 065. Implement `tf.acosh(x)` -> ONNX `Acosh`.
- [ ] 066. Implement `tf.asin(x)` -> ONNX `Asin`.
- [ ] 067. Implement `tf.asinh(x)` -> ONNX `Asinh`.
- [ ] 068. Implement `tf.atan(x)` -> ONNX `Atan`.
- [ ] 069. Implement `tf.atan2(a, b)` -> ONNX Custom/Math.
- [ ] 070. Implement `tf.atanh(x)` -> ONNX `Atanh`.
- [ ] 071. Implement `tf.ceil(x)` -> ONNX `Ceil`.
- [ ] 072. Implement `tf.cos(x)` -> ONNX `Cos`.
- [ ] 073. Implement `tf.cosh(x)` -> ONNX `Cosh`.
- [ ] 074. Implement `tf.erf(x)` -> ONNX `Erf`.
- [ ] 075. Implement `tf.exp(x)` -> ONNX `Exp`.
- [ ] 076. Implement `tf.expm1(x)` -> ONNX `Exp` + `Sub(1)`.
- [ ] 077. Implement `tf.floor(x)` -> ONNX `Floor`.
- [ ] 078. Implement `tf.isFinite(x)`.
- [ ] 079. Implement `tf.isInf(x)` -> ONNX `IsInf`.
- [ ] 080. Implement `tf.isNaN(x)` -> ONNX `IsNaN`.
- [ ] 081. Implement `tf.log(x)` -> ONNX `Log`.
- [ ] 082. Implement `tf.log1p(x)` -> ONNX `Add(1)` + `Log`.
- [ ] 083. Implement `tf.neg(x)` -> ONNX `Neg`.
- [ ] 084. Implement `tf.reciprocal(x)` -> ONNX `Reciprocal`.
- [ ] 085. Implement `tf.round(x)` -> ONNX `Round`.
- [ ] 086. Implement `tf.rsqrt(x)` -> ONNX `Sqrt` + `Reciprocal`.
- [ ] 087. Implement `tf.sign(x)` -> ONNX `Sign`.
- [ ] 088. Implement `tf.sin(x)` -> ONNX `Sin`.
- [ ] 089. Implement `tf.sinh(x)` -> ONNX `Sinh`.
- [ ] 090. Implement `tf.sqrt(x)` -> ONNX `Sqrt`.
- [ ] 091. Implement `tf.square(x)` -> ONNX `Pow(2)`.
- [ ] 092. Implement `tf.tan(x)` -> ONNX `Tan`.
- [ ] 093. Implement `tf.step(x, alpha)` -> ONNX `Where`.

### Phase 5: Matrix Algebra & Convolutions
- [ ] 094. Implement `tf.matMul(a, b, transposeA, transposeB)` -> ONNX `MatMul` (with transposition hooks).
- [ ] 095. Implement `tf.dot(a, b)`.
- [ ] 096. Implement `tf.norm(x, ord, axis, keepDims)`.
- [ ] 097. Implement `tf.outerProduct(v1, v2)`.
- [ ] 098. Implement `tf.conv1d(x, filter, stride, pad, dataFormat, dilation)`.
- [ ] 099. Translate TF.js `padding='same'` / `'valid'` explicitly to ONNX spatial paddings.
