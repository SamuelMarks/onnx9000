...
*   **Deep Memory Control:** By handling the parsing in Python/JS, `onnx9000` can inject custom weight layouts, apply AST-level fusions (e.g., packing W4A16), or dynamically partition the graph *before* handing it over to TensorRT, bypassing many of the strict limitations in NVIDIA's native ONNX parser.
...
- [ ] 001. Detect `libnvinfer.so` (Linux) dynamically via `ctypes.util.find_library`.
- [ ] 002. Detect `nvinfer.dll` (Windows) dynamically.
- [ ] 003. Extract TensorRT API version natively (e.g., 8.6, 10.0) from the shared library.
- [ ] 004. Establish FFI fallback policies if different TRT versions expose different function signatures.
- [ ] 005. Implement `ILogger` C-callback natively in Python to intercept TRT diagnostic messages.
- [ ] 006. Route TRT `ILogger` `kINFO`, `kWARNING`, `kERROR` events directly to standard Python/JS loggers.
- [ ] 007. Provide dynamic memory management across the FFI boundary to prevent C-side segfaults.
- [ ] 008. Implement a global registry of active TRT pointers to ensure explicit destruction (`__del__` / `FinalizationRegistry`).
- [ ] 009. Implement `createInferBuilder_INTERNAL` FFI binding.
- [ ] 010. Support explicitly destroying builders via `destroy()` bindings.
- [ ] 011. Implement `node-ffi-napi` bindings for equivalent Node.js server execution.
- [ ] 012. Map C `enum` values specifically to Python `IntEnum` structures for precise TRT configurations.
- [ ] 013. Catch C-level hardware errors and surface them as readable Python `RuntimeError` exceptions.
- [ ] 014. Support detecting `libnvinfer_plugin.so` automatically for custom operator extensions.
- [ ] 015. Expose `cudaGetDeviceProperties` via FFI to query GPU compute capability automatically.

### Phase 2: TensorRT Builder & Network Definition
- [ ] 016. Initialize `IBuilder` explicitly.
- [ ] 017. Initialize `INetworkDefinition` (Explicit Batch flag required for ONNX).
- [ ] 018. Initialize `IBuilderConfig`.
- [ ] 019. Implement `addInput(name, type, dims)` mapping ONNX Inputs to TRT.
- [ ] 020. Implement `markOutput(tensor)` mapping ONNX Outputs to TRT.
- [ ] 021. Translate ONNX `FLOAT32` to `DataType::kFLOAT`.
- [ ] 022. Translate ONNX `FLOAT16` to `DataType::kHALF`.
- [ ] 023. Translate ONNX `INT32` to `DataType::kINT32`.
- [ ] 024. Translate ONNX `INT8` to `DataType::kINT8`.
- [ ] 025. Translate ONNX `BOOL` to `DataType::kBOOL`.
- [ ] 026. Emulate `INT64` support by explicitly injecting `Cast` nodes (TRT historically lacks strict int64 support).
- [ ] 027. Map ONNX dimensional arrays `[1, 3, 224, 224]` to TRT `Dims` structs in C memory.
- [ ] 028. Map ONNX dynamic axes (`-1`) to TRT dynamic dimensions correctly.
- [ ] 029. Track the mapping between ONNX `NodeArg` strings and TRT `ITensor` pointers dynamically in a dictionary.
- [ ] 030. Configure explicit memory pools (`config->setMemoryPoolLimit(MemoryPoolType::kWORKSPACE, size)`).

### Phase 3: Constant & Weight Translation
- [ ] 031. Implement `addConstant(dims, weights)` mapping ONNX Initializers to TRT.
- [ ] 032. Pass Python NumPy pointers directly into `Weights` structs securely (`data_ptr`).
- [ ] 033. Ensure weights outlive the Engine building phase by pinning Python arrays in memory.
- [ ] 034. Extract scalar ONNX constants correctly to 0-D TRT constants.
- [ ] 035. Embed large external weights (`.bin`) seamlessly by mapping their raw bytes via `mmap` into the TRT `Weights` struct.
- [ ] 036. Handle Endianness requirements natively before passing buffers to `addConstant`.
- [ ] 037. Bypass zero-sized constants natively.
- [ ] 038. Collapse constant chains explicitly in `onnx9000.modifier` before passing them to TRT to save build time.
- [ ] 039. Emit specific warnings if a Constant exceeds TRT dimension size maximums.
- [ ] 040. Prevent memory leaks by explicitly unpinning weight arrays once the `buildSerializedNetwork` call completes.

### Phase 4: Core Math & Matrix Operations (`IElementWiseLayer`)
- [ ] 041. Map ONNX `Add` to `addElementWise(a, b, ElementWiseOperation::kSUM)`.
- [ ] 042. Map ONNX `Sub` to `ElementWiseOperation::kSUB`.
- [ ] 043. Map ONNX `Mul` to `ElementWiseOperation::kPROD`.
- [ ] 044. Map ONNX `Div` to `ElementWiseOperation::kDIV`.
- [ ] 045. Map ONNX `Max` to `ElementWiseOperation::kMAX`.
- [ ] 046. Map ONNX `Min` to `ElementWiseOperation::kMIN`.
- [ ] 047. Map ONNX `Pow` to `ElementWiseOperation::kPOW`.
- [ ] 048. Map ONNX `Equal` to `ElementWiseOperation::kEQUAL`.
- [ ] 049. Map ONNX `Less` to `ElementWiseOperation::kLESS`.
- [ ] 050. Map ONNX `Greater` to `ElementWiseOperation::kGREATER`.
- [ ] 051. Handle ONNX implicit broadcasting manually by injecting `IShuffleLayer` or TRT broadacast flags if required.
- [ ] 052. Map ONNX `MatMul` to `addMatrixMultiply(a, opA, b, opB)`.
- [ ] 053. Handle `MatrixOperation::kTRANSPOSE` natively based on ONNX transpose structures.
- [ ] 054. Map ONNX `Gemm` to `addFullyConnected()` if applicable, or decomposed `MatrixMultiply` + `ElementWise`.
- [ ] 055. Validate multi-dimensional batched MatMul dimensions explicitly.

### Phase 5: Convolution & Pooling Layers
- [ ] 056. Map ONNX `Conv` (2D) to `addConvolutionNd(input, numOutputs, kernelSize, weights, bias)`.
- [ ] 057. Map ONNX `Conv` (3D) to `addConvolutionNd`.
- [ ] 058. Map ONNX `Conv` (1D) to `addConvolutionNd` (Emulating via 2D if required by older TRT versions).
- [ ] 059. Set TRT `Stride` explicitly from ONNX attributes.
- [ ] 060. Set TRT `Padding` (pre/post) explicitly.
- [ ] 061. Set TRT `Dilation` explicitly.
- [ ] 062. Set TRT `NumGroups` explicitly for Depthwise Convolution mapping.
- [ ] 063. Map ONNX `ConvTranspose` to `addDeconvolutionNd()`.
- [ ] 064. Map ONNX `MaxPool` to `addPoolingNd(input, PoolingType::kMAX, windowSize)`.
- [ ] 065. Map ONNX `AveragePool` to `PoolingType::kAVERAGE`.
- [ ] 066. Support `AveragePool` `count_include_pad` attribute mapping.
- [ ] 067. Set pooling `Stride`, `Padding`, and `BlendFactor` dynamically.
- [ ] 068. Map ONNX `GlobalAveragePool` to `addPoolingNd` spanning the entire spatial dimension.
- [ ] 069. Map ONNX `GlobalMaxPool` to `addPoolingNd`.
- [ ] 070. Handle asymmetric padding safely via TRT `setPaddingMode` or explicit `IPaddingLayer`.

### Phase 6: Activation, Normalization, & Unary Ops
- [ ] 071. Map ONNX `Relu` to `addActivation(input, ActivationType::kRELU)`.
- [ ] 072. Map ONNX `Sigmoid` to `ActivationType::kSIGMOID`.
- [ ] 073. Map ONNX `Tanh` to `ActivationType::kTANH`.
- [ ] 074. Map ONNX `LeakyRelu` to `ActivationType::kLEAKY_RELU` (setting `alpha`).
- [ ] 075. Map ONNX `Elu` to `ActivationType::kELU`.
- [ ] 076. Map ONNX `Selu` to `ActivationType::kSELU`.
- [ ] 077. Map ONNX `Softplus` to `ActivationType::kSOFTPLUS`.
- [ ] 078. Map ONNX `Clip` to `ActivationType::kCLIP` (setting `alpha` and `beta`).
- [ ] 079. Map ONNX `HardSigmoid` to `ActivationType::kHARD_SIGMOID`.
- [ ] 080. Map ONNX `Softmax` to `addSoftMax(input)`.
- [ ] 081. Map Softmax `axis` cleanly.
- [ ] 082. Map ONNX `BatchNormalization` to `addScale(input, ScaleMode::kCHANNEL, shift, scale, power)`.
- [ ] 083. Pre-calculate Batch Norm scale/shift values offline in Python before passing to TRT `ScaleLayer`.
- [ ] 084. Map ONNX `InstanceNormalization` to `addScaleNd` or native TRT Plugin.
- [ ] 085. Map ONNX `LayerNormalization` to `addNormalization()` (TRT 10+).
- [ ] 086. Map Unary `Exp` to `addUnaryOperation(input, UnaryOperation::kEXP)`.
- [ ] 087. Map Unary `Log` to `UnaryOperation::kLOG`.
- [ ] 088. Map Unary `Sqrt` to `UnaryOperation::kSQRT`.
- [ ] 089. Map Unary `Abs` to `UnaryOperation::kABS`.
- [ ] 090. Map Unary `Neg` to `UnaryOperation::kNEG`.

### Phase 7: Dimension Manipulation & Routing
- [ ] 091. Map ONNX `Reshape` to `addShuffle(input)`.
- [ ] 092. Handle `Reshape` dynamic dimensions via `setReshapeDimensions`.
- [ ] 093. Map ONNX `Transpose` to `IShuffleLayer` via `setFirstTranspose(perm)`.
- [ ] 094. Map ONNX `Flatten` to `addShuffle` with flattened dims.
- [ ] 095. Map ONNX `Squeeze` to `addShuffle`.
- [ ] 096. Map ONNX `Unsqueeze` to `addShuffle`.
- [ ] 097. Map ONNX `Concat` to `addConcatenation(tensors, numTensors)`.
- [ ] 098. Handle `Concat` axis parameter natively.
- [ ] 099. Map ONNX `Split` to `addSlice()` dynamically allocating individual output tensors.
...
- [ ] 207. Generate comprehensive Optimization Fusions natively in `onnx9000` (Level 3) *before* TRT to speed up TRT compilation times.
