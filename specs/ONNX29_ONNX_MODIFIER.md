...
- [ ] 104. Support exporting large models using the `external_data` ONNX format (zipping the `.onnx` and `.onnx_data` together in the browser).
- [ ] 105. Generate an "Edit Log" sidecar file (a JSON array recording every mutation made during the session) so edits can be replayed scripturally later.
- [ ] 106. Provide a "Copy to Clipboard" button that generates the exact Python `onnx.helper` code required to recreate the user's manual edits.
- [ ] 107. Generate a graph summary text file (Layer counts, Parameter counts, Total MACs).
- [ ] 108. Enable downloading the graph visualization as a high-resolution SVG or PNG file.
- [ ] 109. Support converting the visual graph layout into standard Graphviz `.dot` format.
...
- [ ] 114. Create a specialized UI form for editing `Resize` (Dropdowns for coordinate_transformation_mode, mode, nearest_mode).
- [ ] 115. Create a specialized UI form for editing `Squeeze` / `Unsqueeze` (Axes array editor).
- [ ] 116. Create a specialized UI form for editing `Cast` (Dropdown mapping integer enum values to human-readable strings like 'FLOAT', 'INT8').
- [ ] 117. Create a specialized UI form for editing `Constant` (showing the raw tensor value inline).
- [ ] 118. Create a specialized UI interface for viewing inside `If` subgraphs (Graph within a Graph).
- [ ] 119. Create a specialized UI interface for viewing inside `Loop` subgraphs.
...
- [ ] 124. Expose `onnx9000 change-batch <model.onnx> --size 8` via CLI.
...
- [ ] 129. Map Pyodide state changes back to the UI visualizer instantly.
...
- [ ] 134. Support complete keyboard navigation (tabbing through nodes, pressing enter to open properties).
...
- [ ] 139. Implement "Extract Weights": Converts all large Constants/Inputs into external Initializers automatically.
...
- [ ] 144. Validate the exported `.onnx` byte structure against the native Python `onnx-modifier` outputs.
...
- [ ] 149. Optimize memory: Release the original ArrayBuffer payload immediately after parsing the AST to free up JS Heap.
...
- [ ] 154. Provide "Export to CoreML" integration right from the modifier UI.
...
- [ ] 159. Support WebSocket connections for synchronizing node drags/moves between users.
...
- [ ] 164. Support visualizing 3D/4D tensors specific to vision models (e.g., showing a spatial heatmap of initializers if the shape matches standard convolution patterns).
...
- [ ] 169. Provide comprehensive tutorials: "Fixing Shape Mismatches", "Pruning a Model for Web Deploy".
- [ ] 170. Release v1.0 feature parity certification against the original Python `onnx-modifier`.
- [ ] 171. Add visual indication of node execution time bottlenecks (e.g., coloring nodes red if they take > 10ms during "Run Here" profiling).
- [ ] 172. Add visual indication of node memory footprint bottlenecks.
- [ ] 173. Support `onnx_data` sidecar file modification (updating external weights without pulling them into the main proto).
- [ ] 174. Validate WebGL context loss recovery.
- [ ] 175. Enable importing ONNX models dynamically via HuggingFace Hub repository URLs in the UI.
- [ ] 176. Implement specific validation for `If` graph structures to prevent UI freezing on recursion.
- [ ] 177. Provide layout caching so moving a node retains its position on reload.
- [ ] 178. Handle copy/pasting nodes within the graph (duplication).
- [ ] 179. Handle copy/pasting subgraphs entirely.
- [ ] 180. Validate edge routing algorithm efficiency on graphs > 10,000 edges.
- [ ] 181. Add support for creating subgraph abstractions visually (selecting nodes and grouping them).
- [ ] 182. Implement graph-level attribute viewing (for ONNX models that store metadata in attributes).
- [ ] 183. Manage the display of extremely long string constants efficiently in the properties panel.
- [ ] 184. Implement a strict "Read Only" mode for embedding the visualizer without editing capabilities.
- [ ] 185. Support parsing and visual rendering of ONNX sparse tensor formats.
- [ ] 186. Optimize the undo/redo stack memory footprint (saving deltas instead of full graph copies).
- [ ] 187. Ensure correct parsing of ONNX TensorProto `raw_data` formats (Float16 arrays, BFloat16 arrays).
- [ ] 188. Support importing JSON representation of the ONNX graph directly.
- [ ] 189. Add a feature to "De-duplicate" constants visually in the graph representation.
- [ ] 190. Provide a specific export configuration that forces Opsets to match WebNN compliance.
- [ ] 191. Create custom warning badges on nodes that are known to be slow or unsupported in WebGPU.
- [ ] 192. Integrate with browser File System Access API for seamless local save overwrites.
- [ ] 193. Render text representations of mathematical expressions for basic math nodes (`Add`, `Mul`).
- [ ] 194. Handle rendering of models with empty graphs gracefully.
- [ ] 195. Verify that `value_info` metadata is correctly updated when modifying internal graph shapes.
- [ ] 196. Implement an edge filtering mechanism (e.g., hiding control flow edges to reduce visual noise).
- [ ] 197. Support custom node coloring based on regular expressions (e.g., `Color all 'LayerNorm' nodes blue`).
- [ ] 198. Export the WebGL renderer as an independent generic DAG viewer component.
- [ ] 199. Publish the web asset to a standard Docker container for enterprise internal usage.
- [ ] 200. Execute load tests verifying the browser doesn't crash on standard 2GB LLM `.onnx` files.
- [ ] 201. Add support for visual node snapping to a grid.
- [ ] 202. Provide "Align Left/Right/Center" utilities for selected groups of nodes.
- [ ] 203. Create a feature to automatically format node names based on depth (e.g., `layer_1/conv`, `layer_2/conv`).
- [ ] 204. Validate `Cast` node conversions accurately represent bounds correctly.
- [ ] 205. Implement a fallback Canvas 2D renderer if WebGL initialization fails in the browser.
- [ ] 206. Configure explicit memory thresholds before warning the user that a save might fail.
- [ ] 207. Support viewing the raw protobuf JSON alongside the visual editor.
- [ ] 208. Add right-click -> "Extract to Python script" generating `onnx.helper.make_node` snippets.
- [ ] 209. Enable "Replace with Constant" macro (runs a node and bakes its result as a constant).
- [ ] 210. Implement edge hover preview of tensor shape sizes.
- [ ] 211. Provide visual cues for `INT8` quantized topologies.
- [ ] 212. Provide visual cues for `W4A16` packed weight topologies.
- [ ] 213. Add specific UI warnings for nodes using `double` (float64) precision.
- [ ] 214. Create an automated test checking `changeBatchSize` function across all common layer types.
...
- [ ] 218. Add support for creating new ONNX subgraphs entirely from scratch (starting with blank canvas).
- [ ] 219. Include pre-built snippets (e.g., dragging and dropping a standard ResBlock from a menu).
- [ ] 220. Support exporting the node statistics directly to CSV/Excel.
- [ ] 221. Implement deep linking (e.g., sharing a URL that automatically opens a model from a remote URL and centers on node `Conv_14`).
- [ ] 222. Ensure local IndexedDB cache is properly cleared on page reload to avoid stale state.
- [ ] 223. Support parsing and rendering `TrainingInfoProto` if present in the ONNX file.
- [ ] 224. Allow editing of `dim_param` string variables for dynamic axes.
- [ ] 225. Add a "Validate Opset" macro checking compatibility with opset 13-21.
- [ ] 226. Ensure the UI gracefully handles rapid sequential clicks (debouncing expensive layout recalculations).
- [ ] 227. Test the application across Chrome, Firefox, and Safari engines.
- [ ] 228. Provide a detailed "Help" modal mapping keyboard shortcuts.
- [ ] 229. Expose a global `window.onnxModifier` object for developer console hacking.
- [ ] 230. Validate output generation with `external_data` enabled when handling models >2GB.
- [ ] 231. Handle edge cases involving empty initializers (`raw_data` length 0).
- [ ] 232. Support `Float8E4M3FN` rendering specifically for newer ONNX opsets.
- [ ] 233. Optimize serialization sequence (minimizing GC pauses during download generation).
- [ ] 234. Include custom logic to edit ONNX `Sequence` type attributes.
- [ ] 235. Include custom logic to edit ONNX `Map` type attributes.
- [ ] 236. Allow explicit defining of `SparseTensorProto` constants.
- [ ] 237. Create UI component for tracking multiple sub-graphs within one overarching model.
- [ ] 238. Verify that multi-level nested `If` subgraphs render correctly without logical entanglement.
- [ ] 239. Enable "Save State" to store the current editing session locally without downloading.
- [ ] 240. Allow comparison of two separate ONNX files loaded simultaneously side-by-side.
- [ ] 241. Provide visual indicators mapping identical weights (detecting deduplication).
- [ ] 242. Build visual tool tracking gradient paths (if training nodes exist).
- [ ] 243. Set up memory profiling to alert users if their edits will cause a memory spike on inference.
- [ ] 244. Handle the "Drop ONNX file here" UX explicitly via standard browser events.
- [ ] 245. Validate file drop on all operating systems.
- [ ] 246. Establish automated testing of the `onnx9000 edit` CLI command via local playwright instances.
- [ ] 247. Track exact node deletion counts and log them in console for user reference.
- [ ] 248. Provide an option to "Auto-Fix" missing initializers by injecting dummy Zero arrays.
- [ ] 249. Display warning banners when the model uses deprecated Opset versions.
- [ ] 250. Finalize rigorous integration tests proving the editor works completely offline.
- [ ] 251. Handle `Optional` input edge rendering gracefully (using dashed lines).
- [ ] 252. Add a `Graph Level Attributes` property panel.
- [ ] 253. Render Graph Doc Strings using Markdown in the UI.
- [ ] 254. Support editing the graph `ir_version`.
- [ ] 255. Support visual editing of the `opset_import` list.
- [ ] 256. Handle adding multiple opset domains gracefully.
- [ ] 257. Verify correct parsing and UI representation of `FunctionProto` custom functions.
- [ ] 258. Support editing `FunctionProto` bodies recursively.
- [ ] 259. Manage complex naming collisions when duplicating nodes.
- [ ] 260. Render large constant tensors via paginated tables in the properties panel to prevent UI freezing.
- [ ] 261. Add "Export Node to JSON" feature.
- [ ] 262. Support copying a node's attributes to another node of the same type.
- [ ] 263. Establish a user-feedback collection mechanism inside the UI.
- [ ] 264. Support translating standard PyTorch exported graphs into cleaner visual representations (collapsing noise).
- [ ] 265. Allow pinning specific nodes to fixed coordinates on the canvas.
- [ ] 266. Enable drawing of custom text annotations directly onto the canvas.
- [ ] 267. Export annotations alongside the model in a metadata dictionary.
- [ ] 268. Detect and display warning for model inputs lacking defined shape properties.
- [ ] 269. Render a progress bar tracking the Sugiyama layout algorithm for large graphs.
- [ ] 270. Add an abort button for the layout algorithm if it takes too long.
- [ ] 271. Fallback to a fast grid layout if Sugiyama fails or times out.
- [ ] 272. Implement custom logic for editing `ScatterND` specific tensor updates.
...
- [ ] 276. Ensure proper cleanup of the WebGL context on component unmount.
- [ ] 277. Render graphs using standard WebGL Instanced Drawing for massive performance gains.
- [ ] 278. Add anti-aliasing configurations for the canvas viewer.
- [ ] 279. Support saving layout preferences to `localStorage`.
- [ ] 280. Add multi-language localization support for the Modifier UI.
- [ ] 281. Build the production asset pipeline via Vite or Webpack strictly.
- [ ] 282. Expose specific hooks for unit testing DOM interactions.
...
- [ ] 285. Confirm that models generated by `onnx-modifier` execute flawlessly in standard ONNX Runtime C++.
- [ ] 286. Handle ONNX proto version conflicts gracefully.
- [ ] 287. Implement a "Validate Graph" manual trigger button.
- [ ] 288. Publish full NPM package `@onnx9000/modifier`.
- [ ] 289. Add a strict mode that prevents generating non-standard ONNX architectures.
- [ ] 290. Provide a detailed summary of changes prompt before exporting.
- [ ] 291. Add visual node highlighting based on inference path tracking.
- [ ] 292. Add custom layout padding configurations.
- [ ] 293. Handle invalid JSON attribute inputs safely.
- [ ] 294. Enable custom shape inference logic hooking.
- [ ] 295. Set up continuous integration analyzing WebGL framerates.
- [ ] 296. Verify correct parsing of the `metadata_props` mapping.
- [ ] 297. Render `String` nodes with truncated inline text representations.
- [ ] 298. Validate complete disconnection behavior without crashing shape inference.
- [ ] 299. Create standard Github Issue forms explicitly for the Modifier tool.
- [ ] 300. Maintain rigorous parity checks against new versions of `onnx-modifier`.
