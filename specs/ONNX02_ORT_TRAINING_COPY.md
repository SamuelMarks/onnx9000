...
- [ ] Exclude `Constant` / Frozen nodes from gradient tracking (requires_grad=False)
- [ ] Implement Topological Sort to establish forward execution order
- [ ] Implement Forward pass activation caching (saving intermediate tensors required for backward)
- [ ] Map Forward nodes to their exact Vector-Jacobian Product (VJP) generator functions
- [ ] Statically allocate explicit output tensors for all computed gradients (`dY/dX`)
- [ ] Accumulate gradients (`Add`) explicitly when a tensor fans out to multiple consumers
- [ ] Construct the entire backward pass strictly using standard `ai.onnx` operators
- [ ] Ensure backward pass contains no specialized `ai.onnx.training` operators (maximizing portability)
- [ ] Eliminate mathematically redundant gradient calculations (`0 * dY -> 0`) natively
- [ ] Optimize sequential gradient additions into `Sum` operators
- [ ] Validate shape dimensionality mathematically during VJP injection
- [ ] Ensure VJP broadcasting rules exactly invert Forward broadcasting rules (e.g. `ReduceSum` on broadcasted axes)
- [ ] Map non-differentiable operations to `0` gradients (e.g., `ArgMax`)
- [ ] Allow users to explicitly stop gradients (`StopGradient` emulation)
...
- [ ] Output a single monolithic ONNX graph: Inputs (`X`, `Y_true`) -> Outputs (`Loss`, `Grads`)
- [ ] Expose API to yield intermediate gradients for debugging (`RetainGrad`)
- [ ] Provide analytical shape inference across the entire generated backward graph
- [ ] Ensure `Float32` precision is strictly maintained across gradient accumulation
- [ ] Support mixed-precision scaling (loss scaling for `Float16` backprop)
- [ ] Provide explicit sub-graph partitioning (Separating Forward.onnx and Backward.onnx)
- [ ] Support generating isolated Loss computation graphs
- [ ] Support generating isolated Optimizer step graphs
- [ ] Support dynamic shapes (`-1`) during gradient generation natively
- [ ] Validate symbolic gradient outputs match PyTorch `autograd` perfectly (atol=1e-5)
- [ ] Generate standard ONNX `GradientProto` if specifically requested by user flags
- [ ] Prevent Out-Of-Memory by actively freeing cached forward activations early during the backward pass (Simulated Memory Arena optimization)
- [ ] Track tensor dependencies explicitly to optimize memory reuse statically
...
- [ ] Implement non-differentiable markers for `Round`, `Floor`, `Ceil` (0 gradient)
- [ ] Implement non-differentiable markers for logical ops (`Equal`, `Less`)
...
- [ ] Handle Conv strides natively in VJP
- [ ] Handle Conv padding natively in VJP
- [ ] Handle Conv dilations natively in VJP
- [ ] Handle Conv groups (Depthwise) natively in VJP
...
- [ ] Implement VJP for `Flatten` (dX -> Reshape(dY, original_shape))
- [ ] Implement VJP for `Reshape` (dX -> Reshape(dY, original_shape))
- [ ] Implement VJP for `Transpose` (dX -> Transpose(dY, inverse_permutation))
- [ ] Implement VJP for `Squeeze` (dX -> Unsqueeze(dY))
- [ ] Implement VJP for `Unsqueeze` (dX -> Squeeze(dY))
- [ ] Implement VJP for `Concat` (dX_i -> Slice(dY, along axis i))
- [ ] Implement VJP for `Split` (dX -> Concat(dY_i))
- [ ] Implement VJP for `Slice` (dX -> Pad(dY, filling zero in sliced out regions))
- [ ] Implement VJP for `Gather` (dX -> ScatterND/ScatterElements(dY) accumulating on indices)
- [ ] Implement VJP for `ScatterElements` (dX -> Gather(dY))
- [ ] Implement VJP for `Tile` (dX -> ReduceSum(dY, along tiled axes))
- [ ] Implement VJP for `Expand` (dX -> ReduceSum(dY, along expanded axes))
...
- [ ] Compile `BinaryCrossEntropy` natively
- [ ] Compile `BinaryCrossEntropyWithLogits` natively (Fused for numerical stability)
- [ ] Compile `CategoricalCrossEntropy` natively
- [ ] Compile `SoftmaxCrossEntropyLoss` natively (ONNX operator representation)
- [ ] Compile `NegativeLogLikelihoodLoss` natively (ONNX operator representation)
- [ ] Compile `KullbackLeiblerDivergence` natively
- [ ] Compile `CosineEmbeddingLoss` natively
- [ ] Compile `MarginRankingLoss` natively
...
- [ ] Allow class-weighted loss application
- [ ] Provide reduction semantics: `mean`, `sum`, `none` natively
...
- [ ] AOT compile `AdamW` (Adam with weight decay) natively
- [ ] AOT compile `RMSProp` natively
- [ ] AOT compile `Adagrad` natively
- [ ] AOT compile `Adadelta` natively
...
- [ ] Implement Gradient Clipping (Global Norm) natively across all gradients before optimizer step
- [ ] Implement Gradient Clipping (Value based) natively
- [ ] Output a fully encapsulated `TrainingStep` graph: `[Inputs, Targets, LR, State] -> [Loss, New_States, New_Weights]`
- [ ] Output a simplified `AccumulateGradients` graph
...
- [ ] Test VJP shape preservation across `Reshape` boundaries exactly
- [ ] Test VJP un-broadcasting safely handles implicit right-alignment natively
- [ ] Test VJP of `MatMul` with implicit batch expansion
...
- [ ] Implement VJP for `Acos` (dY -> -dY / Sqrt(1 - X^2))
- [ ] Implement VJP for `Acosh` (dY -> dY / Sqrt(X^2 - 1))
- [ ] Implement VJP for `Asin` (dY -> dY / Sqrt(1 - X^2))
...
- [ ] Implement VJP for `Compress` (dX -> Scatter Elements on condition)
- [ ] Implement VJP for `Trilu` (dX -> Trilu(dY))
- [ ] Implement VJP for `GatherElements` (dX -> ScatterElements(dY))
- [ ] Implement VJP for `GatherND` (dX -> ScatterND(dY))
- [ ] Implement VJP for `ScatterND` (dX -> GatherND(dY), dUpdates -> GatherND(dY))
- [ ] Implement VJP for `ScatterElements` (dX -> GatherElements(dY), dUpdates -> GatherElements(dY))
...
- [ ] Implement VJP for `LayerNormalization` dynamically resolving axis constraints
- [ ] Implement VJP for `MaxRoiPool` (dX -> ScatterND to ArgMax indices per RoI)
- [ ] Implement VJP for `RoiAlign` (dX -> ScatterND using bilinear interpolation weights natively)
...
- [ ] Expand received 1D Gradients back into topological layers natively (`Split` + `Reshape`)
...
- [ ] Support Prefix Tuning dynamically via ONNX `Concat` node injection
- [ ] Support Prompt Tuning natively via ONNX `Gather` / `Concat` node injection
...
- [ ] Unit Test: Differentiate `Reshape` recursively inside a deeply nested `If` node
- [ ] Unit Test: Differentiate `Split` recursively inside a `Loop` node (BPTT - Backprop Through Time)
...
- [ ] Recompute `Gelu` natively during the backward pass
- [ ] Analyze exactly which Forward activations MUST be cached for the Backward pass (e.g. `MatMul` inputs)
- [ ] Analyze exactly which Forward activations can be safely discarded (e.g. `Add` inputs if shapes match)
...
- [ ] Manage random seeds natively for `Dropout` reproducibility between Forward and Backward passes
- [ ] Ensure `BatchNormalization` running mean/var updates are completely detached from gradient tracking
...
- [ ] Implement VJP for `Round` (Non-differentiable -> 0 gradient)
