...
- [ ] 001. Define the base `EagerTensor` class extending `onnx9000.Tensor`.
- [ ] 002. Define the `LazyTensor` class (stores an AST node reference rather than raw data).
- [ ] 003. Implement `Tensor` instantiation from JS Arrays (`const t = np.array([1, 2, 3])`).
- [ ] 004. Implement instantiation from TypedArrays (`Float32Array`, `Int32Array`).
- [ ] 005. Implement explicit `dtype` forcing during instantiation (`dtype='float16'`).
- [ ] 006. Implement `Tensor.shape` property getter.
- [ ] 007. Implement `Tensor.dtype` property getter.
- [ ] 008. Implement `Tensor.ndim` property getter.
- [ ] 009. Implement `Tensor.size` property getter.
- [ ] 010. Implement `Tensor.numpy()` / `Tensor.data()` to extract raw values.
- [ ] 011. Support printing tensors gracefully to the console (truncating large arrays).
- [ ] 012. Implement `np.zeros(shape, dtype)`.
- [ ] 013. Implement `np.ones(shape, dtype)`.
- [ ] 014. Implement `np.empty(shape, dtype)`.
- [ ] 015. Implement `np.full(shape, fill_value, dtype)`.
- [ ] 016. Implement `np.eye(N, M, k, dtype)`.
- [ ] 017. Implement `np.identity(n, dtype)`.
- [ ] 018. Implement `np.arange(start, stop, step, dtype)`.
- [ ] 019. Implement `np.linspace(start, stop, num, endpoint, dtype)`.
- [ ] 020. Implement automatic context management (switching between Lazy builder mode and Eager mode).

### Phase 2: Basic Mathematical Operations (Eager & Lazy)
- [ ] 021. Implement `add(a, b)` / `a.add(b)`.
- [ ] 022. Implement `subtract(a, b)` / `a.sub(b)`.
- [ ] 023. Implement `multiply(a, b)` / `a.mul(b)`.
- [ ] 024. Implement `divide(a, b)` / `a.div(b)`.
- [ ] 025. Implement `power(a, b)` / `a.pow(b)`.
- [ ] 026. Implement `mod(a, b)`.
- [ ] 027. Implement `absolute(a)` / `a.abs()`.
- [ ] 028. Implement `negative(a)` / `a.neg()`.
- [ ] 029. Implement `sign(a)`.
- [ ] 030. Implement `exp(a)`.
- [ ] 031. Implement `log(a)`.
- [ ] 032. Implement `log10(a)`.
- [ ] 033. Implement `log2(a)`.
- [ ] 034. Implement `sqrt(a)`.
- [ ] 035. Implement `square(a)`.
- [ ] 036. Implement `cbrt(a)`.
- [ ] 037. Implement `reciprocal(a)`.
- [ ] 038. Support implicit scalar-to-tensor broadcasting in all math ops (e.g., `a.add(5)`).
- [ ] 039. Support standard NumPy broadcasting rules (e.g., `[3, 1] + [1, 4] -> [3, 4]`).
- [ ] 040. Ensure Lazy mode emits `Constant` nodes automatically for scalar arguments.

### Phase 3: Trigonometric Operations
- [ ] 041. Implement `sin(a)`.
- [ ] 042. Implement `cos(a)`.
- [ ] 043. Implement `tan(a)`.
- [ ] 044. Implement `arcsin(a)`.
- [ ] 045. Implement `arccos(a)`.
- [ ] 046. Implement `arctan(a)`.
- [ ] 047. Implement `sinh(a)`.
- [ ] 048. Implement `cosh(a)`.
- [ ] 049. Implement `tanh(a)`.
- [ ] 050. Implement `arcsinh(a)`.
- [ ] 051. Implement `arccosh(a)`.
- [ ] 052. Implement `arctanh(a)`.
- [ ] 053. Implement `deg2rad(a)`.
- [ ] 054. Implement `rad2deg(a)`.

### Phase 4: Matrix & Linear Algebra Operations
- [ ] 055. Implement `matmul(a, b)`.
- [ ] 056. Implement `dot(a, b)`.
- [ ] 057. Implement `vdot(a, b)`.
- [ ] 058. Implement `inner(a, b)`.
- [ ] 059. Implement `outer(a, b)`.
- [ ] 060. Implement `tensordot(a, b, axes)`.
- [ ] 061. Implement `einsum(subscripts, ...operands)`.
- [ ] 062. Implement `transpose(a, axes)`.
- [ ] 063. Implement `a.T` shorthand for matrix transposition.
- [ ] 064. Implement `swapaxes(a, axis1, axis2)`.
- [ ] 065. Implement `trace(a, offset, axis1, axis2)`.
- [ ] 066. Implement `linalg.norm(x, ord, axis, keepdims)`.
- [ ] 067. Implement `linalg.det(a)`.
- [ ] 068. Implement `linalg.inv(a)` (using WASM fallbacks if ONNX natively lacks it).
- [ ] 069. Implement `linalg.solve(a, b)`.
- [ ] 070. Map complex linear algebra natively to ONNX loops or WebGPU compute if standard ops are insufficient.

### Phase 5: Reduction Operations
- [ ] 071. Implement `sum(a, axis, keepdims)`.
- [ ] 072. Implement `prod(a, axis, keepdims)`.
- [ ] 073. Implement `mean(a, axis, keepdims)`.
- [ ] 074. Implement `std(a, axis, keepdims)`.
- [ ] 075. Implement `var(a, axis, keepdims)`.
- [ ] 076. Implement `min(a, axis, keepdims)`.
- [ ] 077. Implement `max(a, axis, keepdims)`.
- [ ] 078. Implement `argmin(a, axis, keepdims)`.
- [ ] 079. Implement `argmax(a, axis, keepdims)`.
- [ ] 080. Implement `ptp(a, axis)` (Peak to Peak - Max minus Min).
- [ ] 081. Implement `all(a, axis, keepdims)`.
- [ ] 082. Implement `any(a, axis, keepdims)`.
- [ ] 083. Implement `cumsum(a, axis)`.
- [ ] 084. Implement `cumprod(a, axis)`.

### Phase 6: Shape Manipulation & Array Operations
- [ ] 085. Implement `reshape(a, newshape)`.
- [ ] 086. Implement `a.reshape(newshape)`.
- [ ] 087. Implement `ravel(a)` (flattening to 1D).
- [ ] 088. Implement `squeeze(a, axis)`.
- [ ] 089. Implement `expand_dims(a, axis)`.
- [ ] 090. Implement `broadcast_to(array, shape)`.
- [ ] 091. Implement `concatenate(arrays, axis)`.
- [ ] 092. Implement `stack(arrays, axis)`.
- [ ] 093. Implement `vstack(tup)`.
- [ ] 094. Implement `hstack(tup)`.
- [ ] 095. Implement `dstack(tup)`.
- [ ] 096. Implement `split(ary, indices_or_sections, axis)`.
- [ ] 097. Implement `array_split(ary, indices_or_sections, axis)`.
- [ ] 098. Implement `tile(A, reps)`.
- [ ] 099. Implement `repeat(a, repeats, axis)`.
