...
describe('CLI Commands Batch 4', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(fs.readFileSync).mockReturnValue({ buffer: new Uint8Array([1, 2, 3]).buffer } as any);
  ...
  describe('handleNewModelArchCommand', () => {
    ...
    it('processes command', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Scaffolding new model architecture for: my_arch...'),
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
  ...
  describe('handleOnnx2TfCommand', () => {
    ...
    it('processes command with default output', () => {
      ...
      expect(consoleLogSpy).not.toHaveBeenCalledWith(expect.stringContaining('INT8 quantization'));
      handleOnnx2TfCommand(['']);
    });
    ...
    it('processes command with --int8', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('INT8 quantization'));
    });
  });
  ...
  describe('handleOptimumCommand', () => {
    ...
    describe('export command', () => {
      ...
      it('processes boundary logic when model is empty', () => {
        ...
        handleOptimumCommand(['export', { startsWith: () => false, toString: () => '' } as any]);
        handleOptimumCommand([
          'export',
    ...
    describe('optimize command', () => {
      ...
      it('processes optimize with level and size', () => {
        handleOptimumCommand(['optimize', 'model.onnx', '--level', '3', '--optimize-size']);
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('Optimizing model.onnx at level 3 for size...'),
        );
      });
      ...
      it('processes boundary logic when model is empty', () => {
        handleOptimumCommand(['optimize', { startsWith: () => false, toString: () => '' } as any]);
        handleOptimumCommand(['optimize', 'model.onnx', '--level', { toString: () => '' } as any]);
        handleOptimumCommand([
          'optimize',
          { startsWith: () => false, toString: () => '' } as any,
          '--level',
    ...
    describe('quantize command', () => {
      ...
      it('processes quantize without type', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('Quantizing model.onnx with method dynamic...'),
        );
        handleOptimumCommand(['quantize', 'model.onnx', '--quantize']);
      });

      it('processes boundary logic when model is empty', () => {
        handleOptimumCommand(['quantize', { startsWith: () => false, toString: () => '' } as any]);
        handleOptimumCommand([
          'quantize',
          'model.onnx',
          '--quantize',
    ...
    it('errors on unknown command', () => {
      ...
      expect(processExitSpy).toHaveBeenCalledWith(1);
      handleOptimumCommand([{ startsWith: () => false, toString: () => '' } as any]);
      handleOptimumCommand(['']);
    });
  });
  ...
  describe('handleProgressiveLoadingCommand', () => {
    ...
    it('processes command', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Generating progressive loading chunks for model.onnx...'),
      );
      handleProgressiveLoadingCommand(['']);
    });
  });
  ...
  describe('handlePytorchCodegenCommand', () => {
    ...
    it('processes without output file', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('pytorch_code_mock'));
    });

    it('processes with output file -o', async () => {
      vi.mocked(fs.writeFileSync).mockClear();
      await handlePytorchCodegenCommand(['model.onnx', '-o', 'out.py']);
      expect(fs.writeFileSync).toHaveBeenCalledWith('out.py', 'pytorch_code_mock');
      expect(consoleLogSpy).toHaveBeenCalledWith(
