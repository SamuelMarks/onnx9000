...
describe('CLI Commands Batch 5', () => {
  ...
  describe('handleScriptCommand', () => {
    ...
    it('processes command without output', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Successfully compiled script. Use -o to save the output.'),
      );
      handleScriptCommand(['']);
    });
  ...
  describe('handleSparseCommand', () => {
    ...
    describe('prune command', () => {
      ...
      it('processes with default output and sparsity', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('Saving sparse model to model_sparse.onnx...'),
        );
      });

      it('processes with custom output and sparsity', () => {
        handleSparseCommand(['prune', 'model.onnx', '-o', 'out.onnx', '--sparsity', '0.5']);
        expect(consoleLogSpy).toHaveBeenCalledWith(
      ...
      it('handles undefined model fallback log', () => {
        ...
        const badModel = { startsWith: () => false, replace: () => 'r', toString: () => '' } as any;
  ...
  describe('handleSphinxDemoUICommand', () => {
    ...
    it('rejects when process exits with error', async () => {
      ...
      await expect(promise).rejects.toThrow('Sphinx Demo UI exited with code 1');
    });
