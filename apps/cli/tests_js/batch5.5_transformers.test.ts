...
describe('CLI Commands Batch 5', () => {
  ...
  describe('handleTransformersCommand', () => {
    ...
    it('processes command successfully', async () => {
      const mockPipe = vi.fn().mockResolvedValue({ label: 'POSITIVE', score: 0.99 });
    ...
    it('handles errors gracefully', async () => {
      vi.mocked(transformers.pipeline).mockRejectedValue(new Error('Init failed'));

      await handleTransformersCommand(['text-classification']);
      expect(consoleErrorSpy).toHaveBeenCalledWith('Pipeline execution failed:', 'Init failed');
    });
  });
});
