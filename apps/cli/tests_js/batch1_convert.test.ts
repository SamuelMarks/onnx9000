...
describe("handleConvertCommand coverage", () => {
  ...
  it("handles missing args", async () => {
    await expect(handleConvertCommand([])).rejects.toThrow(
      "Process exited with 1",
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
    processExitSpy.mockClear();

    await expect(handleConvertCommand(["--src", "keras"])).rejects.toThrow(
      "Process exited with 1",
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
    processExitSpy.mockClear();
    ...
    await expect(
      handleConvertCommand(["--src", "keras", "--dst", "onnx"]),
    ).rejects.toThrow("Process exited with 1");
    expect(processExitSpy).toHaveBeenCalledWith(1);
  });
  ...
  it("handles batch and single file conversions", async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("Converting file.h5"),
    );
  ...
  it("handles failing conversion", async () => {
    vi.mocked(mmdnn.convert).mockRejectedValueOnce(
