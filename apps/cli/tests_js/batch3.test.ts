...
describe('CLI Commands Batch 3', () => {
  ...
  describe('handleIreeCommand', () => {
    ...
    it('processes compile subCommand', async () => {
      ...
      expect(ireeCompiler.compileModel).toHaveBeenCalledWith('model.onnx', expect.any(Object));
      await handleIreeCommand(['compile']);
      expect(ireeCompiler.compileModel).toHaveBeenCalledWith('', expect.any(Object));
    });
    ...
    it('handles invalid subCommand', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid IREE command'));
    });
  });
  ...
  describe('handleJsonExtractCommand', () => {
    ...
    it('processes without output file', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Extracting JSON'));
    });
    ...
    it('handles big ints correctly', async () => {
      ...
      vi.mocked(core.load).mockResolvedValueOnce({ bigVal: 10n, somethingElse: 'value' });
      await handleJsonExtractCommand(['model.onnx']);
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('10n'));
    });

    it('handles buffers correctly', async () => {
      const core = await import('@onnx9000/core');
      vi.mocked(core.load).mockResolvedValueOnce({ data: new Uint8Array([1, 2, 3]) });
      await handleJsonExtractCommand(['model.onnx']);
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Buffer: 3 bytes'));
    });
  });
  ...
  describe('handleLlamaWebCommand', () => {
    ...
    it('processes without output file', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Generated text:'));
      handleLlamaWebCommand(['', '--prompt', 'test']);
    });

    it('processes with output file via -o', () => {
      vi.mocked(fs.writeFileSync).mockClear();
      handleLlamaWebCommand(['model.onnx', '--prompt', 'test', '-o', 'out.txt']);
      expect(fs.writeFileSync).toHaveBeenCalledWith('out.txt', expect.any(String));
      expect(consoleLogSpy).toHaveBeenCalledWith(
    ...
    it('processes with output file via --output', () => {
      vi.mocked(fs.writeFileSync).mockClear();
      handleLlamaWebCommand(['model.onnx', '--prompt', 'test', '--output', 'out.txt']);
      expect(fs.writeFileSync).toHaveBeenCalledWith('out.txt', expect.any(String));
...
describe('CLI Commands Batch 3 - Mobile Memory', () => {
  ...
  describe('handleMobileMemoryCommand', () => {
    ...
    it('processes command', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('Analyzing mobile memory usage for model.onnx...'),
      );
      handleMobileMemoryCommand(['']);
    });
  });
