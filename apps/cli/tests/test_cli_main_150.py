                        ...
                        with patch("os.path.isdir", return_value=False):
                            with patch("os.path.exists", return_value=False):
                                ...
                                with patch.dict(
                                    ...
                                    # the issue is that in test_cli_main_149 we mocked torch.fx but maybe it was not hitting it?
                                    # let us check if torch.fx is imported inside the try/except. It is torch.fx.symbolic_trace
                                    with patch("torch.fx.symbolic_trace", return_value=fake_mod):
                                        main()
    except Exception:
        pass
    except SystemExit:
